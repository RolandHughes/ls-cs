...
void ReportContext::createError( const QString &description, const QtMsgType type, const QUrl &id,
...
void ReportContext::error( const QString &message, const ReportContext::ErrorCode errorCode,
...
void ReportContext::error( const QString &msg, const QXmlName qname, const SourceLocationReflection *const reflection )
{
    ...
    createError( msg, QtFatalMsg, QUrl( namePool()->stringForNamespace( qname.namespaceURI() ) + QLatin1Char( '#' ) +
                                        namePool()->stringForLocalName(
