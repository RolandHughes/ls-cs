...
const QRegularExpression PatternPlatform::pattern( const DynamicContext::Ptr &context ) const
{
    ...
    {
        retvalPattern = parsePattern( m_operands.at( 1 )->evaluateSingleton( context ).stringValue(), QPatternOption::NoPatternOption,
                                      context );
    }
...
QRegularExpression PatternPlatform::parsePattern( const QString &pattern, QPatternOptionFlags flags,
        const ReportContext::Ptr &context ) const
{
    return parsePattern( pattern, flags, context, this );
}

QRegularExpression PatternPlatform::parsePattern( const QString &patternP, QPatternOptionFlags flags,
...
PatternPlatform::Flags PatternPlatform::parseFlags( const QString &flags, const DynamicContext::Ptr &context ) const
{
    ...
    for ( int i = 0; i < len; ++i )
    {
        ...
        QString message( QtXmlPatterns::tr( "%1 is an invalid flag for regular expressions. Valid flags are:" ).formatArg( formatKeyword(
                             flag ) ) );
...
Expression::Ptr PatternPlatform::compress( const StaticContext::Ptr &context )
{
    ...
    if ( m_operands.at( 1 )->is( IDStringValue ) )
    {
        ...
        m_pattern = parsePattern( m_operands.at( 1 )->evaluateSingleton( dynContext ).stringValue(), QPatternOption::NoPatternOption,
                                  dynContext );
