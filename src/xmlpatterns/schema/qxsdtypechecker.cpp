                                    ...
                                    AnySimpleType::Ptr *boundType ) const
{
    ...
    if ( !type->isDefinedBySchema() )
    {
        ...
        if ( BuiltinTypes::xsQName->wxsTypeMatches( type ) )
        {
            ...
            {
                errorMsg = QtXmlPatterns::tr( "%1 is not valid according to %2." ).formatArg( formatData( normalizedString ) ).formatArg(
                               formatType(
                                   m_namePool, type ) );
                return false;
            }
        ...
        const AnySimpleType::Ptr &type, QString &errorMsg ) const
{
    ...
    if ( facets.contains( XsdFacet::Pattern ) )
    {
        ...
        for ( int j = 0; j < multiValue.count(); ++j )
        {
            ...
            const QRegularExpression exp = PatternPlatform::parsePattern( pattern, QPatternOption::ExactMatchOption, m_context,
                                           m_reflection );
