            ...
            if ( wildcard->processContents() != XsdWildcard::Skip )
            {
                ...
                else
                {
                    if ( !validateElement( elementDeclaration, hasStateMachine ) )
                    {
                        if ( wildcard->processContents() == XsdWildcard::Strict )
                        {
                            error( QtXmlPatterns::tr( "Element %1 contains invalid content." ).formatArg( formatKeyword( m_namePool->displayName(
                                        name() ) ) ) );
                            return false;
                        }
                    }
...
bool XsdValidatingInstanceReader::validateElement( const XsdElement::Ptr &declaration, bool &hasStateMachine )
{
    ...
    else
    {
        if ( hasAttribute( m_xsiNilName ) )
        {
            ...
            if ( nil->hasError() )
            {
                error( QtXmlPatterns::tr( "Attribute %1 contains invalid data: %2" ).formatArg( formatKeyword(
                            QLatin1String( "nil." ) ) ).formatArg(
                           formatData( value ) ) );
                return false;
            }
