...
NamePool::NamePool()
{
    ...
    /* Namespaces. */
    {
        ...
        Q_ASSERT_X( m_namespaces.count() == StandardNamespaceCount, Q_FUNC_INFO,
                    qPrintable( QString::fromLatin1( "Expected is %1, actual is %2." ).formatArg( StandardNamespaceCount ).formatArg(
                                    m_namespaces.count() ) ) );
    }
    ...
    /* Prefixes. */
    {
        ...
        Q_ASSERT_X( m_prefixes.count() == StandardPrefixCount, Q_FUNC_INFO,
                    qPrintable( QString::fromLatin1( "Expected is %1, actual is %2." ).formatArg( StandardPrefixCount ).formatArg(
                                    m_prefixes.count() ) ) );
    }
