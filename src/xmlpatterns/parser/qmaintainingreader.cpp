...
template<typename TokenLookupClass, typename LookupKey> MaintainingReader<TokenLookupClass, LookupKey>::MaintainingReader(
    const typename ElementDescription<TokenLookupClass,
    ...
    if ( m_elementDescriptions.contains( elementName ) )
    {
        ...
        if ( !requiredButMissing.isEmpty() )
        {
            error( QtXmlPatterns::tr( "The attribute %1 must appear on element %2." )
                   .formatArgs( QPatternist::formatKeyword( TokenLookupClass::toString( *requiredButMissing.constBegin() ) ),
                                formatKeyword( name() ) ),
