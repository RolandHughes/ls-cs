...
static void getWordBreaks( const QString &str, QCharAttributes *attributes )
{
    ...
    for ( auto iter = str.begin(); iter != str.end(); ++iter, ++k )
    {
        ...
        else if ( tmpState == WB::NoBr )
        {
            ...
            if ( cur_bClass == QUnicodeTables::WordBreak_Extend || cur_bClass == QUnicodeTables::WordBreak_ZWJ
                    || cur_bClass == QUnicodeTables::WordBreak_Format )
            {
        ...
        else if ( tmpState == WB::Scan || tmpState == WB::Word )
        {
            ...
            for ( auto tmp_iter = iter + 1; tmp_iter != str.end(); ++tmp_iter, ++tmp_k )
            {
                ...
                if ( t_bClass == QUnicodeTables::WordBreak_Extend || t_bClass == QUnicodeTables::WordBreak_ZWJ
                        || t_bClass == QUnicodeTables::WordBreak_Format )
                {
                    // WB4: X(Extend|Format)* -> X
                    continue;
                }

                if ( t_bClass == last_bClass || ( tmpState == WB::Word && ( t_bClass == QUnicodeTables::WordBreak_HebrewLetter
                                                  || t_bClass == QUnicodeTables::WordBreak_ALetter ) ) )
                {
                    k    = tmp_k;
                    iter = tmp_iter;
