...
inline void perl_matcher<BidiIterator, Allocator, traits>::push_repeater_count( int i, repeater_count<BidiIterator> **s )
{
    ...
    ( void ) new ( pmp )saved_repeater<BidiIterator>( i, s, position,
            this->recursion_stack.size() ? this->recursion_stack.back().idx : ( INT_MIN + 3 ) );
    m_backup_state = pmp;
}

template <class BidiIterator, class Allocator, class traits>
inline void perl_matcher<BidiIterator, Allocator, traits>::push_single_repeat( std::size_t c, const re_repeat *r,
        BidiIterator last_position,
        int state_id )
{
...
template <class BidiIterator, class Allocator, class traits>
inline void perl_matcher<BidiIterator, Allocator, traits>::push_recursion( int idx, const re_syntax_base *p,
...
bool perl_matcher<BidiIterator, Allocator, traits>::match_dot_repeat_fast()
{
    ...
    const re_repeat *rep = static_cast<const re_repeat *>( pstate );
    bool greedy = ( rep->greedy ) && ( !( m_match_flags & regex_constants::match_any ) || m_independent );
    std::size_t count = static_cast<std::size_t>( ( std::min )( static_cast<std::size_t>( std::distance( position, last ) ),
