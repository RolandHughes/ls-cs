...
struct digraph : public std::pair<charT, charT>
{
    ...
    template <class Seq> digraph( const Seq &s ) : std::pair<charT, charT>()
    {
        assert( s.size() <= 2 );
        assert( s.size() );
        this->first = s[0];
...
public:
    ...
    void finalize( const typename traits::string_type::const_iterator iter_first,
...
template <class charT, class traits> basic_regex_creator<charT, traits>::basic_regex_creator( regex_data<charT, traits> *data )
    : m_pdata( data ), m_traits( *( data->m_ptraits ) ), m_last_state( nullptr ), m_repeater_id( 0 ), m_has_backrefs( false ),
...
re_syntax_base *basic_regex_creator<charT, traits>::append_set( const basic_char_set<charT, traits> &char_set )
{
    ...
    re_set_long<m_type> *result = static_cast<re_set_long<m_type>*>( append_state( syntax_element_long_set,
                                  sizeof( re_set_long<m_type> ) ) );
