...
#ifndef LSCSMETA_INTERNAL_2_H
#define LSCSMETA_INTERNAL_2_H
...
// classes for these 2 methods, located in lscsmeta.h around line 360
template<class E>
const QString &LSCS_ReturnType<E, typename std::enable_if<std::is_enum_v<E>>::type>::getName()
{
...
template<class E>
const QString &LSCS_ReturnType<QFlags<E>>::getName()
{
...
template<class T>
void lscs_namespace_register_enum( const char *name, std::type_index id, const char *scope )
{
    const_cast<QMetaObject_T<T>&>( T::staticMetaObject() ).register_enum( QString::fromUtf8( name ), id, QString::fromUtf8( scope ) );
}

template<class T>
void lscs_namespace_register_enum_data( const char *data )
{
...
// ** flags
template<class T>
void lscs_namespace_register_flag( const char *enumName, const char *scope, const char *flagName, std::type_index id )
{
