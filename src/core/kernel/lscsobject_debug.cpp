...
static void dumpRecursive( int level, QObject *object )
{
    ...
    if ( object )
    {
        ...
        qDebug( "%s%s::%s", buffer.constData(), lscsPrintable( object->metaObject()->className() ),
                lscsPrintable( object->objectName() ) );
...
void QObject::dumpObjectInfo()
{
...
#if defined(LSCS_SHOW_DEBUG_CORE)
    qDebug( "\n--  dumpObjectInfo  --\n" );
    qDebug( "  OBJECT %s::%s", lscsPrintable( this->metaObject()->className() ), objectName().isEmpty() ? "unnamed" :
            lscsPrintable( objectName() ) );
    ...
    for ( int index = 0; index < metaObject->methodCount(); ++index )
    {
        ...
        {
            qDebug( "        signal name: %s", lscsPrintable( signalMetaMethod.methodSignature() ) );
            ...
            if ( obj )
            {
                ...
                qDebug( "          --> %s::%s", lscsPrintable( receiverMetaObject->className() ),
                        obj->objectName().isEmpty() ? "unnamed" : lscsPrintable( obj->objectName() ) );
    ...
    for ( auto sender : senderList )
    {
        ...
        if ( obj )
        {
            ...
            qDebug( "          <-- %s::%s", lscsPrintable( senderMetaObject->className() ),
                    obj->objectName().isEmpty() ? "unnamed" : lscsPrintable( obj->objectName() ) );
