...
template <typename T> CSArgument<T>::CSArgument( const T &data )
    : m_data( data ), m_typeName( cs_typeToName<T>() )
{
}

template <typename T> CSArgument<T>::CSArgument( const T &data, const QString &typeName )
...
template <typename T> CSReturnArgument<T>::CSReturnArgument( T &data )
    : m_data( data )
{
}
