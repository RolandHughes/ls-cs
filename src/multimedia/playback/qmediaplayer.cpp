...
bool QMediaPlayerPrivate::isInChain( QUrl url )
{
    ...
    for ( QMediaPlaylist *current = rootMedia.playlist(); current
            && current != playlist; current = current->currentMedia().playlist() )
    {
    ...
    : QMediaObject( *new QMediaPlayerPrivate, parent, playerService( flags ) )
{
    ...
    else
    {
        ...
        d->networkAccessControl = dynamic_cast<QMediaNetworkAccessControl *>( d->service->requestControl(
                                      QMediaNetworkAccessControl_iid ) );
        ...
        {
            connect( d->networkAccessControl, &QMediaNetworkAccessControl::configurationChanged, this,
