...
void QCameraPrivate::initControls()
{
    ...
    if ( service )
    {
        ...
        viewfinderSettingsControl2 = dynamic_cast<QCameraViewfinderSettingsControl2 *>( service->requestControl(
                                         QCameraViewfinderSettingsControl2_iid ) );

        if ( ! viewfinderSettingsControl2 )
        {
            viewfinderSettingsControl = dynamic_cast<QCameraViewfinderSettingsControl *>( service->requestControl(
...
QCameraViewfinderSettings QCamera::viewfinderSettings() const
{
    ...
    if ( d->viewfinderSettingsControl )
    {
        ...
        {
            settings.setResolution( d->viewfinderSettingsControl->viewfinderParameter(
                                        QCameraViewfinderSettingsControl::Resolution ).toSize() );
        }

        if ( d->viewfinderSettingsControl->isViewfinderParameterSupported( QCameraViewfinderSettingsControl::MinimumFrameRate ) )
        {
            settings.setMinimumFrameRate( d->viewfinderSettingsControl->viewfinderParameter(
                                              QCameraViewfinderSettingsControl::MinimumFrameRate ).toReal() );
        }

        if ( d->viewfinderSettingsControl->isViewfinderParameterSupported( QCameraViewfinderSettingsControl::MaximumFrameRate ) )
        {
            settings.setMaximumFrameRate( d->viewfinderSettingsControl->viewfinderParameter(
                                              QCameraViewfinderSettingsControl::MaximumFrameRate ).toReal() );
        }

        if ( d->viewfinderSettingsControl->isViewfinderParameterSupported( QCameraViewfinderSettingsControl::PixelAspectRatio ) )
        {
            settings.setPixelAspectRatio( d->viewfinderSettingsControl->viewfinderParameter(
                                              QCameraViewfinderSettingsControl::PixelAspectRatio ).toSize() );
        }
...
void QCamera::setViewfinderSettings( const QCameraViewfinderSettings &settings )
{
    ...
    else if ( d->viewfinderSettingsControl )
    {
        ...
        if ( d->viewfinderSettingsControl->isViewfinderParameterSupported( QCameraViewfinderSettingsControl::MinimumFrameRate ) )
        {
            d->viewfinderSettingsControl->setViewfinderParameter( QCameraViewfinderSettingsControl::MinimumFrameRate,
                    settings.minimumFrameRate() );
        }

        if ( d->viewfinderSettingsControl->isViewfinderParameterSupported( QCameraViewfinderSettingsControl::MaximumFrameRate ) )
        {
            d->viewfinderSettingsControl->setViewfinderParameter( QCameraViewfinderSettingsControl::MaximumFrameRate,
                    settings.maximumFrameRate() );
        }

        if ( d->viewfinderSettingsControl->isViewfinderParameterSupported( QCameraViewfinderSettingsControl::PixelAspectRatio ) )
        {
            d->viewfinderSettingsControl->setViewfinderParameter( QCameraViewfinderSettingsControl::PixelAspectRatio,
    ...
    for ( const QCameraViewfinderSettings &s : supported )
    {
        if ( ( settings.resolution().isEmpty() || settings.resolution() == s.resolution() )
                && ( qFuzzyIsNull( settings.minimumFrameRate() )
                     || qFuzzyCompare( ( float )settings.minimumFrameRate(), ( float )s.minimumFrameRate() ) )
                && ( qFuzzyIsNull( settings.maximumFrameRate() )
                     || qFuzzyCompare( ( float )settings.maximumFrameRate(), ( float )s.maximumFrameRate() ) )
