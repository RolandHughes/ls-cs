...
QString QMediaStorageLocation::generateFileName( const QString &prefix, const QDir &dir, const QString &extension ) const
{
    ...
    if ( lastMediaIndex == 0 )
    {
        ...
        for ( const QString &fileName : dir.entryList( QStringList() << QString( "%1*.%2" ).formatArg( prefix ).formatArg(
                    extension ) ) )
        {
            const qint64 mediaIndex = fileName.mid( prefix.length(),
                                                    fileName.size() - prefix.length() - extension.length() - 1 ).toInteger<qint64>();
