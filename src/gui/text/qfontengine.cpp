...
bool QFontEngine::supportsScript( QChar::Script script ) const
{
    ...
    if ( getSfntTableData( MAKE_TAG( 'm', 'o', 'r', 't' ), nullptr, &len )
            || getSfntTableData( MAKE_TAG( 'm', 'o', 'r', 'x' ), nullptr, &len ) )
    {
        return true;
    }
...
QFontEngine *QFontEngineMulti::createMultiFontEngine( QFontEngine *fe, int script )
{
    ...
    while ( it != end && it.key() == key )
    {
        ...
        if ( fe == cachedEngine->engine( 0 ) || ( faceIsLocal
                && fe->faceId().filename == cachedEngine->engine( 0 )->faceId().filename ) )
        {
            engine = cachedEngine;
