...
public:
    ...
    typename std::enable_if < ! std::is_convertible<Func1, QObject *>::value, QAction * >::type addAction( const QString &text,
            Func1 slot, const QKeySequence &shortcut = QKeySequence() )
    {
        QAction *result = addAction( text );
    ...
    static QAction *exec( const QList<QAction *> &actionList, const QPoint &point, QAction *action = nullptr,
