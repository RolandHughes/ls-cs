...
static QString lscs_GetLongPathName( const QString &strShortPath )
{
...
#ifdef Q_OS_WIN
    QString longPath = lscs_GetLongPathName( path );
#else
...
bool QFileSystemModel::setData( const QModelIndex &idx, const QVariant &value, int role )
{
            ...
            || ! QDir( filePath( parent( idx ) ) ).rename( oldName, newName ) )
    {
        ...
        QMessageBox::information( nullptr, QFileSystemModel::tr( "Invalid Filename" ),
                                  QFileSystemModel::tr( "<b>\"%1\" is invalid.</b><br>"
...
#if defined(Q_OS_WIN)
    QString longNewPath = lscs_GetLongPathName( newPath );
...
void QFileSystemModelPrivate::removeNode( QFileSystemModelPrivate::QFileSystemNode *parentNode, const QString &name )
{
    ...
    {
        q->beginRemoveRows( parent, translateVisibleLocation( parentNode, vLocation ), translateVisibleLocation( parentNode,
                            vLocation ) );
    }
