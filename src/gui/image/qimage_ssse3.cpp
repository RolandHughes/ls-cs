...
Q_GUI_EXPORT void lscs_convert_rgb888_to_rgb32_ssse3( quint32 *dst, const uchar *src, int len )
{
    ...
    const __m128i shuffleMask = _mm_set_epi8( char( 0xff ), 9, 10, 11, char( 0xff ), 6, 7, 8, char( 0xff ), 3, 4, 5, char( 0xff ), 0,
                                1, 2 );

    // Mask the 4 last colors of a RGB888 vector with an offset of 1 (so the last 3 bytes are RGB)
    const __m128i shuffleMaskEnd = _mm_set_epi8( char( 0xff ), 13, 14, 15, char( 0xff ), 10, 11, 12, char( 0xff ), 7, 8, 9,
                                   char( 0xff ), 4, 5, 6 );
...
void convert_RGB888_to_RGB32_ssse3( QImageData *dest, const QImageData *src, Qt::ImageConversionFlags )
{
    ...
    {
        lscs_convert_rgb888_to_rgb32_ssse3( dest_data, src_data, src->width );
