...
init_context:
    ...
    {
        sslContext->errorStr = QSslSocket::tr( "Invalid or empty cipher list (%1)" ).formatArg(
    ...
    if ( !sslContext->sslConfiguration.localCertificate().isNull() )
    {
        ...
        {
            sslContext->errorStr = QSslSocket::tr( "Cannot provide a certificate with no key, %1" ).formatArg(
                                       QSslSocketBackendPrivate::getErrorsFromOpenSsl() );
            sslContext->errorCode = QSslError::UnspecifiedError;
            return sslContext;
        }
        ...
        {
            sslContext->errorStr = QSslSocket::tr( "Error loading local certificate, %1" ).formatArg(
                                       QSslSocketBackendPrivate::getErrorsFromOpenSsl() );
            sslContext->errorCode = QSslError::UnspecifiedError;
            return sslContext;
        }
        ...
        {
            sslContext->errorStr = QSslSocket::tr( "Error loading private key, %1" ).formatArg(
                                       QSslSocketBackendPrivate::getErrorsFromOpenSsl() );
            sslContext->errorCode = QSslError::UnspecifiedError;
            return sslContext;
        }
...
static int next_proto_cb( SSL *, unsigned char **out, unsigned char *outlen, const unsigned char *in, unsigned int inlen,
                          void *arg )
{
