...
public:
    ...
    static void readKdeSystemPalette( const QStringList &kdeDirs, int kdeVersion, QHash<QString, QSettings *> &kdeSettings,
                                      QPalette *pal );
...
static inline bool kdeColor( QPalette *pal, QPalette::ColorRole role, const QVariant &value )
{
    ...
    pal->setBrush( role, QColor( values.at( 0 ).toInteger<int>(), values.at( 1 ).toInteger<int>(),
                                 values.at( 2 ).toInteger<int>() ) );
        ...
        QPalette *pal )
{
    ...
    kdeColor( pal, QPalette::HighlightedText, readKdeSetting( "Colors:Selection/ForegroundNormal", kdeDirs, kdeVersion,
              kdeSettings ) );
