...
xcb_window_t QXcbNativeInterface::locateSystemTray( xcb_connection_t *conn, const QXcbScreen *screen )
{
    if ( m_sysTraySelectionAtom == XCB_ATOM_NONE )
    {
        ...
        xcb_intern_atom_cookie_t intern_c = xcb_intern_atom_unchecked( conn, true, net_sys_tray.size_storage(),
                                            net_sys_tray.constData() );
...
void *QXcbNativeInterface::atspiBus()
{
    ...
    if ( defaultConnection != nullptr )
    {
        ...
        xcb_get_property_reply_t *reply = Q_XCB_CALL2( xcb_get_property_reply( defaultConnection->xcb_connection(), cookie, nullptr ),
                                          defaultConnection );
...
QPlatformNativeInterface::FP_Context QXcbNativeInterface::handlerNativeResourceFunctionForContext(
    const QByteArray &resource ) const
{
    for ( int i = 0; i < m_handlers.size(); i++ )
    {
...
QPlatformNativeInterface::FP_Screen QXcbNativeInterface::handlerNativeResourceFunctionForScreen(
    const QByteArray &resource ) const
{
    for ( int i = 0; i < m_handlers.size(); i++ )
    {
...
QPlatformNativeInterface::FP_Window QXcbNativeInterface::handlerNativeResourceFunctionForWindow(
    const QByteArray &resource ) const
{
    for ( int i = 0; i < m_handlers.size(); i++ )
    {
