            ...
            switch ( devices[i].classes[c]->type )
            {
                ...
                case XIButtonClass:
                {
                    ...
                    if ( bci->num_buttons >= 7 )
                    {
                        ...
                        if ( ( !label6 || qatom( label6 ) == QXcbAtom::ButtonHorizWheelLeft ) && ( !label7
    ...
    if ( xiEvent->evtype == XI_Motion && scrollingDevice.orientations )
    {
        ...
        if ( QXcbWindow *platformWindow = platformWindowFromId( xiDeviceEvent->event ) )
        {
            ...
            if ( !angleDelta.isNull() )
            {
                ...
                QWindowSystemInterface::handleWheelEvent( platformWindow->window(), xiEvent->time, local, global, rawDelta, angleDelta,
                        modifiers );
            }
        }
    }
    else if ( xiEvent->evtype == XI_ButtonRelease && scrollingDevice.legacyOrientations )
    {
        ...
        if ( QXcbWindow *platformWindow = platformWindowFromId( xiDeviceEvent->event ) )
        {
            ...
            if ( !angleDelta.isNull() )
            {
                ...
                QWindowSystemInterface::handleWheelEvent( platformWindow->window(), xiEvent->time, local, global, QPoint(), angleDelta,
                        modifiers );
            }
        }
    }
