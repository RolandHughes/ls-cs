...
QString getEnglishName( const QString &familyName )
{
    ...
    memcpy( lf.lfFaceName, tmp.data(), sizeof( wchar_t ) * qMin( static_cast<std::wstring::size_type>( LF_FACESIZE - 1 ),
            tmp.size() ) );
...
LOGFONT QWindowsFontDatabase::fontDefToLOGFONT( const QFontDef &request )
{
    ...
    if ( fam == QLatin1String( "MS Sans Serif" ) && ( request.style == QFont::StyleItalic || ( -lf.lfHeight > 18
            && -lf.lfHeight != 24 ) ) )
    {
