...
static bool monitorData( HMONITOR hMonitor, QWindowsScreenData *data )
{
    ...
    data->geometry = QRect( QPoint( info.rcMonitor.left, info.rcMonitor.top ), QPoint( info.rcMonitor.right - 1,
                            info.rcMonitor.bottom - 1 ) );
    data->availableGeometry = QRect( QPoint( info.rcWork.left, info.rcWork.top ), QPoint( info.rcWork.right - 1,
                                     info.rcWork.bottom - 1 ) );
...
QPlatformScreen::SubpixelAntialiasingType QWindowsScreen::subpixelAntialiasingTypeHint() const
{
    ...
    {
        QSettings settings( QLatin1String( "HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Avalon.Graphics\\DISPLAY1" ),
