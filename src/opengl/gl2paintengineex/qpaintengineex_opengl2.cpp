...
void QGL2PaintEngineExPrivate::updateTextureFilter( GLenum target, GLenum wrapMode, bool smoothPixmapTransform,
        GLuint id )
{
    //glActiveTexture(GL_TEXTURE0 + LSCS_BRUSH_TEXTURE_UNIT); //### Is it always this texture unit?
...
void QGL2PaintEngineExPrivate::updateBrushTexture()
{
    ...
    if ( ( style >= Qt::Dense1Pattern ) && ( style <= Qt::DiagCrossPattern ) )
    {
        ...
        QImage texImage = lscs_imageForBrush( style, false );

        glActiveTexture( GL_TEXTURE0 + LSCS_BRUSH_TEXTURE_UNIT );
...
void QGL2PaintEngineExPrivate::updateBrushUniforms()
{
    ...
    else
    {
        ...
        shaderManager->currentProgram()->setUniformValue( location( QGLEngineShaderManager::BrushTexture ), LSCS_BRUSH_TEXTURE_UNIT );
    }
...
void QGL2PaintEngineExPrivate::updateMatrix()
{
    ...
    glVertexAttrib3fv( LSCS_PMV_MATRIX_1_ATTR, pmvMatrix[0] );
    glVertexAttrib3fv( LSCS_PMV_MATRIX_2_ATTR, pmvMatrix[1] );
    glVertexAttrib3fv( LSCS_PMV_MATRIX_3_ATTR, pmvMatrix[2] );
        ...
        bool opaque, bool pattern )
{
    ...
    {
        shaderManager->currentProgram()->setUniformValue( location( QGLEngineShaderManager::ImageTexture ), LSCS_IMAGE_TEXTURE_UNIT );
    }
    ...
    if ( ! d->ctx->contextHandle()->isOpenGLES() )
    {
        ...
        if ( fmt.majorVersion() < 3 || ( fmt.majorVersion() == 3 && fmt.minorVersion() < 1 )
                || ( fmt.majorVersion() == 3 && fmt.minorVersion() == 1
                     && ctx->contextHandle()->hasExtension( QByteArrayLiteral( "GL_ARB_compatibility" ) ) )
...
void QGL2PaintEngineExPrivate::resetGLState()
{
    ...
    ctx->d_func()->setVertexAttribArrayEnabled( LSCS_TEXTURE_COORDS_ATTR, false );
    ctx->d_func()->setVertexAttribArrayEnabled( LSCS_VERTEX_COORDS_ATTR, false );
    ctx->d_func()->setVertexAttribArrayEnabled( LSCS_OPACITY_ATTR, false );
...
void QGL2PaintEngineEx::stroke( const QVectorPath &path, const QPen &pen )
{
    ...
    if ( lscs_pen_is_cosmetic( pen, s->renderHints ) && ! lscs_scaleForTransform( s->transform(), nullptr ) )
    {
...
void QGL2PaintEngineEx::drawPixmap( const QRectF &dest, const QPixmap &pixmap, const QRectF &src )
{
    ...
    d->glActiveTexture( GL_TEXTURE0 + LSCS_IMAGE_TEXTURE_UNIT );
...
void QGL2PaintEngineExPrivate::drawCachedGlyphs( QFontEngine::GlyphFormat glyphFormat, QStaticTextItem *staticTextItem )
{
    ...
    if ( glyphFormat == QFontEngine::Format_A32 )
    {
        ...
        else
        {
            ...
            glActiveTexture( GL_TEXTURE0 + LSCS_MASK_TEXTURE_UNIT );
        ...
        int fragmentCount, const QPixmap &pixmap, QPainter::PixmapFragmentHints hints )
{
    ...
    glActiveTexture( GL_TEXTURE0 + LSCS_IMAGE_TEXTURE_UNIT );
