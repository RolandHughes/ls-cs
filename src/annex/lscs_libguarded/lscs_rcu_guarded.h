...
/**
   \headerfile lscs_rcu_guarded.h <LsCsLibGuarded/lscs_rcu_guarded.h>
...
template <typename T>
template <typename... Us> rcu_guarded<T>::rcu_guarded( Us &&... data )
...
template <typename T> rcu_guarded<T>::write_handle::write_handle( T *ptr )
    : m_ptr( ptr ), m_accessed( false )
{
}
