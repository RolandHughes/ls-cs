...
/**
   \headerfile lscs_rcu_list.h <LsCsLibGuarded/lscs_rcu_list.h>
...
template <typename T, typename M, typename Alloc> rcu_list<T, M, Alloc>::rcu_list()
{
    m_head.store( nullptr );
    m_tail.store( nullptr );
}

template <typename T, typename M, typename Alloc> rcu_list<T, M, Alloc>::rcu_list( const Alloc &alloc )
