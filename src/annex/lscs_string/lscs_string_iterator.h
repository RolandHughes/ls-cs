...
#ifndef LIB_LSCS_STRING_ITERATOR_H
#define LIB_LSCS_STRING_ITERATOR_H
...
class LIB_LSCS_STRING_EXPORT CsCharArrow
{
public:
...
// private methods
template <typename E, typename A> LsCsStringIterator<E,A>::LsCsStringIterator( v_iter data )
{
    m_iter = data;
}
...
public:
    ...
    template <typename U> LsCsStringReverseIterator( LsCsStringReverseIterator<U> iter )
...
template <typename T> decltype( std::declval<T>().operator*() ) LsCsStringReverseIterator<T>::operator*() const
{
    auto tmp = this->base();
    return ( --tmp ).operator*();
}
