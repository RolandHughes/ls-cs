...
#ifndef LIB_LSCS_INTERNAL_H
#define LIB_LSCS_INTERNAL_H
...
template<class ...Ts>
template<class T> TeaCup<Ts...>::TeaCup( T lambda )
    : TeaCup< typename removeLastType<Ts...>::type >( [this]()
{
...
template<class T> TeaCup<>::TeaCup( T )
{
}
...
template<class ...Ts> TeaCup_Data<Ts...>::TeaCup_Data( bool needs_copying, Ts...Vs )
    : TeaCup<Ts...>( [this]()
{
    return m_data;
} ),
...
// (1) lambda
template<class T> Bento<T>::Bento( T lambda )
    : m_lambda( lambda )
{
}
...
template<class FunctionReturn, class ...FunctionArgs> Bento<FunctionReturn ( * )( FunctionArgs... )>::Bento( FunctionReturn (
            *ptr )( FunctionArgs... ) ) :
    m_methodPtr( ptr )
{
}
...
template<class MethodClass, class MethodReturn, class...MethodArgs> Bento<MethodReturn( MethodClass::* )( MethodArgs... )>::Bento(
    MethodReturn( MethodClass::*ptr )( MethodArgs... ) )
    : m_methodPtr( ptr )
{
}
...
template<class MethodClass, class MethodReturn, class ...MethodArgs>
void Bento<MethodReturn( MethodClass::* )( MethodArgs... ) const>::invoke( SlotBase *receiver,
        const TeaCupAbstract *dataPack ) const
{
    if ( ! receiver )
    {
        return;
