...
public:
    static const char *name()
    {
        return "assignToHTMLImageElement";
    }
...
JSValue QtPixmapInstance::getMethod( ExecState *exec, const Identifier &propertyName )
{
    ...
    return new ( exec ) RuntimeMethod( exec, exec->lexicalGlobalObject(), WebCore::deprecatedGetDOMStructure<RuntimeMethod>( exec ),
                                       propertyName, methodList );
}
