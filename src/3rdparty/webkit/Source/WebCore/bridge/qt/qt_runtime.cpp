...
public:
    ...
    template<typename T>
    inline QWKNoDebug &operator<<( const T & )
    {
        return *this;
    }
};
...
static QVariantMap convertValueToQVariantMap( ExecState *exec, JSObject *object, HashSet<JSObject *> *visitedObjects,
        int recursionLimit )
{
    ...
    switch ( hint )
    {
        ...
        case QVariant::Time:
            if ( type == Date )
            {
                ...
                {
                    ret = QDateTime( QDate( gdt.year + 1900, gdt.month + 1, gdt.monthDay ), QTime( gdt.hour, gdt.minute, gdt.second ),
                                     QTimeZone::utc() );
                    dist = 0;
...
JSValue convertQVariantToValue( ExecState *exec, PassRefPtr<RootObject> root, const QVariant &variant )
{
    ...
    if ( type == QVariant::ByteArray )
    {
        ...
        return new ( exec ) JSC::JSByteArray( exec, JSC::JSByteArray::createStructure( exec->globalData(), jsNull() ),
                                              wtfByteArray.get() );
    }
...
bool QtRuntimeMetaMethod::getOwnPropertyDescriptor( ExecState *exec, const Identifier &propertyName,
...
bool QtRuntimeConnectionMethod::getOwnPropertyDescriptor( ExecState *exec, const Identifier &propertyName,
        PropertyDescriptor &descriptor )
{
    if ( propertyName == exec->propertyNames().length )
    {
...
QtConnectionObject::QtConnectionObject( JSGlobalData &globalData, PassRefPtr<QtInstance> instance, int signalIndex,
                                        JSObject *thisObject, JSObject *funcObject )
    : m_instance( instance )
