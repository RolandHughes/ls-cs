...
{
    WTF_MAKE_NONCOPYABLE( XMLHttpRequestStaticData );
...
{
    if ( responseTypeCode() != ResponseTypeDefault && responseTypeCode() != ResponseTypeText
            && responseTypeCode() != ResponseTypeDocument )
    {
        ec = INVALID_STATE_ERR;
        return 0;
    }
...
ArrayBuffer *XMLHttpRequest::responseArrayBuffer( ExceptionCode &ec )
{
    ...
    {
        m_responseArrayBuffer = ArrayBuffer::create( const_cast<char *>( m_binaryResponseBuilder->data() ),
                                static_cast<unsigned>( m_binaryResponseBuilder->size() ) );
...
void XMLHttpRequest::callReadyStateChangeListener()
{
    ...
    {
        m_progressEventThrottle.dispatchEvent( XMLHttpRequestProgressEvent::create( eventNames().readystatechangeEvent ),
...
void XMLHttpRequest::open( const String &method, const KURL &url, bool async, ExceptionCode &ec )
{
    ...
    if ( methodUpper == "COPY" || methodUpper == "DELETE" || methodUpper == "GET" || methodUpper == "HEAD"
            ...
            || methodUpper == "OPTIONS" || methodUpper == "POST" || methodUpper == "PROPFIND" || methodUpper == "PROPPATCH"
            || methodUpper == "PUT"
            || methodUpper == "UNLOCK" )
    {
...
void XMLHttpRequest::didFail( const ResourceError &error )
{
    ...
    {
        reportUnsafeUsage( scriptExecutionContext(),
                           "XMLHttpRequest cannot load " + error.failingURL() + ". " + error.localizedDescription() );
    }
...
void XMLHttpRequest::didFinishLoading( unsigned long identifier, double )
{
    ...
    InspectorInstrumentation::resourceRetrievedByXMLHttpRequest( scriptExecutionContext(), identifier,
            m_responseBuilder.toStringPreserveCapacity(), m_url, m_lastSendURL, m_lastSendLineNumber );
...
void XMLHttpRequest::didSendData( unsigned long long bytesSent, unsigned long long totalBytesToBeSent )
{
    ...
    {
        m_upload->dispatchEvent( XMLHttpRequestProgressEvent::create( eventNames().progressEvent, true, bytesSent,
                                 totalBytesToBeSent ) );
    }
...
void XMLHttpRequest::didReceiveData( const char *data, int len )
{
    ...
    bool useDecoder = responseTypeCode() == ResponseTypeDefault || responseTypeCode() == ResponseTypeText
                      || responseTypeCode() == ResponseTypeDocument;
