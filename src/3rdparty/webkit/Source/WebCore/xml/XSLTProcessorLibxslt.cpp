...
SOFT_LINK( libxslt, xsltApplyStylesheetUser, xmlDocPtr, ( xsltStylesheetPtr style, xmlDocPtr doc, const char **params,
           const char *output, FILE *profile, xsltTransformContextPtr userCtxt ), ( style, doc, params, output, profile, userCtxt ) )
...
SOFT_LINK( libxslt, xsltSaveResultTo, int, ( xmlOutputBufferPtr buf, xmlDocPtr result, xsltStylesheetPtr style ), ( buf, result,
           style ) )
...
SOFT_LINK( libxslt, xsltSetSecurityPrefs, int, ( xsltSecurityPrefsPtr sec, xsltSecurityOption option, xsltSecurityCheck func ),
           ( sec, option, func ) )
SOFT_LINK( libxslt, xsltSetCtxtSecurityPrefs, int, ( xsltSecurityPrefsPtr sec, xsltTransformContextPtr ctxt ), ( sec, ctxt ) )
SOFT_LINK( libxslt, xsltSecurityForbid, int, ( xsltSecurityPrefsPtr sec, xsltTransformContextPtr ctxt, const char *value ), ( sec,
           ctxt, value ) )
                                ...
                                xsltLoadType type )
{
    ...
    switch ( type )
    {
        case XSLT_LOAD_DOCUMENT:
        {
            ...
            bool requestAllowed = globalCachedResourceLoader->frame()
                                  && globalCachedResourceLoader->document()->securityOrigin()->canRequest( url );
...
static inline void setXSLTLoadCallBack( xsltDocLoaderFunc func, XSLTProcessor *processor,
...
static int writeToVector( void *context, const char *buffer, int len )
{
    ...
    WTF::Unicode::ConversionResult result = WTF::Unicode::convertUTF8ToUTF16( &stringCurrent, buffer + len, &bufferUChar,
                                            bufferUCharEnd );
...
static xsltStylesheetPtr xsltStylesheetPointer( RefPtr<XSLStyleSheet> &cachedStylesheet, Node *stylesheetRootNode )
{
    ...
    {
        cachedStylesheet = XSLStyleSheet::createForXSLTProcessor( stylesheetRootNode->parentNode() ? stylesheetRootNode->parentNode() :
                           stylesheetRootNode,
