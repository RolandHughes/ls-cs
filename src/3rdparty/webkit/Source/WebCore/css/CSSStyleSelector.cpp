...
{
    return e->hasTagName( htmlTag ) || e->hasTagName( headTag ) || e->hasTagName( bodyTag ) || e->hasTagName( divTag )
           || e->hasTagName( spanTag ) || e->hasTagName( brTag ) || e->hasTagName( aTag );
}
    ...
    , m_applyProperty( CSSStyleApplyProperty::sharedCSSStyleApplyProperty() )
{
    ...
    if ( root )
    {
        m_rootDefaultStyle = styleForElement( root, 0, false,
                                              true );    // don't ref, because the RenderStyle is allocated from global heap
    }
...
static void loadFullDefaultStyle()
{
    ...
    String defaultRules = String( htmlUserAgentStyleSheet,
                                  sizeof( htmlUserAgentStyleSheet ) ) + RenderTheme::defaultTheme()->extraDefaultStyleSheet();
    ...
    String quirksRules = String( quirksUserAgentStyleSheet,
                                 sizeof( quirksUserAgentStyleSheet ) ) + RenderTheme::defaultTheme()->extraQuirksStyleSheet();
...
static void loadViewSourceStyle()
{
    ...
    defaultViewSourceStyle->addRulesFromSheet( parseUASheet( sourceUserAgentStyleSheet, sizeof( sourceUserAgentStyleSheet ) ),
            screenEval() );
}
...
void CSSStyleSelector::matchRules( RuleSet *rules, int &firstRuleIndex, int &lastRuleIndex, bool includeEmptyRules )
{
    ...
    {
        ASSERT( m_styledElement );
        matchRulesForList( rules->getPseudoRules( m_element->shadowPseudoId().impl() ), firstRuleIndex, lastRuleIndex,
                           includeEmptyRules );
    }
...
void CSSStyleSelector::matchRulesForList( const Vector<RuleData> *rules, int &firstRuleIndex, int &lastRuleIndex,
...
EInsideLink CSSStyleSelector::SelectorChecker::determineLinkStateSlowCase( Element *element ) const
{
    ...
    if ( iface )
    {
        return iface->historyContains( QString( reinterpret_cast<QChar *>( url.data() ),
                                                url.size() ) ) ? InsideVisitedLink : InsideUnvisitedLink;
    }
...
bool CSSStyleSelector::canShareStyleWithElement( Node *node ) const
{
    ...
    if ( element->hasTagName( iframeTag ) || element->hasTagName( frameTag ) || element->hasTagName( embedTag )
            || element->hasTagName( objectTag ) || element->hasTagName( appletTag ) )
    {
        return false;
    }
...
PassRefPtr<RenderStyle> CSSStyleSelector::styleForDocument( Document *document )
{
    ...
    if ( Settings *settings = document->settings() )
    {
        ...
        fontDescription.setComputedSize( CSSStyleSelector::getComputedSizeFromSpecifiedSize( document, documentStyle.get(),
                                         fontDescription.isAbsoluteSize(), size, useSVGZoomRules ) );
    }
...
PassRefPtr<RenderStyle> CSSStyleSelector::styleForElement( Element *e, RenderStyle *defaultParent, bool allowSharing,
        bool resolveForRootDefault, bool matchVisitedPseudoClass )
{
    ...
    if ( !matchVisitedPseudoClass && m_parentStyle && ( m_parentStyle->insideLink() || e->isLink() )
    ...
    if ( visitedStyle )
    {
        ...
        {
            visitedStyle =
                0;    // We made the style to avoid timing attacks. Just throw it away now that we did that, since we don't need it.
        }
        else
        {
    ...
    if ( !loadedMediaStyleSheet && ( e->hasTagName( videoTag ) || e->hasTagName( audioTag ) ) )
    {
        ...
        String mediaRules = String( mediaControlsUserAgentStyleSheet,
                                    sizeof( mediaControlsUserAgentStyleSheet ) ) + RenderTheme::themeForPage( e->document()->page() )->extraMediaControlsStyleSheet();
    ...
    if ( !loadedFullScreenStyleSheet && e->document()->webkitIsFullScreen() )
    {
        ...
        String fullscreenRules = String( fullscreenUserAgentStyleSheet,
                                         sizeof( fullscreenUserAgentStyleSheet ) ) + RenderTheme::defaultTheme()->extraFullScreenStyleSheet();
...
PassRefPtr<RenderStyle> CSSStyleSelector::styleForKeyframe( const RenderStyle *elementStyle,
...
void CSSStyleSelector::adjustRenderStyle( RenderStyle *style, RenderStyle *parentStyle, Element *e )
{
    ...
    if ( style->display() != NONE )
    {
        ...
        if ( e && e->hasTagName( tableTag ) && ( style->textAlign() == WEBKIT_LEFT || style->textAlign() == WEBKIT_CENTER
        ...
        if ( style->display() == INLINE && style->styleType() == NOPSEUDO && parentStyle
...
PassRefPtr<CSSRuleList> CSSStyleSelector::pseudoStyleRulesForElement( Element *e, PseudoId pseudoId, bool authorOnly,
        bool includeEmptyRules, CSSRuleFilter filter )
{
...
inline bool CSSStyleSelector::checkSelector( const RuleData &ruleData )
{
    ...
    // Slow path.
    SelectorMatch match = m_checker.checkSelector( ruleData.selector(), m_element, &m_selectorAttrs, m_dynamicPseudo, false, false,
                          style(), m_parentNode ? m_parentNode->renderStyle() : 0 );
...
static inline bool isFastCheckableSelector( const CSSSelector *selector )
{
    ...
    {
        if ( selector->relation() != CSSSelector::Descendant && selector->relation() != CSSSelector::Child
...
inline bool fastCheckSingleSelector( const CSSSelector *&selector, const Element *&element,
                                     const CSSSelector *&topChildOrSubselector, const Element *&topChildOrSubselectorMatchElement )
{
...
CSSStyleSelector::SelectorMatch CSSStyleSelector::SelectorChecker::checkSelector( CSSSelector *sel, Element *e,
        HashSet<AtomicStringImpl *> *selectorAttrs, PseudoId &dynamicPseudo, bool isSubSelector, bool encounteredLink,
    ...
    switch ( relation )
    {
        ...
        case CSSSelector::SubSelector:
            ...
            if ( ( elementStyle || m_collectRulesOnly ) && dynamicPseudo != NOPSEUDO && dynamicPseudo != SELECTION &&
                    !( ( RenderScrollbar::scrollbarForStyleResolve() || dynamicPseudo == SCROLLBAR_CORNER || dynamicPseudo == RESIZER )
...
bool CSSStyleSelector::SelectorChecker::checkOneSelector( CSSSelector *sel, Element *e,
        HashSet<AtomicStringImpl *> *selectorAttrs, PseudoId &dynamicPseudo, bool isSubSelector, bool encounteredLink,
        RenderStyle *elementStyle, RenderStyle *elementParentStyle ) const
{
    ASSERT( e );
    ...
    if ( sel->hasAttribute() )
    {
        ...
        if ( elementStyle && ( !e->isStyledElement() || ( !static_cast<StyledElement *>( e )->isMappedAttribute( attr )
                               && attr != typeAttr && attr != readonlyAttr ) ) )
        {
    ...
    if ( sel->m_match == CSSSelector::PseudoClass )
    {
        ...
        else if ( dynamicPseudo != NOPSEUDO && ( RenderScrollbar::scrollbarForStyleResolve() || dynamicPseudo == SCROLLBAR_CORNER
        ...
        switch ( sel->pseudoType() )
        {
            ...
            case CSSSelector::PseudoAny:
                ...
                {
                    if ( checkSelector( selector, e, selectorAttrs, dynamicPseudo, true, encounteredLink, elementStyle,
                                        elementParentStyle ) == SelectorMatches )
                    {
                        return true;
                    }
                }
...
static Length convertToLength( CSSPrimitiveValue *primitiveValue, RenderStyle *style, RenderStyle *rootStyle, bool toFloat,
...
static Length convertToIntLength( CSSPrimitiveValue *primitiveValue, RenderStyle *style, RenderStyle *rootStyle,
                                  double multiplier = 1, bool *ok = 0 )
{
    return convertToLength( primitiveValue, style, rootStyle, false, multiplier, ok );
}

static Length convertToFloatLength( CSSPrimitiveValue *primitiveValue, RenderStyle *style, RenderStyle *rootStyle,
                                    double multiplier = 1, bool *ok = 0 )
{
...
void CSSStyleSelector::matchPageRulesForList( const Vector<RuleData> *rules, bool isLeftPage, bool isFirstPage,
...
// lists
        ...
        case CSSPropertyFontFamily:
        {
            ...
            else if ( isInitial )
            {
                ...
                {
                    setFontSize( fontDescription, fontSizeForKeyword( m_checker.m_document, CSSValueXxSmall + fontDescription.keywordSize() - 1,
                                 false ) );
                }
...
// shorthand properties
        ...
        case CSSPropertyBorderColor:
            if ( id == CSSPropertyBorder || id == CSSPropertyBorderColor )
            {
                if ( isInherit )
                {
                    m_style->setBorderTopColor( m_parentStyle->borderTopColor().isValid() ? m_parentStyle->borderTopColor() :
                                                m_parentStyle->color() );
                    m_style->setBorderBottomColor( m_parentStyle->borderBottomColor().isValid() ? m_parentStyle->borderBottomColor() :
                                                   m_parentStyle->color() );
                    m_style->setBorderLeftColor( m_parentStyle->borderLeftColor().isValid() ? m_parentStyle->borderLeftColor() :
                                                 m_parentStyle->color() );
                    m_style->setBorderRightColor( m_parentStyle->borderRightColor().isValid() ? m_parentStyle->borderRightColor():
                                                  m_parentStyle->color() );
                }
                else if ( isInitial )
                {
        ...
        case CSSPropertyFont:
            ...
            else if ( primitiveValue )
            {
                ...
                if ( fontDescription.isAbsoluteSize() )
                {
                    ...
                    fontDescription.setComputedSize( getComputedSizeFromSpecifiedSize( m_checker.m_document, m_style.get(),
                                                     fontDescription.isAbsoluteSize(), fontDescription.specifiedSize(), useSVGZoomRules( m_element ) ) );
        ...
        case CSSPropertyBorderBottomRightRadius:
        {
            ...
            {
                radiusWidth = Length( max( intMinForLength, min( intMaxForLength, pair->first()->computeLengthInt( style(), m_rootElementStyle,
                                           zoomFactor ) ) ), Fixed );
            }
            ...
            {
                radiusHeight = Length( max( intMinForLength, min( intMaxForLength, pair->second()->computeLengthInt( style(), m_rootElementStyle,
                                            zoomFactor ) ) ), Fixed );
            }
        ...
        case CSSPropertyWebkitBoxShadow:
        {
            if ( isInherit )
            {
                ...
                {
                    return m_style->setTextShadow( m_parentStyle->textShadow() ? adoptPtr( new ShadowData( *m_parentStyle->textShadow() ) ) :
                                                   nullptr );
                }
            ...
            for ( int i = 0; i < len; i++ )
            {
                ...
                OwnPtr<ShadowData> shadowData = adoptPtr( new ShadowData( x, y, blur, spread, shadowStyle, id == CSSPropertyWebkitBoxShadow,
        ...
        case CSSPropertyWebkitBoxReflect:
        {
            ...
            if ( reflectValue->offset() )
            {
                ...
                {
                    reflection->setOffset( Length( reflectValue->offset()->computeLengthIntForLength( style(), m_rootElementStyle, zoomFactor ),
                                                   Fixed ) );
                }
            }
        ...
        case CSSPropertyWebkitBackfaceVisibility:
            ...
            if ( primitiveValue )
            {
                m_style->setBackfaceVisibility( ( primitiveValue->getIdent() == CSSValueVisible ) ? BackfaceVisibilityVisible :
        ...
        case CSSPropertyWebkitColumnRule:
            if ( isInherit )
            {
                m_style->setColumnRuleColor( m_parentStyle->columnRuleColor().isValid() ? m_parentStyle->columnRuleColor() :
                                             m_parentStyle->color() );
        ...
        case CSSPropertyWebkitHyphenateLimitAfter:
        {
            ...
            {
                m_style->setHyphenationLimitAfter( min( primitiveValue->getIntValue( CSSPrimitiveValue::CSS_NUMBER ),
        ...
        case CSSPropertyWebkitHyphenateLimitBefore:
        {
            ...
            {
                m_style->setHyphenationLimitBefore( min( primitiveValue->getIntValue( CSSPrimitiveValue::CSS_NUMBER ),
                                                    static_cast<int>( numeric_limits<short>::max() ) ) );
            }
        ...
        case CSSPropertyWebkitDashboardRegion:
        {
            ...
            while ( region )
            {
                ...
                {
                    m_style->setDashboardRegion( StyleDashboardRegion::Circle, region->m_label, top, right, bottom, left,
                                                 region == first ? false : true );
                }
                else if ( region->m_isRectangle )
                {
                    m_style->setDashboardRegion( StyleDashboardRegion::Rectangle, region->m_label, top, right, bottom, left,
                                                 region == first ? false : true );
                }
        ...
        case CSSPropertyWebkitTextStrokeWidth:
        {
            ...
            switch ( primitiveValue->getIdent() )
            {
                ...
                case CSSValueThick:
                {
                    ...
                    width = CSSPrimitiveValue::create( result, CSSPrimitiveValue::CSS_EMS )->computeLengthFloat( style(), m_rootElementStyle,
                            zoomFactor );
                    break;
                }
        ...
        case CSSPropertyWebkitTransformStyle:
            ...
            if ( primitiveValue )
            {
                m_style->setTransformStyle3D( ( primitiveValue->getIdent() == CSSValuePreserve3d ) ? TransformStyle3DPreserve3D :
        ...
        case CSSPropertyWebkitPerspective:
        {
            ...
            else if ( type == CSSPrimitiveValue::CSS_NUMBER )
            {
                ...
                perspectiveValue = CSSPrimitiveValue::create( primitiveValue->getDoubleValue(),
                                   CSSPrimitiveValue::CSS_PX )->computeLengthFloat( style(), m_rootElementStyle, zoomFactor );
            }
            else
            {
                return;
...
bool CSSStyleSelector::pageSizeFromName( CSSPrimitiveValue *pageSizeName, CSSPrimitiveValue *pageOrientation, Length &width,
        Length &height )
{
...
{
    return Length( CSSPrimitiveValue::create( mm, CSSPrimitiveValue::CSS_MM )->computeLengthIntForLength( style(),
                   m_rootElementStyle ), Fixed );
}

Length CSSStyleSelector::inchLength( double inch ) const
{
    return Length( CSSPrimitiveValue::create( inch, CSSPrimitiveValue::CSS_IN )->computeLengthIntForLength( style(),
                   m_rootElementStyle ), Fixed );
}
...
void CSSStyleSelector::mapAnimationTimingFunction( Animation *animation, CSSValue *value )
{
    ...
    if ( value->isTimingFunctionValue() )
    {
        ...
        if ( timingFunction->isCubicBezierTimingFunctionValue() )
        {
            ...
            animation->setTimingFunction( CubicBezierTimingFunction::create( cubicTimingFunction->x1(), cubicTimingFunction->y1(),
        ...
        else if ( timingFunction->isStepsTimingFunctionValue() )
        {
            ...
            animation->setTimingFunction( StepsTimingFunction::create( stepsTimingFunction->numberOfSteps(),
...
void CSSStyleSelector::checkForGenericFamilyChange( RenderStyle *style, RenderStyle *parentStyle )
{
    ...
    {
        size = fontSizeForKeyword( m_checker.m_document, CSSValueXxSmall + childFont.keywordSize() - 1,
...
float CSSStyleSelector::getComputedSizeFromSpecifiedSize( Document *document, RenderStyle *style, bool isAbsoluteSize,
        float specifiedSize, bool useSVGZoomRules )
{
...
Color CSSStyleSelector::getColorFromPrimitiveValue( CSSPrimitiveValue *primitiveValue ) const
{
    ...
    if ( ident )
    {
        ...
        {
            col = m_element->isLink()
                  && m_checker.m_matchVisitedPseudoClass ? m_element->document()->visitedLinkColor() : m_element->document()->linkColor();
        }
...
bool CSSStyleSelector::affectedByViewportChange() const
{
    ...
    {
        if ( m_medium->eval( &m_viewportDependentMediaQueryResults[i]->m_expression ) !=
...
bool CSSStyleSelector::createTransformOperations( CSSValue *inValue, RenderStyle *style, RenderStyle *rootStyle,
    ...
    for ( unsigned i = 0; i < size; i++ )
    {
        ...
        switch ( transformValue->operationType() )
        {
            ...
            case WebKitCSSTransformValue::ScaleYTransformOperation:
            {
                ...
                operations.operations().append( ScaleTransformOperation::create( sx, sy, 1.0,
                                                getTransformOperationType( transformValue->operationType() ) ) );
                break;
            }
            ...
            case WebKitCSSTransformValue::Scale3DTransformOperation:
            {
                ...
                operations.operations().append( ScaleTransformOperation::create( sx, sy, sz,
                                                getTransformOperationType( transformValue->operationType() ) ) );
                break;
            }
            ...
            case WebKitCSSTransformValue::TranslateYTransformOperation:
            {
                ...
                operations.operations().append( TranslateTransformOperation::create( tx, ty, Length( 0, Fixed ),
                                                getTransformOperationType( transformValue->operationType() ) ) );
                break;
            }
            ...
            case WebKitCSSTransformValue::Translate3DTransformOperation:
            {
                ...
                operations.operations().append( TranslateTransformOperation::create( tx, ty, tz,
                                                getTransformOperationType( transformValue->operationType() ) ) );
                break;
            }
            ...
            case WebKitCSSTransformValue::RotateTransformOperation:
            {
                ...
                operations.operations().append( RotateTransformOperation::create( 0, 0, 1, angle,
                                                getTransformOperationType( transformValue->operationType() ) ) );
                break;
            }
            ...
            case WebKitCSSTransformValue::RotateZTransformOperation:
            {
                ...
                operations.operations().append( RotateTransformOperation::create( x, y, z, angle,
                                                getTransformOperationType( transformValue->operationType() ) ) );
                break;
            }
            ...
            case WebKitCSSTransformValue::SkewYTransformOperation:
            {
                ...
                operations.operations().append( SkewTransformOperation::create( angleX, angleY,
                                                getTransformOperationType( transformValue->operationType() ) ) );
                break;
            }
...
void CSSStyleSelector::loadPendingImages()
{
    ...
    for ( HashSet<int>::const_iterator it = m_pendingImageProperties.begin(); it != end; ++it )
    {
        ...
        switch ( currentProperty )
        {
            case CSSPropertyBackgroundImage:
            {
                for ( FillLayer *backgroundLayer = m_style->accessBackgroundLayers(); backgroundLayer;
            ...
            case CSSPropertyContent:
            {
                for ( ContentData *contentData = const_cast<ContentData *>( m_style->contentData() ); contentData;
            ...
            case CSSPropertyWebkitBorderImage:
            {
                ...
                if ( borderImage.image() && borderImage.image()->isPendingImage() )
                {
                    ...
                    m_style->setBorderImage( NinePieceImage( imageValue->cachedImage( cachedResourceLoader ), borderImage.slices(),
                                             borderImage.horizontalRule(), borderImage.verticalRule() ) );
                }
            ...
            case CSSPropertyWebkitMaskBoxImage:
            {
                ...
                if ( maskBoxImage.image() && maskBoxImage.image()->isPendingImage() )
                {
                    ...
                    m_style->setMaskBoxImage( NinePieceImage( imageValue->cachedImage( cachedResourceLoader ), maskBoxImage.slices(),
                                              maskBoxImage.horizontalRule(), maskBoxImage.verticalRule() ) );
                }
