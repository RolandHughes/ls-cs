...
class CSSStyleSelector
{
    ...
    WTF_MAKE_NONCOPYABLE( CSSStyleSelector );
    WTF_MAKE_FAST_ALLOCATED;
public:
    ...
    PassRefPtr<RenderStyle> styleForElement( Element *e, RenderStyle *parentStyle = 0, bool allowSharing = true,
            bool resolveForRootDefault = false, bool matchVisitedPseudoClass = false );

    void keyframeStylesForAnimation( Element *, const RenderStyle *, KeyframeList &list );

    PassRefPtr<RenderStyle> pseudoStyleForElement( PseudoId pseudo, Element *e, RenderStyle *parentStyle = 0,
            bool matchVisitedPseudoClass = false );
...
public:
    ...
    PassRefPtr<CSSRuleList> styleRulesForElement( Element *, bool authorOnly, bool includeEmptyRules = false,
            CSSRuleFilter filter = AllCSSRules );
...
public:
    void setStyle( PassRefPtr<RenderStyle> s )
    {
        m_style = s;    // Used by the document when setting up its root style.
    }
...
private:
    ...
    static float getComputedSizeFromSpecifiedSize( Document *, RenderStyle *, bool isAbsoluteSize, float specifiedSize,
...
public:
    ...
    {
        m_checker.allVisitedStateChanged();
    }
    void visitedStateChanged( LinkHash visitedHash )
    {
        m_checker.visitedStateChanged( visitedHash );
    }
    ...
    {
        return m_features.siblingRules;
    }
    ...
    {
        return m_features.usesLinkRules;
    }

    static bool createTransformOperations( CSSValue *inValue, RenderStyle *inStyle, RenderStyle *rootStyle,
...
private:
    ...
    void addMatchedRule( const RuleData *rule )
    {
    ...
    bool isRightPage( int pageIndex ) const
    {
        return !isLeftPage( pageIndex );
    }
...
public:
    ...
    public:
        ...
        SelectorMatch checkSelector( CSSSelector *, Element *, HashSet<AtomicStringImpl *> *selectorAttrs, PseudoId &dynamicPseudo,
                                     bool isSubSelector, bool encounteredLink, RenderStyle * = 0, RenderStyle *elementParentStyle = 0 ) const;
        bool checkOneSelector( CSSSelector *, Element *, HashSet<AtomicStringImpl *> *selectorAttrs, PseudoId &dynamicPseudo,
                               bool isSubSelector, bool encounteredLink, RenderStyle *, RenderStyle *elementParentStyle ) const;
