...
public:
    ...
    CSSPrimitiveValueCache *primitiveValueCache() const
    {
        return m_primitiveValueCache.get();
    }
    ...
    {
        return m_numParsedProperties > 0;
    }
    ...
    PassRefPtr<CSSValue> parseFillPositionComponent( CSSParserValueList *, unsigned &cumulativeFlags,
    ...
    bool parseTransformOrigin( int propId, int &propId1, int &propId2, int &propId3, RefPtr<CSSValue> &, RefPtr<CSSValue> &,
                               RefPtr<CSSValue> & );
    ...
    Vector<OwnPtr<CSSParserSelector> > *reusableSelectorVector()
    {
        return &m_reusableSelectorVector;
    }
    ...
    {
        m_selectorListRange.start = m_selectorListRange.end = 0;
    }
    void resetRuleBodyMarks()
    {
        m_ruleBodyRange.start = m_ruleBodyRange.end = 0;
    }
    void resetPropertyMarks()
    {
        m_propertyRange.start = m_propertyRange.end = UINT_MAX;
    }
    int lex( void *yylval );
    int token()
    {
