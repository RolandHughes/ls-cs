...
static bool parseColorValue( CSSMutableStyleDeclaration *declaration, int propertyId, const String &string, bool important,
                             bool strict )
{
    if ( !string.length() )
    {
...
static bool parseSimpleLengthValue( CSSMutableStyleDeclaration *declaration, int propertyId, const String &string, bool important,
                                    bool strict )
{
                                  ...
                                  RefPtr<CSSStyleSourceData> *styleSourceData )
{
    ...
    if ( m_currentRuleData )
    {
        ...
        for ( Vector<CSSPropertySourceData>::iterator it = m_currentRuleData->styleSourceData->propertyData.begin(),
                endIt = m_currentRuleData->styleSourceData->propertyData.end(); it != endIt; ++it )
        {
    ...
    switch ( static_cast<CSSPropertyID>( propId ) )
    {
        ...
        case CSSPropertyCursor:
        {
            ...
            if ( list )
            {
                ...
                else if ( value && ( ( value->id >= CSSValueAuto && value->id <= CSSValueWebkitGrabbing ) || value->id == CSSValueCopy
                                     || value->id == CSSValueNone ) )
                {
            ...
            else if ( ( value->id >= CSSValueAuto && value->id <= CSSValueWebkitGrabbing ) || value->id == CSSValueCopy
                      || value->id == CSSValueNone )
            {
                validPrimitive = true;
            }
        ...
        case CSSPropertyBorderBottomRightRadius:
        {
            ...
            RefPtr<CSSPrimitiveValue> parsedValue1 = primitiveValueCache()->createValue( value->fValue,
        ...
        case CSSPropertyWebkitLineClamp:
            ...
            validPrimitive = ( !id && ( value->unit == CSSPrimitiveValue::CSS_PERCENTAGE || value->fValue )
                               && validUnit( value, FInteger | FPercent | FNonNeg, false ) );
            break;
...
static bool parseBackgroundClip( CSSParserValue *parserValue, RefPtr<CSSValue> &cssValue,
                                 CSSPrimitiveValueCache *primitiveValueCache )
{
            ...
            if ( !parsedProperty[i] )
            {
                ...
                if ( parseFillProperty( properties[i], propId1, propId2, val1, val2 ) )
                {
                    ...
                    if ( properties[i] == CSSPropertyBackgroundOrigin || properties[i] == CSSPropertyWebkitMaskOrigin )
                    {
                        ...
                        {
                            addFillValue( clipValue,
                                          CSSInitialValue::createImplicit() );    // Some value was used for origin that is not supported by clip. Just reset clip instead.
                        }
                    }
...
CSSParser::SizeParameterType CSSParser::parseSizeParameter( CSSValueList *parsedValues, CSSParserValue *value,
        SizeParameterType prevParamType )
{
    switch ( value->id )
    {
        case CSSValueAuto:
        ...
        case 0:
            ...
            {
                parsedValues->append( primitiveValueCache()->createValue( value->fValue,
...
PassRefPtr<CSSValue> CSSParser::parseFillPositionComponent( CSSParserValueList *valueList, unsigned &cumulativeFlags,
        FillPositionFlag &individualFlag )
{
    int id = valueList->current()->id;
...
PassRefPtr<CSSValue> CSSParser::parseAnimationName()
{
    ...
    if ( value->unit == CSSPrimitiveValue::CSS_STRING || value->unit == CSSPrimitiveValue::CSS_IDENT )
    {
        if ( value->id == CSSValueNone || ( value->unit == CSSPrimitiveValue::CSS_STRING
                                            && equalIgnoringCase( value->string, "none" ) ) )
        {
...
{
    CSSParserValueList *args = val->function->args.get();
    return equalIgnoringCase( val->function->name, "format(" ) && ( args->current()->unit == CSSPrimitiveValue::CSS_STRING
        ...
        else if ( val->unit == CSSParserValue::Function )
        {
            ...
            if ( args && args->size() == 1 )
            {
                if ( equalIgnoringCase( val->function->name, "local(" ) && !expectComma
                        && ( args->current()->unit == CSSPrimitiveValue::CSS_STRING || args->current()->unit == CSSPrimitiveValue::CSS_IDENT ) )
                {
                    expectComma = true;
                    allowFormat = false;
                    CSSParserValue *a = args->current();
...
static bool parseColorIntOrPercentage( const UChar *&string, const UChar *end, const char terminator,
                                       CSSPrimitiveValue::UnitTypes &expect, int &value )
{
...
struct ShadowParseContext
{
    ...
    void commitValue()
    {
        ...
        if ( x || y || blur || spread || color || style )
        {
            ...
            values->append( ShadowValue::create( x.release(), y.release(), blur.release(), spread.release(), style.release(),
                                                 color.release() ) );
        }
...
struct BorderImageParseContext
{
    ...
    bool allowNumber() const
    {
        return m_allowNumber;
    }
    bool allowSlash() const
    {
        return m_allowSlash;
    }
    bool allowWidth() const
    {
        return m_allowWidth;
    }
    ...
    void commitSlash()
    {
        m_allowBreak = m_allowSlash = m_allowNumber = false;
        m_allowWidth = true;
    }
    ...
    PassRefPtr<CSSValue> commitBorderImage( CSSParser *p, bool important )
    {
        ...
        {
            m_bottom = m_primitiveValueCache->createValue( m_top->getDoubleValue(), ( CSSPrimitiveValue::UnitTypes )m_top->primitiveType() );
            m_left = m_primitiveValueCache->createValue( m_right->getDoubleValue(),
...
bool CSSParser::parseBorderImage( int propId, bool important, RefPtr<CSSValue> &result )
{
    ...
    while ( ( val = m_valueList->next() ) )
    {
        ...
        else if ( propId == CSSPropertyWebkitBorderImage && context.allowSlash() && val->unit == CSSParserValue::Operator
                  && val->iValue == '/' )
        {
            context.commitSlash();
        }
...
bool CSSParser::parseBorderRadius( int propId, bool important )
{
    ...
    for ( unsigned i = 0; i < num; ++i )
    {
        ...
        RefPtr<CSSPrimitiveValue> radius = primitiveValueCache()->createValue( value->fValue,
                                           static_cast<CSSPrimitiveValue::UnitTypes>( value->unit ) );
    ...
    m_implicitShorthand = true;
    addProperty( CSSPropertyBorderTopLeftRadius, primitiveValueCache()->createValue( Pair::create( radii[0][0].release(),
                 radii[1][0].release() ) ), important );
    addProperty( CSSPropertyBorderTopRightRadius, primitiveValueCache()->createValue( Pair::create( radii[0][1].release(),
                 radii[1][1].release() ) ), important );
    addProperty( CSSPropertyBorderBottomRightRadius, primitiveValueCache()->createValue( Pair::create( radii[0][2].release(),
                 radii[1][2].release() ) ), important );
    addProperty( CSSPropertyBorderBottomLeftRadius, primitiveValueCache()->createValue( Pair::create( radii[0][3].release(),
                 radii[1][3].release() ) ), important );
    m_implicitShorthand = false;
    return true;
}
...
bool CSSParser::parseDeprecatedGradient( RefPtr<CSSValue> &gradient )
{
    ...
    if ( gradientType == CSSRadialGradient )
    {
        ...
        static_cast<CSSRadialGradientValue *>( result.get() )->setFirstRadius( primitiveValueCache()->createValue( a->fValue,
    ...
    if ( gradientType == CSSRadialGradient )
    {
        ...
        static_cast<CSSRadialGradientValue *>( result.get() )->setSecondRadius( primitiveValueCache()->createValue( a->fValue,
                CSSPrimitiveValue::CSS_NUMBER ) );
    }
...
public:
        ...
        , m_unit( CSSParser::FUnknown )
    {
        ...
        else if ( equalIgnoringCase( name, "translate(" ) || equalIgnoringCase( name, "translatex(" )
                  || equalIgnoringCase( name, "translatey(" ) || equalIgnoringCase( name, "translatez(" ) )
        {
    ...
    WebKitCSSTransformValue::TransformOperationType type() const
    {
        return m_type;
    }
    ...
    bool unknown() const
    {
        return m_type == WebKitCSSTransformValue::UnknownTransformOperation;
    }
    bool hasCorrectArgCount( unsigned argCount )
    {
        return m_argCount == argCount || ( m_allowSingleArgument && argCount == 1 );
    }
...
bool CSSParser::parseTransformOrigin( int propId, int &propId1, int &propId2, int &propId3, RefPtr<CSSValue> &value,
                                      RefPtr<CSSValue> &value2, RefPtr<CSSValue> &value3 )
{
    ...
    switch ( propId )
    {
        ...
        case CSSPropertyWebkitTransformOriginZ:
        {
            if ( validUnit( m_valueList->current(), FLength, m_strict ) )
            {
                value = primitiveValueCache()->createValue( m_valueList->current()->fValue,
                        ( CSSPrimitiveValue::UnitTypes )m_valueList->current()->unit );
            }
...
bool CSSParser::parsePerspectiveOrigin( int propId, int &propId1, int &propId2, RefPtr<CSSValue> &value,
                                        RefPtr<CSSValue> &value2 )
{
    propId1 = propId;
    propId2 = propId;
...
bool CSSParser::parseTextEmphasisStyle( bool important )
{
    ...
    for ( CSSParserValue *value = m_valueList->current(); value; value = m_valueList->next() )
    {
        if ( value->unit == CSSPrimitiveValue::CSS_STRING )
        {
            ...
            addProperty( CSSPropertyWebkitTextEmphasisStyle, primitiveValueCache()->createValue( value->string,
                         CSSPrimitiveValue::CSS_STRING ), important );
            m_valueList->next();
            return true;
        }
        ...
        else if ( value->id == CSSValueDot || value->id == CSSValueCircle || value->id == CSSValueDoubleCircle
                  || value->id == CSSValueTriangle || value->id == CSSValueSesame )
        {
            if ( shape )
            {
                return false;
...
static inline int yyerror( const char * )
{
    return 1;
}
...
PassOwnPtr<Vector<OwnPtr<CSSParserSelector> > > CSSParser::sinkFloatingSelectorVector( Vector<OwnPtr<CSSParserSelector> >
        *selectorVector )
{
    if ( selectorVector )
    {
...
MediaQuery *CSSParser::createFloatingMediaQuery( MediaQuery::Restrictor restrictor, const String &mediaType,
        PassOwnPtr<Vector<OwnPtr<MediaQueryExp> > > expressions )
{
...
CSSRule *CSSParser::createFontFaceRule()
{
    ...
    for ( unsigned i = 0; i < m_numParsedProperties; ++i )
    {
        ...
        if ( ( id == CSSPropertyFontWeight || id == CSSPropertyFontStyle || id == CSSPropertyFontVariant )
                && property->value()->isPrimitiveValue() )
        {
        ...
        else if ( id == CSSPropertyFontFamily && ( !property->value()->isValueList()
                  || static_cast<CSSValueList *>( property->value() )->length() != 1 ) )
        {
...
void CSSParser::updateSpecifiersWithElementName( const AtomicString &namespacePrefix, const AtomicString &elementName,
        CSSParserSelector *specifiers )
{
    AtomicString determinedNamespace = namespacePrefix != nullAtom
                                       && m_styleSheet ? m_styleSheet->determineNamespace( namespacePrefix ) : m_defaultNamespace;
...
void CSSParser::markPropertyEnd( bool isImportantFound, bool isPropertyParsed )
{
    ...
    if ( m_propertyRange.start != UINT_MAX && m_currentRuleData )
    {
        ...
        m_currentRuleData->styleSourceData->propertyData.append(
            CSSPropertySourceData( name, value, isImportantFound, isPropertyParsed, SourceRange( start - m_ruleBodyRange.start,
                                   end - m_ruleBodyRange.start ) ) );
    }
