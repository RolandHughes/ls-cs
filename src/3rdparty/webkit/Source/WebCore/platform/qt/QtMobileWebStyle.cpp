...
QPixmap QtMobileWebStyle::findChecker( const QRect &rect, bool disabled ) const
{
    ...
    QPixmap result;
    static const QString prefix = QLatin1String( "$qt-maemo5-" ) + QLatin1String( metaObject()->className() )+
                                  QLatin1String( "-checker-" );
    QString key = prefix + QString::number( size ) + QLatin1String( "-" ) + ( disabled ? QLatin1String( "disabled" ) :
...
QPixmap QtMobileWebStyle::findRadio( const QSize &size, bool checked, bool disabled ) const
{
    ...
    static const QString prefix = QLatin1String( "$qt-maemo5-" ) + QLatin1String( metaObject()->className() ) +
                                  QLatin1String( "-radio-" );
    QString key = prefix + QString::number( size.width() ) + QLatin1String( "-" ) + QString::number(
                      size.height() ) + QLatin1String( "-" )
                  + ( disabled ? QLatin1String( "disabled" ) : QLatin1String( "enabled" ) ) + ( checked ? QLatin1String( "-checked" ) :
                          QLatin1String( "" ) );
...
void QtMobileWebStyle::drawControl( ControlElement element, const QStyleOption *option, QPainter *painter,
...
void QtMobileWebStyle::drawPrimitive( PrimitiveElement element, const QStyleOption *option, QPainter *painter,
                                      const QWidget *widget ) const
{
    switch ( element )
    {
...
void QtMobileWebStyle::drawComplexControl( ComplexControl control, const QStyleOptionComplex *option, QPainter *painter,
        const QWidget *widget ) const
{
    switch ( control )
    {
