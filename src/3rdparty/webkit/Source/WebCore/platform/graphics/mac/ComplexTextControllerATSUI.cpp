...
OSStatus ComplexTextController::ComplexTextRun::overrideLayoutOperation( ATSULayoutOperationSelector, ATSULineRef atsuLineRef,
        URefCon refCon, void *, ATSULayoutOperationCallbackStatus *callbackStatus )
{
    ...
    status = ATSUDirectGetLayoutDataArrayPtrFromLineRef( atsuLineRef, kATSUDirectDataLayoutRecordATSLayoutRecordCurrent, true,
             reinterpret_cast<void **>( &layoutRecords ), &count );
...
static void shapeArabic( const UChar *source, UChar *dest, unsigned totalLength )
{
    ...
    while ( shapingStart < totalLength )
    {
        ...
        unsigned charsWritten = shapeArabic( source + shapingStart, shapingEnd - shapingStart, dest + shapingStart,
                                             shapingEnd - shapingStart, U_SHAPE_LETTERS_SHAPE | U_SHAPE_LENGTH_FIXED_SPACES_NEAR, &shapingError );
...
ComplexTextController::ComplexTextRun::ComplexTextRun( ATSUTextLayout atsuTextLayout, const SimpleFontData *fontData,
        const UChar *characters, unsigned stringLocation, size_t stringLength, bool ltr, bool directionalOverride )
    : m_fontData( fontData )
...
static void disableLigatures( const SimpleFontData *fontData, ATSUStyle atsuStyle, TypesettingFeatures typesettingFeatures )
{
    ...
    // See bugzilla 5166.
    if ( ( typesettingFeatures & Ligatures ) || ( fontData->platformData().orientation() == Horizontal
            && fontData->platformData().allowsLigatures() ) )
    {
        return;
    }
...
void ComplexTextController::collectComplexTextRunsForCharactersATSUI( const UChar *cp, unsigned length, unsigned stringLocation,
        const SimpleFontData *fontData )
{
    ...
    m_complexTextRuns.append( ComplexTextRun::create( atsuTextLayout, fontData, cp, stringLocation, length, m_run.ltr(),
                              m_run.directionalOverride() ) );
}
