...
void Font::drawEmphasisMarks( GraphicsContext *context, const TextRun &run, const AtomicString &mark, const FloatPoint &point,
...
float Font::width( const TextRun &run, HashSet<const SimpleFontData *> *fallbackFonts, GlyphOverflow *glyphOverflow ) const
{
    ...
    if ( codePathToUse != Complex )
    {
        ...
        return floatWidthForSimpleText( run, 0, returnFallbackFonts ? fallbackFonts : 0, codePathToUse == SimpleWithGlyphOverflow
                                        || ( glyphOverflow && glyphOverflow->computeBounds ) ? glyphOverflow : 0 );
    }
...
Font::CodePath Font::codePath( const TextRun &run ) const
{
    ...
    for ( int i = 0; i < run.length(); i++ )
    {
        ...
        if ( c < 0x0591
                || c == 0x05BE ) // U+0591 through U+05CF excluding U+05BE Hebrew combining marks, Hebrew punctuation Paseq, Sof Pasuq and Nun Hafukha
        {
            continue;
        }
...
unsigned Font::expansionOpportunityCount( const UChar *characters, size_t length, TextDirection direction,
