...
struct IntPointHash
{
    ...
    {
        return WTF::intHash( static_cast<uint64_t>( p.x() ) << 32 | p.y() );
    }
...
template<> struct HashTraits<WebCore::IntPoint> : GenericHashTraits<WebCore::IntPoint>
{
    ...
    static WebCore::IntPoint emptyValue()
    {
        return WebCore::IntPoint( 0, std::numeric_limits<int>::min() );
    }

    static void constructDeletedValue( WebCore::IntPoint &slot )
    {
        slot = WebCore::IntPoint( std::numeric_limits<int>::min(), 0 );
    }
    static bool isDeletedValue( const WebCore::IntPoint &slot )
    {
        return slot == WebCore::IntPoint( std::numeric_limits<int>::min(), 0 );
    }
};
