...
GlyphData Font::glyphDataForCharacter( UChar32 c, bool mirror, FontDataVariant variant ) const
{
    ...
    if ( variant == NormalVariant )
    {
        ...
        while ( true )
        {
            ...
            if ( page )
            {
                ...
                if ( data.fontData && ( data.fontData->platformData().orientation() == Horizontal
    ...
    if ( characterFontData )
    {
        ...
        GlyphData data = fallbackPage
                         && fallbackPage->fontDataForCharacter( c ) ? fallbackPage->glyphDataForCharacter( c ) : characterFontData->missingGlyphData();
...
void Font::drawEmphasisMarksForSimpleText( GraphicsContext *context, const TextRun &run, const AtomicString &mark,
...
void Font::drawEmphasisMarks( GraphicsContext *context, const GlyphBuffer &glyphBuffer, const AtomicString &mark,
                                     ...
                                     GlyphOverflow *glyphOverflow ) const
{
    ...
    if ( glyphOverflow )
    {
        glyphOverflow->top = max<int>( glyphOverflow->top,
                                       ceilf( -it.minGlyphBoundingBoxY() ) - ( glyphOverflow->computeBounds ? 0 : fontMetrics().ascent() ) );
        glyphOverflow->bottom = max<int>( glyphOverflow->bottom,
                                          ceilf( it.maxGlyphBoundingBoxY() ) - ( glyphOverflow->computeBounds ? 0 : fontMetrics().descent() ) );
...
FloatRect Font::selectionRectForSimpleText( const TextRun &run, const FloatPoint &point, int h, int from, int to ) const
{
    ...
    if ( run.rtl() )
    {
        ...
        return FloatRect( floorf( point.x() + totalWidth - afterWidth ), point.y(),
                          roundf( point.x() + totalWidth - beforeWidth ) - floorf( point.x() + totalWidth - afterWidth ), h );
    }

    return FloatRect( floorf( point.x() + beforeWidth ), point.y(),
                      roundf( point.x() + afterWidth ) - floorf( point.x() + beforeWidth ), h );
}
