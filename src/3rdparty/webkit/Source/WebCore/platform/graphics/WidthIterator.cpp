...
WidthIterator::WidthIterator( const Font *font, const TextRun &run, HashSet<const SimpleFontData *> *fallbackFonts,
                              bool accountForGlyphBounds, bool forTextEmphasis )
    : m_font( font )
    , m_run( run )
    , m_end( run.length() )
        ...
        if ( hasExtraSpacing )
        {
            ...
            if ( treatAsSpace || ( expandAroundIdeographs && Font::isCJKIdeographOrSymbol( c ) ) )
            {
                ...
                if ( m_expansion )
                {
                    ...
                    if ( m_run.allowsTrailingExpansion() || ( m_run.ltr()
                            && currentCharacter + clusterLength < static_cast<size_t>( m_run.length() ) )
