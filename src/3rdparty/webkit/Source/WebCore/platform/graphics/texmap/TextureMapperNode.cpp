...
void TextureMapperNode::computeLayerType()
{
    ...
    if ( ( hasReplica && hasDescendantsWithContent ) || ( hasReplica && hasTransparency ) || ( hasTransparency
...
void TextureMapperNode::computeTransformations()
{
    ...
    m_transforms.target = TransformationMatrix( parent ? parent->m_transforms.forDescendants : TransformationMatrix() ).multiply(
                              m_transforms.local );
...
void TextureMapperNode::paint( TextureMapper *textureMapper, const TextureMapperContentLayer::PaintOptions &options )
{
    ...
    if ( textureMapper->allowSurfaceForRoot() || m_state.hasSurfaceDescendants )
    {
        ...
        textureMapper->paintToTarget( *m_surface.get(), options.viewportSize, options.transform, options.opacity * m_state.opacity,
                                      options.targetRect );
    }
...
bool TextureMapperNode::paintReplica( const TexmapPaintOptions &options )
{
    ...
    options.textureMapper->drawTexture( texture, replicaRect(), m_transforms.replica, replica->m_state.opacity,
                                        replicaMaskTexture ? replicaMaskTexture.get() : maskTexture.get() );
    options.textureMapper->drawTexture( texture, IntRect( IntPoint( 0, 0 ), options.surface->size() ), TransformationMatrix(), 1.0f,
                                        maskTexture.get() );
    options.textureMapper->bindSurface( options.surface );
    options.cache->mark( options.surface );
    options.textureMapper->drawTexture( *m_replicaSurface.get(), IntRect( IntPoint( 0, 0 ), options.surface->size() ),
                                        TransformationMatrix(), options.opacity, 0 );
    return true;
}
...
void TextureMapperNode::syncCompositingStateSelf( GraphicsLayerTextureMapper *graphicsLayer, TextureMapper *textureMapper )
{
    ...
    if ( changeMask & ( ChildrenTransformChange | Preserves3DChange | TransformChange | AnchorPointChange | SizeChange |
                        ContentsRectChange | BackfaceVisibilityChange | PositionChange | MaskLayerChange | DrawsContentChange | ContentChange |
                        ReplicaLayerChange ) )
    {
...
void TextureMapperNode::syncCompositingStateInternal( GraphicsLayerTextureMapper *graphicsLayer, bool recurse,
        TextureMapper *textureMapper )
{
    ...
    if ( m_state.maskLayer )
    {
        m_state.maskLayer->syncCompositingStateInternal( toGraphicsLayerTextureMapper( graphicsLayer->maskLayer() ), false,
                textureMapper );
    ...
    if ( m_state.replicaLayer )
    {
        m_state.replicaLayer->syncCompositingStateInternal( toGraphicsLayerTextureMapper( graphicsLayer->replicaLayer() ), false,
                textureMapper );
    }
