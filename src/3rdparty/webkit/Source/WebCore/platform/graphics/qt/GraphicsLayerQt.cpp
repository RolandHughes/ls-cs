...
public:
    ...
    static bool allowAcceleratedCompositingCache()
    {
        return QPixmapCache::cacheLimit() > gMinimumPixmapCacheLimit;
    }
...
#if ENABLE(TILED_BACKING_STORE)
    const bool requiresTiling = ( m_state.drawsContent && m_currentContent.contentType == HTMLContentType )
                                && ( m_size.width() > GRAPHICS_LAYER_TILING_THRESHOLD || m_size.height() > GRAPHICS_LAYER_TILING_THRESHOLD );
...
IntRect GraphicsLayerQtImpl::tiledBackingStoreVisibleRect()
{
    ...
    return mapFromScene( view->mapToScene(
                             view->viewport()->visibleRegion().boundingRect() ).boundingRect() ).boundingRect().toAlignedRect();
}
#endif
...
{
public:
    AnimationQtBase( GraphicsLayerQtImpl *layer, const KeyframeValueList &values, const IntSize &boxSize, const Animation *anim,
                     const QString &name )
...
public:
    AnimationQt( GraphicsLayerQtImpl *layer, const KeyframeValueList &values, const IntSize &boxSize, const Animation *anim,
                 const QString &name )
...
{
public:
    TransformAnimationQt( GraphicsLayerQtImpl *layer, const KeyframeValueList &values, const IntSize &boxSize, const Animation *anim,
                          const QString &name )
    ...
    virtual AnimatedPropertyID animatedProperty() const
    {
    ...
    virtual void applyFrame( const TransformOperations &sourceOperations, const TransformOperations &targetOperations,
                             qreal progress )
    {
...
{
public:
    OpacityAnimationQt( GraphicsLayerQtImpl *layer, const KeyframeValueList &values, const IntSize &boxSize, const Animation *anim,
                        const QString &name )
...
bool GraphicsLayerQt::addAnimation( const KeyframeValueList &values, const IntSize &boxSize, const Animation *anim,
