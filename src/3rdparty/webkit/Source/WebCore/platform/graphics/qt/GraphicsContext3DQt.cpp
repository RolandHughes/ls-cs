...
void GraphicsContext3DInternal::reshape( int width, int height )
{
    ...
    if ( m_attrs.alpha )
    {
        glTexImage2D( GraphicsContext3D::TEXTURE_2D, /* level */ 0, GraphicsContext3D::RGBA, width, height, /* border */ 0,
                      GraphicsContext3D::RGBA, GraphicsContext3D::UNSIGNED_BYTE, /* data */ 0 );
    }
    else
    {
        glTexImage2D( GraphicsContext3D::TEXTURE_2D, /* level */ 0, GraphicsContext3D::RGB, width, height, /* border */ 0,
                      GraphicsContext3D::RGB, GraphicsContext3D::UNSIGNED_BYTE, /* data */ 0 );
    }
    ...
    bindFramebuffer( GraphicsContext3D::FRAMEBUFFER, m_canvasFbo );
    framebufferTexture2D( GraphicsContext3D::FRAMEBUFFER, GraphicsContext3D::COLOR_ATTACHMENT0, GraphicsContext3D::TEXTURE_2D,
                          m_texture, 0 );

    if ( m_attrs.depth )
    {
        framebufferRenderbuffer( GraphicsContext3D::FRAMEBUFFER, GraphicsContext3D::DEPTH_ATTACHMENT, GraphicsContext3D::RENDERBUFFER,
                                 m_depthBuffer );
    }
    ...
    if ( m_attrs.stencil )
    {
        framebufferRenderbuffer( GraphicsContext3D::FRAMEBUFFER, GraphicsContext3D::STENCIL_ATTACHMENT, GraphicsContext3D::RENDERBUFFER,
                                 m_depthBuffer );
    }
...
void GraphicsContext3DInternal::paint( QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget )
{
    ...
    m_glWidget->makeCurrent();
    bindFramebuffer( GraphicsContext3D::FRAMEBUFFER, m_canvasFbo );
    glReadPixels( /* x */ 0, /* y */ 0, rect.width(), rect.height(), GraphicsContext3D::RGBA, GraphicsContext3D::UNSIGNED_BYTE,
                          imagePixels );
...
PassRefPtr<GraphicsContext3D> GraphicsContext3D::create( GraphicsContext3D::Attributes attrs, HostWindow *hostWindow,
...
void GraphicsContext3D::getAttachedShaders( Platform3DObject program, GC3Dsizei maxCount, GC3Dsizei *count,
...
void GraphicsContext3D::copyTexImage2D( GC3Denum target, GC3Dint level, GC3Denum internalformat, GC3Dint x, GC3Dint y,
...
void GraphicsContext3D::copyTexSubImage2D( GC3Denum target, GC3Dint level, GC3Dint xoffset, GC3Dint yoffset, GC3Dint x, GC3Dint y,
...
void GraphicsContext3D::framebufferRenderbuffer( GC3Denum target, GC3Denum attachment, GC3Denum renderbuffertarget,
...
void GraphicsContext3D::framebufferTexture2D( GC3Denum target, GC3Denum attachment, GC3Denum textarget, Platform3DObject texture,
        GC3Dint level )
{
    m_internal->m_glWidget->makeCurrent();
...
void GraphicsContext3D::readPixels( GC3Dint x, GC3Dint y, GC3Dsizei width, GC3Dsizei height, GC3Denum format, GC3Denum type,
                                    void *data )
{
    m_internal->m_glWidget->makeCurrent();
...
void GraphicsContext3D::vertexAttribPointer( GC3Duint index, GC3Dint size, GC3Denum type, GC3Dboolean normalized,
        GC3Dsizei stride, GC3Dintptr offset )
{
    m_internal->m_glWidget->makeCurrent();
    m_internal->vertexAttribPointer( index, size, type, normalized, stride,
...
void GraphicsContext3D::getFramebufferAttachmentParameteriv( GC3Denum target, GC3Denum attachment, GC3Denum paramName,
        GC3Dint *value )
{
    m_internal->m_glWidget->makeCurrent();
...
bool GraphicsContext3D::texImage2D( GC3Denum target, GC3Dint level, GC3Denum internalformat, GC3Dsizei width, GC3Dsizei height,
                                    GC3Dint border, GC3Denum format, GC3Denum type, const void *pixels )
{
    m_internal->m_glWidget->makeCurrent();
...
void GraphicsContext3D::texSubImage2D( GC3Denum target, GC3Dint level, GC3Dint xoff, GC3Dint yoff, GC3Dsizei width,
                                       GC3Dsizei height, GC3Denum format, GC3Denum type, const void *pixels )
{
    m_internal->m_glWidget->makeCurrent();
                                      ...
                                      Vector<uint8_t> &outputVector )
{
    ...
    if ( image->data() )
    {
        nativeImage = QImage::fromData( reinterpret_cast<const uchar *>( image->data()->data() ),
                                        image->data()->size() ).convertToFormat( QImage::Format_ARGB32 );
    }
    else
    {
    ...
    return packPixels( nativeImage.bits(), SourceFormatBGRA8, image->width(), image->height(), 0, format, type, neededAlphaOp,
                       outputVector.data() );
}
