        ...
        PathCoordinateMode mode )
{
    if ( mode == AbsoluteCoordinates )
    {
        m_stringBuilder.append( String::format( "C %.6lg %.6lg %.6lg %.6lg %.6lg %.6lg ", point1.x(), point1.y(), point2.x(), point2.y(),
...
void SVGPathStringBuilder::curveToCubicSmooth( const FloatPoint &point2, const FloatPoint &targetPoint, PathCoordinateMode mode )
{
    if ( mode == AbsoluteCoordinates )
    {
        m_stringBuilder.append( String::format( "S %.6lg %.6lg %.6lg %.6lg ", point2.x(), point2.y(), targetPoint.x(),
                                                targetPoint.y() ) );
    }
    else
    {
...
void SVGPathStringBuilder::curveToQuadratic( const FloatPoint &point1, const FloatPoint &targetPoint, PathCoordinateMode mode )
{
    if ( mode == AbsoluteCoordinates )
    {
        m_stringBuilder.append( String::format( "Q %.6lg %.6lg %.6lg %.6lg ", point1.x(), point1.y(), targetPoint.x(),
                                                targetPoint.y() ) );
    }
    else
    {
...
void SVGPathStringBuilder::arcTo( float r1, float r2, float angle, bool largeArcFlag, bool sweepFlag,
                                  const FloatPoint &targetPoint, PathCoordinateMode mode )
{
    if ( mode == AbsoluteCoordinates )
    {
        m_stringBuilder.append( String::format( "A %.6lg %.6lg %.6lg %d %d %.6lg %.6lg ", r1, r2, angle, largeArcFlag, sweepFlag,
                                                targetPoint.x(), targetPoint.y() ) );
    }
    else
    {
