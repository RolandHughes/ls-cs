...
static inline const SVGFontData *svgFontAndFontFaceElementForFontData( const SimpleFontData *fontData,
        SVGFontFaceElement *&fontFace, SVGFontElement *&font )
{
        ...
        for ( int i = from; i < to; ++i )
        {
            ...
            for ( ; it != end; ++it )
            {
                ...
                if ( identifier.isValid
                        && isCompatibleGlyph( identifier, isVerticalText, language, chars, i, i + identifier.unicodeStringLength ) )
                {
...
static float floatWidthOfSubStringUsingSVGFont( const Font *font, const TextRun &run, int extraCharsAvailable, int from, int to,
        int &charsConsumed, String &glyphName )
{
    ...
    if ( const SVGFontData *fontData = svgFontAndFontFaceElementForFontData( font->primaryFont(), fontFaceElement, fontElement ) )
    {
        ...
        SVGTextRunWalker<SVGTextRunWalkerMeasuredLengthData> runWalker( fontData, fontElement, data, floatWidthUsingSVGFontCallback,
                                 ...
                                 const FloatPoint &point, int from, int to ) const
{
    ...
    if ( const SVGFontData *fontData = svgFontAndFontFaceElementForFontData( primaryFont(), fontFaceElement, fontElement ) )
    {
        ...
        float xStartOffset = floatWidthOfSubStringUsingSVGFont( this, run, 0, run.rtl() ? to : 0, run.rtl() ? run.length() : from,
                             charsConsumed, glyphName );
        ...
        SVGTextRunWalker<SVGTextRunWalkerDrawTextData> runWalker( fontData, fontElement, data, drawTextUsingSVGFontCallback,
        ...
        for ( unsigned i = 0; i < numGlyphs; ++i )
        {
            ...
            if ( identifier.isValid )
            {
                ...
                if ( !identifier.pathData.isEmpty() )
                {
                    ...
                    if ( activePaintingResource->applyResource( referencingRenderObjectParent, referencingRenderObjectParentStyle, context,
                            resourceMode ) )
                    {
                        if ( referencingRenderObject )
                        {
            ...
            else
            {
                ...
                fallbackCharacterRun.setText( &data.fallbackCharacters[run.rtl() ? data.fallbackCharacters.size() - fallbackCharacterIndex - 1 :
                                                        fallbackCharacterIndex], 1 );
...
FloatRect Font::selectionRectForTextUsingSVGFont( const TextRun &run, const FloatPoint &point, int height, int from,
        int to ) const
{
    int charsConsumed;
    String glyphName;

    return FloatRect( point.x() + floatWidthOfSubStringUsingSVGFont( this, run, 0, run.rtl() ? to : 0,
                      run.rtl() ? run.length() : from, charsConsumed, glyphName ),
