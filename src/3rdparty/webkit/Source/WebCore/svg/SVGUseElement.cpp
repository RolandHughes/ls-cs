...
DEFINE_ANIMATED_BOOLEAN( SVGUseElement, SVGNames::externalResourcesRequiredAttr, ExternalResourcesRequired,
...
void SVGUseElement::svgAttributeChanged( const QualifiedName &attrName )
{
    ...
    if ( SVGURIReference::isKnownAttribute( attrName ) )
    {
        ...
        {
            OwnPtr<SVGDocumentExtensions::SVGPendingElements> clients( document()->accessSVGExtensions()->removePendingResource(
                        m_resourceId ) );
...
void dumpInstanceTree( unsigned int &depth, String &text, SVGElementInstance *targetInstance )
{
    ...
    text += String::format( "SVGElementInstance this=%p, (parentNode=%s (%p), firstChild=%s (%p), correspondingElement=%s (%p), directUseElement=%s (%p), shadowTreeElement=%s (%p), id=%s)\n",
                            ...
                            elementNodeName.latin1().data(), element, directUseElementName.latin1().data(), directUseElement,
                            shadowTreeElementNodeName.latin1().data(), shadowTreeElement, elementId.latin1().data() );
...
void SVGUseElement::buildShadowTree( SVGShadowTreeRootElement *shadowRoot, SVGElement *target,
