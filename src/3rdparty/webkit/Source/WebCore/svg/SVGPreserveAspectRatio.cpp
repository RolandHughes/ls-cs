...
SVGPreserveAspectRatio SVGPreserveAspectRatio::parsePreserveAspectRatio( const UChar *&currParam, const UChar *end, bool validate,
        bool &result )
{
...
AffineTransform SVGPreserveAspectRatio::getCTM( float logicX, float logicY, float logicWidth, float logicHeight, float physWidth,
        float physHeight ) const
{
    ...
    if ( ( logicalRatio < physRatio && ( m_meetOrSlice == SVG_MEETORSLICE_MEET ) ) || ( logicalRatio >= physRatio
            && ( m_meetOrSlice == SVG_MEETORSLICE_SLICE ) ) )
    {
        transform.scaleNonUniform( physHeight / logicHeight, physHeight / logicHeight );

        if ( m_align == SVG_PRESERVEASPECTRATIO_XMINYMIN || m_align == SVG_PRESERVEASPECTRATIO_XMINYMID
                || m_align == SVG_PRESERVEASPECTRATIO_XMINYMAX )
        {
            transform.translate( -logicX, -logicY );
        }
        else if ( m_align == SVG_PRESERVEASPECTRATIO_XMIDYMIN || m_align == SVG_PRESERVEASPECTRATIO_XMIDYMID
    ...
    if ( m_align == SVG_PRESERVEASPECTRATIO_XMINYMIN || m_align == SVG_PRESERVEASPECTRATIO_XMIDYMIN
            || m_align == SVG_PRESERVEASPECTRATIO_XMAXYMIN )
    {
        transform.translate( -logicX, -logicY );
    }
    else if ( m_align == SVG_PRESERVEASPECTRATIO_XMINYMID || m_align == SVG_PRESERVEASPECTRATIO_XMIDYMID
