...
bool JSDatabaseSyncPrototype::getOwnPropertyDescriptor( ExecState *exec, const Identifier &propertyName,
...
{
    return new ( exec ) JSDatabaseSyncPrototype( exec->globalData(), globalObject,
            JSDatabaseSyncPrototype::createStructure( globalObject->globalData(), globalObject->objectPrototype() ) );
}
...
EncodedJSValue JSC_HOST_CALL jsDatabaseSyncPrototypeFunctionTransaction( ExecState *exec )
{
    ...
    RefPtr<SQLTransactionSyncCallback> callback = JSSQLTransactionSyncCallback::create( asObject( exec->argument( 0 ) ),
