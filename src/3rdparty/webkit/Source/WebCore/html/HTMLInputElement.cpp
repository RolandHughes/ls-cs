...
void HTMLInputElement::parseMappedAttribute( Attribute *attr )
{
    ...
    else if ( attr->name() == multipleAttr || attr->name() == patternAttr || attr->name() == precisionAttr
              || attr->name() == stepAttr )
    {
        setNeedsValidityCheck();
    }
...
void HTMLInputElement::defaultEventHandler( Event *evt )
{
    ...
    bool callBaseClassEarly = isTextField() && ( evt->type() == eventNames().keydownEvent
                              || evt->type() == eventNames().keypressEvent );
...
bool HTMLInputElement::isSpeechEnabled() const
{
    ...
    return m_inputType->shouldRespectSpeechAttribute() && RuntimeEnabledFeatures::speechInputEnabled()
           && hasAttribute( webkitspeechAttr );
}
