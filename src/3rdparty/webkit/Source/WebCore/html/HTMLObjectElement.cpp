...
PassRefPtr<HTMLObjectElement> HTMLObjectElement::create( const QualifiedName &tagName, Document *document, HTMLFormElement *form,
        bool createdByParser )
{
...
void HTMLObjectElement::parametersForPlugin( Vector<String> &paramNames, Vector<String> &paramValues, String &url,
        String &serviceType )
{
    ...
    for ( Node *child = firstChild(); child; child = child->nextSibling() )
    {
        ...
        if ( url.isEmpty() && urlParameter.isEmpty() && ( equalIgnoringCase( name, "src" ) || equalIgnoringCase( name, "movie" )
...
bool HTMLObjectElement::shouldAllowQuickTimeClassIdQuirk()
{
    ...
    for ( unsigned i = 0; i < length; ++i )
    {
        ...
        if ( equalIgnoringCase( metaElement->name(), "generator" )
                && metaElement->content().startsWith( "Mac OS X Server Web Services Server", false ) )
        {
            return true;
        }
    }
...
bool HTMLObjectElement::hasValidClassId()
{
    ...
    if ( equalIgnoringCase( serviceType(), "application/x-qt-plugin" )
            || equalIgnoringCase( serviceType(), "application/x-qt-styled-widget" ) )
    {
        return true;
    }
...
void HTMLObjectElement::updateWidget( PluginCreationOption pluginCreationOption )
{
    ...
    bool success = beforeLoadAllowedLoad && hasValidClassId()
                   && loader->requestObject( this, url, getAttribute( nameAttr ), serviceType, paramNames, paramValues );
