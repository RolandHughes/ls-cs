...
PassRefPtr<HTMLTextAreaElement> HTMLTextAreaElement::create( const QualifiedName &tagName, Document *document,
...
void HTMLTextAreaElement::parseMappedAttribute( Attribute *attr )
{
    ...
    else if ( attr->name() == wrapAttr )
    {
        ...
        if ( equalIgnoringCase( attr->value(), "physical" ) || equalIgnoringCase( attr->value(), "hard" )
                || equalIgnoringCase( attr->value(), "on" ) )
        {
            wrap = HardWrap;
        }
...
{
    if ( renderer() && ( event->isMouseEvent() || event->isDragEvent() || event->isWheelEvent()
...
void HTMLTextAreaElement::handleBeforeTextInsertedEvent( BeforeTextInsertedEvent *event ) const
{
    ...
    unsigned selectionLength = focused() ? numGraphemeClusters( plainText(
