...
void FrameView::adjustViewSize()
{
    ...
    ScrollView::setScrollOrigin( IntPoint( -root->docLeft(), -root->docTop() ), !m_frame->document()->printing(),
                                 size == contentsSize() );
        ...
        if ( body && body->renderer() )
        {
            ...
            else if ( body->hasTagName( bodyTag ) )
            {
                ...
                RenderObject *o = rootRenderer->style()->overflowX() == OVISIBLE
                                  && document->documentElement()->hasTagName( htmlTag ) ? body->renderer() : rootRenderer;
...
{
    if ( m_useSlowRepaints || m_slowRepaintObjectCount > 0 || ( platformWidget() && m_fixedObjectCount > 0 ) || m_isOverlapped
            || !m_contentIsOpaque )
    {
        return true;
    }
...
bool FrameView::scrollToAnchor( const String &name )
{
    ...
    if ( m_frame->document()->isSVGDocument() )
    {
        ...
        else
        {
            ...
            {
                RefPtr<SVGViewElement> viewElement = anchorNode->hasTagName( SVGNames::viewTag ) ? static_cast<SVGViewElement *>
                                                     ( anchorNode ) : 0;
...
void FrameView::scrollToAnchor()
{
    ...
    anchorNode->renderer()->enclosingLayer()->scrollRectToVisible( rect, true, ScrollAlignment::alignToEdgeIfNeeded,
...
void FrameView::paintOverhangAreas( GraphicsContext *context, const IntRect &horizontalOverhangArea,
...
void FrameView::forceLayoutForPagination( const FloatSize &pageSize, float maximumShrinkFactor,
        Frame::AdjustViewSizeOrNot shouldAdjustViewSize )
{
