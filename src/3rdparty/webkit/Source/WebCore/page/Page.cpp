...
Page::Page( PageClients &pageClients )
    ...
    , m_deviceMotionController( RuntimeEnabledFeatures::deviceMotionEnabled() ? adoptPtr( new DeviceMotionController(
                                    pageClients.deviceMotionClient ) ) : nullptr )
    , m_deviceOrientationController( RuntimeEnabledFeatures::deviceOrientationEnabled() ? adoptPtr( new DeviceOrientationController(
                                         this, pageClients.deviceOrientationClient ) ) : nullptr )
#endif
#if ENABLE(MEDIA_STREAM)
    , m_mediaStreamController( RuntimeEnabledFeatures::mediaStreamEnabled() ? adoptPtr( new MediaStreamController(
                                   pageClients.mediaStreamClient ) ) : PassOwnPtr<MediaStreamController>() )
#endif
...
{
    return findString( target, ( caseSensitivity == TextCaseInsensitive ? CaseInsensitive : 0 ) |
                       ( direction == FindDirectionBackward ? Backwards : 0 ) | ( shouldWrap ? WrapAround : 0 ) );
}
...
SharedGraphicsContext3D *Page::sharedGraphicsContext3D()
{
    ...
    if ( !m_sharedGraphicsContext3D )
        ...
        m_sharedGraphicsContext3D = SharedGraphicsContext3D::create( chrome(),
                                    0 /*settings()->legacyAccelerated2dCanvasEnabled() ? 0 : SharedGraphicsContext3D::UseSkiaGPU*/ );
