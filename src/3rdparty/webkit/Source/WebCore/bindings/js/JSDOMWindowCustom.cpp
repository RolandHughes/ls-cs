...
{
    return new ( exec ) JSFunction( exec, exec->lexicalGlobalObject(), exec->lexicalGlobalObject()->functionStructure(), length,
                                    propertyName, nativeFunction );
}

static JSValue childFrameGetter( ExecState *exec, JSValue slotBase, const Identifier &propertyName )
{
    return toJS( exec, static_cast<JSDOMWindow *>( asObject( slotBase ) )->impl()->frame()->tree()->child( identifierToAtomicString(
                     propertyName ) )->domWindow() );
}
...
bool JSDOMWindow::getOwnPropertySlot( ExecState *exec, const Identifier &propertyName, PropertySlot &slot )
{
    ...
    if ( document->isHTMLDocument() )
    {
        ...
        if ( atomicPropertyName && ( static_cast<HTMLDocument *>( document )->hasNamedItem( atomicPropertyName )
...
bool JSDOMWindow::defineOwnProperty( JSC::ExecState *exec, const JSC::Identifier &propertyName,
...
JSValue JSDOMWindow::open( ExecState *exec )
{
    ...
    AtomicString frameName = exec->argument( 1 ).isUndefinedOrNull() ? "_blank" : ustringToAtomicString( exec->argument( 1 ).toString(
                                 exec ) );
    ...
    RefPtr<DOMWindow> openedWindow = impl()->open( urlString, frameName, windowFeaturesString, activeDOMWindow( exec ),
                                     firstDOMWindow( exec ) );
...
JSValue JSDOMWindow::showModalDialog( ExecState *exec )
{
    ...
    impl()->showModalDialog( urlString, dialogFeaturesString, activeDOMWindow( exec ), firstDOMWindow( exec ), setUpDialog,
                             &handler );
...
JSValue JSDOMWindow::addEventListener( ExecState *exec )
{
    ...
    impl()->addEventListener( ustringToAtomicString( exec->argument( 0 ).toString( exec ) ),
                              JSEventListener::create( asObject( listener ), this, false, currentWorld( exec ) ), exec->argument( 2 ).toBoolean( exec ) );
    return jsUndefined();
}
...
JSValue JSDOMWindow::removeEventListener( ExecState *exec )
{
    ...
    impl()->removeEventListener( ustringToAtomicString( exec->argument( 0 ).toString( exec ) ),
                                 JSEventListener::create( asObject( listener ), this, false, currentWorld( exec ) ).get(), exec->argument( 2 ).toBoolean( exec ) );
    return jsUndefined();
}
