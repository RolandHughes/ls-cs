...
JSValue jsStringSlowCase( ExecState *exec, JSStringCache &stringCache, StringImpl *stringImpl )
{
    ...
    stringCache.add( stringImpl, Weak<JSString>( exec->globalData(), wrapper, currentWorld( exec )->stringWrapperOwner(),
                     stringImpl ) );
    return wrapper;
}
...
void reportException( ExecState *exec, JSValue exception )
{
    ...
    ScriptExecutionContext *scriptExecutionContext = static_cast<JSDOMGlobalObject *>
...
{
    return new ( exec ) JSFunction( exec, exec->lexicalGlobalObject(), exec->lexicalGlobalObject()->functionStructure(), 0,
                                    propertyName, objectProtoFuncToString );
}
...
Structure *cacheDOMStructure( JSDOMGlobalObject *globalObject, Structure *structure, const ClassInfo *classInfo )
{
    ...
    return structures.set( classInfo, WriteBarrier<Structure>( globalObject->globalData(), globalObject,
                           structure ) ).first->second.get();
}
