...
void InlineBox::showLineTreeAndMark( const InlineBox *markedBox1, const char *markedLabel1, const InlineBox *markedBox2,
...
{
    if ( !paintInfo.shouldPaintWithinRoot( renderer() ) || ( paintInfo.phase != PaintPhaseForeground
    ...
    {
        childPoint = renderer()->containingBlock()->flipForWritingMode( toRenderBox( renderer() ), childPoint,
                     RenderBox::ParentToChildFlippingAdjustment );
    }
