...
void InlineFlowBox::addToLine( InlineBox *child )
{
    ...
    if ( !child->renderer()->isPositioned() )
    {
        if ( child->isText() )
        {
            ...
            if ( childStyle->letterSpacing() < 0 || childStyle->textShadow() || childStyle->textEmphasisMark() != TextEmphasisMarkNone
        ...
        else if ( !child->renderer()->isBR() && ( child->renderer()->style( m_firstLine )->boxShadow()
...
void InlineFlowBox::adjustPosition( float dx, float dy )
{
    ...
    {
        m_overflow->move( dx,
                          dy );    // FIXME: Rounding error here since overflow was pixel snapped, but nobody other than list markers passes non-integral values here.
    }
}
...
void InlineFlowBox::determineSpacingForFlowBoxes( bool lastLine, bool isLogicallyLastRunWrapped,
        RenderObject *logicallyLastRunRenderer )
{
    ...
    if ( parent() )
    {
        ...
        if ( !lineBoxList->lastLineBox()->isConstructed() )
        {
            ...
            bool isLastObjectOnLine = !isAnsectorAndWithinBlock( renderer(), logicallyLastRunRenderer )
                                      || ( isLastChildForRenderer( renderer(), logicallyLastRunRenderer ) && !isLogicallyLastRunWrapped );
        ...
        FontBaseline baselineType, VerticalPositionCache &verticalPositionCache )
{
    ...
    for ( InlineBox *curr = firstChild(); curr; curr = curr->nextOnLine() )
    {
        ...
        else if ( !inlineFlowBox || strictMode || inlineFlowBox->hasTextChildren()
...
void InlineFlowBox::placeBoxesInBlockDirection( int top, int maxHeight, int maxAscent, bool strictMode, int &lineTop,
        int &lineBottom, bool &setLineTop,
        int &lineTopIncludingMargins, int &lineBottomIncludingMargins, bool &hasAnnotationsBefore, bool &hasAnnotationsAfter,
    ...
    for ( InlineBox *curr = firstChild(); curr; curr = curr->nextOnLine() )
    {
        ...
        if ( childAffectsTopBottomPos )
        {
            if ( curr->renderer()->isRubyRun() )
            {
                ...
                {
                    int bottomRubyBaseLeading = ( curr->logicalHeight() - rubyBase->logicalBottom() ) + rubyBase->logicalHeight() -
                                                ( rubyBase->lastRootBox() ? rubyBase->lastRootBox()->lineBottom() : 0 );
            ...
            if ( curr->isInlineTextBox() )
            {
                ...
                if ( static_cast<InlineTextBox *>( curr )->getEmphasisMarkPosition( curr->renderer()->style( m_firstLine ),
            ...
            lineBottomIncludingMargins = max( lineBottom, max( lineBottomIncludingMargins,
...
inline void InlineFlowBox::addTextBoxVisualOverflow( InlineTextBox *textBox, GlyphOverflowAndFallbackFontsMap &textBoxDataMap,
        IntRect &logicalVisualOverflow )
{
    ...
    int logicalRightVisualOverflow = max( textBox->pixelSnappedLogicalRight() + childOverflowLogicalRight,
...
inline void InlineFlowBox::addReplacedChildOverflow( const InlineBox *inlineBox, IntRect &logicalLayoutOverflow,
        IntRect &logicalVisualOverflow )
{
...
bool InlineFlowBox::nodeAtPoint( const HitTestRequest &request, HitTestResult &result, int x, int y, int tx, int ty, int lineTop,
                                 int lineBottom )
{
    ...
    for ( InlineBox *curr = lastChild(); curr; curr = curr->prevOnLine() )
    {
        if ( ( curr->renderer()->isText() || !curr->boxModelObject()->hasSelfPaintingLayer() )
                && curr->nodeAtPoint( request, result, x, y, tx, ty, lineTop, lineBottom ) )
        {
    ...
    {
        renderer()->updateHitTestResult( result, flipForWritingMode( IntPoint( x - tx,
                                         y - ty ) ) ); // Don't add in m_x or m_y here, we want coords in the containing block's space.
...
void InlineFlowBox::paintFillLayers( const PaintInfo &paintInfo, const Color &c, const FillLayer *fillLayer, int _tx, int _ty,
                                     int w, int h, CompositeOperator op )
{
    if ( !fillLayer )
    {
        return;
...
void InlineFlowBox::paintFillLayer( const PaintInfo &paintInfo, const Color &c, const FillLayer *fillLayer, int tx, int ty, int w,
                                    int h, CompositeOperator op )
{
    ...
    else
    {
        ...
        boxModelObject()->paintFillLayerExtended( paintInfo, c, fillLayer, stripX, stripY, stripWidth, stripHeight, BackgroundBleedNone,
                this, w, h, op );
    }
}

void InlineFlowBox::paintBoxShadow( GraphicsContext *context, RenderStyle *s, ShadowStyle shadowStyle, int tx, int ty, int w,
                                    int h )
{
...
void InlineFlowBox::paintBoxDecorations( PaintInfo &paintInfo, int tx, int ty )
{
    ...
    if ( ( !parent() && m_firstLine && styleToUse != renderer()->style() ) || ( parent() && renderer()->hasBoxDecorations() ) )
    {
        ...
        if ( parent() && renderer()->style()->hasBorder() )
        {
            ...
            {
                boxModelObject()->paintBorder( context, tx, ty, w, h, renderer()->style(), BackgroundBleedNone, includeLogicalLeftEdge(),
...
void InlineFlowBox::paintMask( PaintInfo &paintInfo, int tx, int ty )
{
    ...
    else
    {
        ...
        boxModelObject()->paintNinePieceImage( paintInfo.context, stripX, stripY, stripWidth, stripHeight, renderer()->style(),
                                               maskNinePieceImage, compositeOp );
    }
...
int InlineFlowBox::computeOverAnnotationAdjustment( int allowedPosition ) const
{
    ...
    for ( InlineBox *curr = firstChild(); curr; curr = curr->nextOnLine() )
    {
        ...
        if ( curr->renderer()->isReplaced() && curr->renderer()->isRubyRun() )
        {
            ...
            {
                int bottomOfLastRubyTextLine = rubyText->logicalTop() + ( rubyText->lastRootBox() ? rubyText->lastRootBox()->lineBottom() :
        ...
        if ( curr->isInlineTextBox() )
        {
            ...
            if ( style->textEmphasisMark() != TextEmphasisMarkNone
                    && static_cast<InlineTextBox *>( curr )->getEmphasisMarkPosition( style, emphasisMarkPosition )
...
void InlineFlowBox::collectLeafBoxesInLogicalOrder( Vector<InlineBox *> &leafBoxesInLogicalOrder,
        CustomInlineBoxRangeReverse customReverseImplementation, void *userData ) const
{
...
void InlineFlowBox::showLineTreeAndMark( const InlineBox *markedBox1, const char *markedLabel1, const InlineBox *markedBox2,
        const char *markedLabel2, const RenderObject *obj, int depth ) const
{
