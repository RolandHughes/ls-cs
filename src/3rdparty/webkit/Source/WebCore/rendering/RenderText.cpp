...
static void makeCapitalized( String *string, UChar previous )
{
    ...
    for ( endOfWord = textBreakNext( boundary ); endOfWord != TextBreakDone;
            startOfWord = endOfWord, endOfWord = textBreakNext( boundary ) )
    {
        if ( startOfWord != 0 ) // Ignore first char of previous string
        {
            data[startOfWord - 1] = characters[startOfWord - 1] == noBreakSpace ? noBreakSpace : toTitleCase(
                                        stringWithPrevious[startOfWord] );
        }
...
void RenderText::styleDidChange( StyleDifference diff, const RenderStyle *oldStyle )
{
    ...
    else if ( oldStyle->font().needsTranscoding() != style()->font().needsTranscoding() || ( style()->font().needsTranscoding()
              && oldStyle->font().family().family() != style()->font().family().family() ) )
    {
        updateNeedsTranscoding();
        needsResetText = true;
    }
...
VisiblePosition RenderText::positionForPoint( const IntPoint &point )
{
    ...
    if ( firstTextBox() && pointBlockDirection <  firstTextBox()->root()->selectionBottom()
            && pointLineDirection < firstTextBox()->logicalLeft() )
    {
    ...
    if ( lastTextBox() && pointBlockDirection >= lastTextBox()->root()->selectionTop()
            && pointLineDirection >= lastTextBox()->logicalRight() )
    {
...
ALWAYS_INLINE float RenderText::widthFromCache( const Font &f, int start, int len, float xPos,
        HashSet<const SimpleFontData *> *fallbackFonts, GlyphOverflow *glyphOverflow ) const
{
...
void RenderText::computePreferredLogicalWidths( float leadWidth, HashSet<const SimpleFontData *> &fallbackFonts,
        GlyphOverflow &glyphOverflow )
{
...
float RenderText::width( unsigned from, unsigned len, float xPos, bool firstLine, HashSet<const SimpleFontData *> *fallbackFonts,
...
float RenderText::width( unsigned from, unsigned len, const Font &f, float xPos, HashSet<const SimpleFontData *> *fallbackFonts,
                         GlyphOverflow *glyphOverflow ) const
{
...
int RenderText::previousOffsetForBackwardDeletion( int current ) const
{
            ...
            || ( ( character >= HANGUL_SYLLABLE_START ) && ( character <= HANGUL_SYLLABLE_END ) ) )
    {
        ...
        while ( current > 0 && ( ( character = text.characterStartingAt( current - 1 ) ) >= HANGUL_CHOSEONG_START )
                && ( character <= HANGUL_SYLLABLE_END ) && ( ( character <= HANGUL_JONGSEONG_END ) || ( character >= HANGUL_SYLLABLE_START ) ) )
        {
            switch ( state )
            {
                case HangulStateV:
