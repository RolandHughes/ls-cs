    ...
    , m_determinedMarginBeforeQuirk( false )
{
    ...
    m_canCollapseMarginBeforeWithChildren = m_canCollapseWithChildren && ( beforeBorderPadding == 0 )
                                            && block->style()->marginBeforeCollapse() != MSEPARATE;
    ...
    m_canCollapseMarginAfterWithChildren = m_canCollapseWithChildren && ( afterBorderPadding == 0 ) &&
                                           ( block->style()->logicalHeight().isAuto() && block->style()->logicalHeight().value() == 0 )
                                           && block->style()->marginAfterCollapse() != MSEPARATE;
...
void RenderBlock::styleWillChange( StyleDifference diff, const RenderStyle *newStyle )
{
    ...
    if ( style() && parent() && diff == StyleDifferenceLayout && style()->position() != newStyle->position() )
    {
        ...
        if ( containsFloats() && !isFloating() && !isPositioned() && ( newStyle->position() == AbsolutePosition
...
void RenderBlock::addChildToAnonymousColumnBlocks( RenderObject *newChild, RenderObject *beforeChild )
{
    ...
    RenderBlock *beforeChildParent = toRenderBlock( beforeChild
                                     && beforeChild->parent()->isRenderBlock() ? beforeChild->parent() : lastChild() );
    ...
    {
        return toRenderBlock( immediateChild->previousSibling() )->addChildIgnoringAnonymousColumnBlocks( newChild,
                0 );    // Treat like an append.
    }
...
RenderBlock *RenderBlock::containingColumnsBlock( bool allowAnonymousColumnBlock )
{
    ...
    {
        if ( !curr->isRenderBlock() || curr->isFloatingOrPositioned() || curr->isTableCell() || curr->isRoot() || curr->isRenderView()
                || curr->hasOverflowClip()
...
void RenderBlock::makeChildrenAnonymousColumnBlocks( RenderObject *beforeChild, RenderBlock *newBlockBox, RenderObject *newChild )
{
    ...
    RenderBlock *post = 0;
    RenderBlock *block =
        this; // Eventually block will not just be |this|, but will also be a block nested inside |this|.  Assign to a variable
...
{
    if ( !isAnonymousBlock() && firstChild() && ( firstChild()->isAnonymousColumnsBlock()
...
void RenderBlock::moveChildTo( RenderBlock *to, RenderObject *child, RenderObject *beforeChild, bool fullRemoveInsert )
{
    ...
    to->children()->insertChildNode( to, children()->removeChildNode( this, child, fullRemoveInsert ), beforeChild,
                                     fullRemoveInsert );
}

void RenderBlock::moveChildrenTo( RenderBlock *to, RenderObject *startChild, RenderObject *endChild, RenderObject *beforeChild,
...
void RenderBlock::removeLeftoverAnonymousBlock( RenderBlock *child )
{
    ...
    if ( child->continuation() || ( child->firstChild() && ( child->isAnonymousColumnSpanBlock()
                                    || child->isAnonymousColumnsBlock() ) ) )
    {
        return;
    }
    ...
    if ( canMergeAnonymousBlocks && prev && next )
    {
        ...
        if ( prev->childrenInline() != next->childrenInline() )
        {
            ...
            blockChildrenBlock->children()->insertChildNode( blockChildrenBlock, inlineChildrenBlock,
                    prev == inlineChildrenBlock ? blockChildrenBlock->firstChild() : 0,
...
void RenderBlock::layoutBlock( bool relayoutChildren, int pageLogicalHeight )
{
    ...
    LayoutStateMaintainer statePusher( view(), this, IntSize( x(), y() ), hasColumns() || hasTransform() || hasReflection()
                                       || style()->isFlippedBlocksWritingMode(), pageLogicalHeight, pageLogicalHeightChanged, colInfo );
    ...
    if ( !didFullRepaint && repaintLogicalTop != repaintLogicalBottom && ( style()->visibility() == VISIBLE
            || enclosingLayer()->hasVisibleContent() ) )
    {
        ...
        {
            repaintRect = IntRect( repaintLogicalLeft, repaintLogicalTop, repaintLogicalRight - repaintLogicalLeft,
                                   repaintLogicalBottom - repaintLogicalTop );
        }
        else
        {
            repaintRect = IntRect( repaintLogicalTop, repaintLogicalLeft, repaintLogicalBottom - repaintLogicalTop,
                                   repaintLogicalRight - repaintLogicalLeft );
        }
...
bool RenderBlock::handleRunInChild( RenderBox *child )
{
    ...
    for ( RenderObject *runInChild = blockRunIn->firstChild(); runInChild; )
    {
        ...
        if ( runInIsGenerated || ( runInChild->style()->styleType() != BEFORE && runInChild->style()->styleType() != AFTER ) )
        {
            ...
            inlineRunIn->addChild(
                runInChild ); // Use addChild instead of appendChildNode since it handles correct placement of the children relative to :after-generated content.
        }
...
int RenderBlock::clearFloatsIfNeeded( RenderBox *child, MarginInfo &marginInfo, int oldTopPosMargin, int oldTopNegMargin,
                                      int yPos )
{
...
void RenderBlock::determineLogicalLeftPositionForChild( RenderBox *child )
{
    ...
    {
        int startOff = style()->isLeftToRightDirection() ? logicalLeftOffsetForLine( logicalHeight(),
                       false ) : totalAvailableLogicalWidth - logicalRightOffsetForLine( logicalHeight(), false );
    ...
    setLogicalLeftForChild( child, style()->isLeftToRightDirection() ? newPosition : totalAvailableLogicalWidth - newPosition -
                            logicalWidthForChild( child ), ApplyLayoutDelta );
}
...
void RenderBlock::setCollapsedBottomMargin( const MarginInfo &marginInfo )
{
    if ( marginInfo.canCollapseWithMarginAfter() && !marginInfo.canCollapseWithMarginBefore() )
    {
        ...
        // with our children.
        setMaxMarginAfterValues( max( maxPositiveMarginAfter(), marginInfo.positiveMargin() ), max( maxNegativeMarginAfter(),
...
void RenderBlock::layoutBlockChildren( bool relayoutChildren, int &maxFloatLogicalBottom )
{
    ...
    while ( next )
    {
        ...
        if ( relayoutChildren || ( ( child->style()->logicalHeight().isPercent() || child->style()->logicalMinHeight().isPercent()
                                     || child->style()->logicalMaxHeight().isPercent() ) && !isRenderView() ) )
        {
            child->setChildNeedsLayout( true, false );
        }
...
void RenderBlock::layoutBlockChild( RenderBox *child, MarginInfo &marginInfo, int &previousFloatLogicalBottom,
                                    int &maxFloatLogicalBottom )
{
    ...
    {
        maxFloatLogicalBottom = max( maxFloatLogicalBottom, addOverhangingFloats( toRenderBlock( child ), -child->logicalLeft(),
                                     -child->logicalTop(), !childNeededLayout ) );
    }
...
void RenderBlock::layoutPositionedObjects( bool relayoutChildren )
{
    ...
    if ( hasColumns() )
    {
        view()->layoutState()->m_columnInfo =
            columnInfo();    // FIXME: Kind of gross. We just put this back into the layout state so that pop() will work.
    }
}
...
void RenderBlock::markForPaginationRelayoutIfNeeded()
{
    ...
    if ( view()->layoutState()->pageLogicalHeightChanged() || ( view()->layoutState()->pageLogicalHeight()
            && view()->layoutState()->pageLogicalOffset( logicalTop() ) != pageLogicalOffset() ) )
    {
...
void RenderBlock::repaintOverhangingFloats( bool paintAllDescendants )
{
    ...
    for ( FloatingObjectSetIterator it = floatingObjectSet.begin(); it != end; ++it )
    {
        ...
        if ( logicalBottomForFloat( r ) > logicalHeight() && ( ( paintAllDescendants && r->m_renderer->isDescendantOf( this ) )
                || r->m_shouldPaint ) && !r->m_renderer->hasSelfPaintingLayer() )
        {
...
void RenderBlock::paint( PaintInfo &paintInfo, int tx, int ty )
{
    ...
    if ( hasOverflowClip() && style()->visibility() == VISIBLE && ( phase == PaintPhaseBlockBackground
            || phase == PaintPhaseChildBlockBackground ) && paintInfo.shouldPaintWithinRoot( this ) )
    {
...
void RenderBlock::paintColumnContents( PaintInfo &paintInfo, int tx, int ty, bool paintingFloats )
{
    ...
    for ( unsigned i = 0; i < colCount; i++ )
    {
        ...
        IntSize offset = isHorizontalWritingMode() ? IntSize( logicalLeftOffset, currLogicalTopOffset ) : IntSize( currLogicalTopOffset,
                         logicalLeftOffset );
        colRect.move( tx, ty );
...
void RenderBlock::paintObject( PaintInfo &paintInfo, int tx, int ty )
{
    ...
    // 5. paint outline.
    if ( ( paintPhase == PaintPhaseOutline || paintPhase == PaintPhaseSelfOutline ) && hasOutline()
            && style()->visibility() == VISIBLE )
    {
...
void RenderBlock::paintFloats( PaintInfo &paintInfo, int tx, int ty, bool preservePhase )
{
    ...
    for ( FloatingObjectSetIterator it = floatingObjectSet.begin(); it != end; ++it )
    {
        ...
        if ( r->m_shouldPaint && !r->m_renderer->hasSelfPaintingLayer() )
        {
            ...
            IntPoint childPoint = flipFloatForWritingMode( r, IntPoint( tx + xPositionForFloatIncludingMargin( r ) - r->m_renderer->x(),
                                  ty + yPositionForFloatIncludingMargin( r ) - r->m_renderer->y() ) );
...
static void clipOutPositionedObjects( const PaintInfo *paintInfo, const IntPoint &offset,
                                      RenderBlock::PositionedObjectsListHashSet *positionedObjects )
{
                                     ...
                                     int &lastLogicalTop, int &lastLogicalLeft, int &lastLogicalRight, const PaintInfo *paintInfo )
{
    ...
    if ( paintInfo )
    {
        ...
        if ( isBody() || isRoot() ) // The <body> must make sure to examine its containingBlock's positioned objects.
            ...
            {
                clipOutPositionedObjects( paintInfo, IntPoint( cb->x(), cb->y() ),
                                          cb->m_positionedObjects.get() );    // FIXME: Not right for flipped writing modes.
            }
    ...
    if ( childrenInline() )
    {
        result = inlineSelectionGaps( rootBlock, rootBlockPhysicalPosition, offsetFromRootBlock, lastLogicalTop, lastLogicalLeft,
                                      lastLogicalRight, paintInfo );
    }
    else
    {
        result = blockSelectionGaps( rootBlock, rootBlockPhysicalPosition, offsetFromRootBlock, lastLogicalTop, lastLogicalLeft,
                                     lastLogicalRight, paintInfo );
    }

    // Go ahead and fill the vertical gap all the way to the bottom of our block if the selection extends past our block.
    if ( rootBlock == this && ( selectionState() != SelectionBoth && selectionState() != SelectionEnd ) )
        result.uniteCenter( blockSelectionGap( rootBlock, rootBlockPhysicalPosition, offsetFromRootBlock, lastLogicalTop, lastLogicalLeft,
                                               lastLogicalRight,
...
GapRects RenderBlock::inlineSelectionGaps( RenderBlock *rootBlock, const IntPoint &rootBlockPhysicalPosition,
        const IntSize &offsetFromRootBlock,
        int &lastLogicalTop, int &lastLogicalLeft, int &lastLogicalRight, const PaintInfo *paintInfo )
{
    GapRects result;
    ...
    for ( ; curr && curr->hasSelectedChildren(); curr = curr->nextRootBox() )
    {
        ...
        logicalRect.move( isHorizontalWritingMode() ? offsetFromRootBlock : IntSize( offsetFromRootBlock.height(),
                          offsetFromRootBlock.width() ) );
        IntRect physicalRect = rootBlock->logicalRectToPhysicalRect( rootBlockPhysicalPosition, logicalRect );

        if ( !paintInfo || ( isHorizontalWritingMode() && physicalRect.y() < paintInfo->rect.maxY()
                             && physicalRect.maxY() > paintInfo->rect.y() )
                || ( !isHorizontalWritingMode() && physicalRect.x() < paintInfo->rect.maxX() && physicalRect.maxX() > paintInfo->rect.x() ) )
        {
            result.unite( curr->lineSelectionGap( rootBlock, rootBlockPhysicalPosition, offsetFromRootBlock, selTop, selHeight,
                                                  paintInfo ) );
        }
...
GapRects RenderBlock::blockSelectionGaps( RenderBlock *rootBlock, const IntPoint &rootBlockPhysicalPosition,
        const IntSize &offsetFromRootBlock,
        int &lastLogicalTop, int &lastLogicalLeft, int &lastLogicalRight, const PaintInfo *paintInfo )
{
    GapRects result;
    ...
    for ( bool sawSelectionEnd = false; curr && !sawSelectionEnd; curr = curr->nextSiblingBox() )
    {
        ...
        bool paintsOwnSelection = curr->shouldPaintSelectionGaps()
                                  || curr->isTable(); // FIXME: Eventually we won't special-case table like this.
        ...
        if ( fillBlockGaps )
        {
            ...
            if ( leftGap )
            {
                result.uniteLeft( logicalLeftSelectionGap( rootBlock, rootBlockPhysicalPosition, offsetFromRootBlock, this, curr->logicalLeft(),
                                  curr->logicalTop(), curr->logicalHeight(), paintInfo ) );
            }

            if ( rightGap )
            {
                result.uniteRight( logicalRightSelectionGap( rootBlock, rootBlockPhysicalPosition, offsetFromRootBlock, this,
                                   curr->logicalRight(), curr->logicalTop(), curr->logicalHeight(), paintInfo ) );
            }
            ...
            result.unite( toRenderBlock( curr )->selectionGaps( rootBlock, rootBlockPhysicalPosition,
                          IntSize( offsetFromRootBlock.width() + curr->x(), offsetFromRootBlock.height() + curr->y() ),
...
IntRect RenderBlock::blockSelectionGap( RenderBlock *rootBlock, const IntPoint &rootBlockPhysicalPosition,
                                        const IntSize &offsetFromRootBlock,
                                        int lastLogicalTop, int lastLogicalLeft, int lastLogicalRight, int logicalBottom, const PaintInfo *paintInfo )
{
    ...
    IntRect gapRect = rootBlock->logicalRectToPhysicalRect( rootBlockPhysicalPosition, IntRect( logicalLeft, logicalTop, logicalWidth,
                      logicalHeight ) );
...
IntRect RenderBlock::logicalLeftSelectionGap( RenderBlock *rootBlock, const IntPoint &rootBlockPhysicalPosition,
        const IntSize &offsetFromRootBlock,
        RenderObject *selObj, int logicalLeft, int logicalTop, int logicalHeight, const PaintInfo *paintInfo )
{
    ...
    int rootBlockLogicalLeft = max( logicalLeftSelectionOffset( rootBlock, logicalTop ), logicalLeftSelectionOffset( rootBlock,
                                    logicalTop + logicalHeight ) );
    int rootBlockLogicalRight = min( inlineDirectionOffset( rootBlock, offsetFromRootBlock ) + logicalLeft,
                                     min( logicalRightSelectionOffset( rootBlock, logicalTop ), logicalRightSelectionOffset( rootBlock,
                                             logicalTop + logicalHeight ) ) );
    ...
    IntRect gapRect = rootBlock->logicalRectToPhysicalRect( rootBlockPhysicalPosition, IntRect( rootBlockLogicalLeft,
                      rootBlockLogicalTop, rootBlockLogicalWidth, logicalHeight ) );
...
IntRect RenderBlock::logicalRightSelectionGap( RenderBlock *rootBlock, const IntPoint &rootBlockPhysicalPosition,
        const IntSize &offsetFromRootBlock,
        RenderObject *selObj, int logicalRight, int logicalTop, int logicalHeight, const PaintInfo *paintInfo )
{
    int rootBlockLogicalTop = blockDirectionOffset( rootBlock, offsetFromRootBlock ) + logicalTop;
    int rootBlockLogicalLeft = max( inlineDirectionOffset( rootBlock, offsetFromRootBlock ) + logicalRight,
                                    max( logicalLeftSelectionOffset( rootBlock, logicalTop ), logicalLeftSelectionOffset( rootBlock, logicalTop + logicalHeight ) ) );
    int rootBlockLogicalRight = min( logicalRightSelectionOffset( rootBlock, logicalTop ), logicalRightSelectionOffset( rootBlock,
                                     logicalTop + logicalHeight ) );
    int rootBlockLogicalWidth = rootBlockLogicalRight - rootBlockLogicalLeft;
...
RenderBlock::FloatingObject *RenderBlock::insertFloatingObject( RenderBox *o )
{
    ...
    FloatingObject *newObj = new FloatingObject( o->style()->floating() == FLEFT ? FloatingObject::FloatLeft :
    ...
    newObj->m_shouldPaint =
        !o->hasSelfPaintingLayer(); // If a layer exists, the float will paint itself.  Otherwise someone else will.
...
bool RenderBlock::positionNewFloats()
{
    ...
    for ( ; it != end; ++it )
    {
        ...
        if ( childBox->style()->floating() == FLEFT )
        {
            ...
            while ( logicalRightOffsetForLine( logicalTop, rightOffset, false,
                                               &heightRemainingRight ) - floatLogicalLeft < floatLogicalWidth )
            {
        ...
        setLogicalHeightForFloat( floatingObject,
                                  logicalHeightForChild( childBox ) + marginBeforeForChild( childBox ) + marginAfterForChild( childBox ) );
...
void RenderBlock::markLinesDirtyInBlockRange( int logicalTop, int logicalBottom, RootInlineBox *highest )
{
    ...
    while ( afterLowest && afterLowest != highest && ( afterLowest->blockLogicalHeight() >= logicalTop
            || afterLowest->blockLogicalHeight() < 0 ) )
    {
...
void RenderBlock::clearFloats()
{
    ...
    while ( prev && ( prev->isFloatingOrPositioned() || !prev->isBox() || !prev->isRenderBlock()
                      || toRenderBlock( prev )->avoidsFloats() ) )
    {
        if ( prev->isFloating() )
        {
        ...
        if ( m_floatingObjects )
        {
            ...
            for ( FloatingObjectSetIterator it = floatingObjectSet.begin(); it != end; ++it )
            {
                ...
                if ( oldFloatingObject )
                {
                    ...
                    if ( logicalWidthForFloat( f ) != logicalWidthForFloat( oldFloatingObject )
                            || logicalLeftForFloat( f ) != logicalLeftForFloat( oldFloatingObject ) )
                    {
                        changeLogicalTop = 0;
...
int RenderBlock::addOverhangingFloats( RenderBlock *child, int logicalLeftOffset, int logicalTopOffset,
                                       bool makeChildPaintOtherFloats )
{
    // Prevent floats from being added to the canvas by the root element, e.g., <html>.
    if ( child->hasOverflowClip() || !child->containsFloats() || child->isRoot() || child->hasColumns()
            || child->isWritingModeRoot() )
    {
        return 0;
    }
    ...
    for ( FloatingObjectSetIterator childIt = child->m_floatingObjects->set().begin(); childIt != childEnd; ++childIt )
    {
        ...
        if ( logicalBottom > logicalHeight() )
        {
            ...
            if ( !containsFloat( r->m_renderer ) )
            {
                ...
                FloatingObject *floatingObj = new FloatingObject( r->type(), IntRect( r->x() - leftOffset, r->y() - topOffset, r->width(),
                        r->height() ) );
        ...
        else
        {
            ...
            if ( r->m_isDescendant )
            {
                child->addOverflowFromChild( r->m_renderer, IntSize( xPositionForFloatIncludingMargin( r ),
                                             yPositionForFloatIncludingMargin( r ) ) );
            }
        }
    }
...
void RenderBlock::markAllDescendantsWithFloatsForLayout( RenderBox *floatToRemove, bool inLayout )
{
    ...
    if ( !childrenInline() )
    {
        for ( RenderObject *child = firstChild(); child; child = child->nextSibling() )
        {
            ...
            if ( ( floatToRemove ? childBlock->containsFloat( floatToRemove ) : childBlock->containsFloats() )
                    || childBlock->shrinkToAvoidFloats() )
            {
...
bool RenderBlock::nodeAtPoint( const HitTestRequest &request, HitTestResult &result, int _x, int _y, int _tx, int _ty,
                               HitTestAction hitTestAction )
{
    ...
    if ( ( hitTestAction == HitTestBlockBackground || hitTestAction == HitTestChildBlockBackground )
            && isPointInOverflowControl( result, _x, _y, tx, ty ) )
    {
    ...
    bool checkChildren = !useClip
                         || ( hasControlClip() ? controlClipRect( tx, ty ).intersects( hitTestArea ) : overflowClipRect( tx, ty,
                                 IncludeOverlayScrollbarSize ).intersects( hitTestArea ) );
...
bool RenderBlock::hitTestColumns( const HitTestRequest &request, HitTestResult &result, int x, int y, int tx, int ty,
                                  HitTestAction hitTestAction )
{
    ...
    for ( i = colCount - 1; i >= 0; i-- )
    {
        ...
        if ( colRect.intersects( result.rectForPoint( x, y ) ) )
        {
            ...
            {
                return hitTestContents( request, result, x, y, finalX, finalY, hitTestAction ) || ( hitTestAction == HitTestFloat
                        && hitTestFloats( request, result, x, y, finalX, finalY ) );
            }
        }
    }
...
bool RenderBlock::hitTestContents( const HitTestRequest &request, HitTestResult &result, int x, int y, int tx, int ty,
                                   HitTestAction hitTestAction )
{
    ...
    else
    {
        ...
        for ( RenderBox *child = lastChildBox(); child; child = child->previousSiblingBox() )
        {
            ...
            if ( !child->hasSelfPaintingLayer() && !child->isFloating()
                    && child->nodeAtPoint( request, result, x, y, childPoint.x(), childPoint.y(), childHitTest ) )
            {
                return true;
            }
        }
...
static VisiblePosition positionForPointRespectingEditingBoundaries( RenderBlock *parent, RenderBox *child,
        const IntPoint &pointInParentCoordinates )
{
...
bool RenderBlock::layoutColumns( bool hasSpecifiedPageLogicalHeight, int pageLogicalHeight, LayoutStateMaintainer &statePusher )
{
        ...
        if ( minColumnCount >= desiredColumnCount )
        {
            ...
            if ( !pageLogicalHeight )
            {
                int distanceBetweenBreaks = max( colInfo->maximumDistanceBetweenForcedBreaks(),
                                                 view()->layoutState()->pageLogicalOffset( borderBefore() + paddingBefore() + contentLogicalHeight() ) -
    ...
    if ( columnCount( colInfo ) )
    {
        setLogicalHeight( borderBefore() + paddingBefore() + colInfo->columnHeight() + borderAfter() + paddingAfter() +
...
IntPoint RenderBlock::flipForWritingModeIncludingColumns( const IntPoint &point ) const
{
    ...
    int expandedLogicalHeight = borderBefore() + paddingBefore() + columnCount( colInfo ) * columnLogicalHeight + borderAfter() +
...
void RenderBlock::adjustForColumns( IntSize &offset, const IntPoint &point ) const
{
    ...
    for ( size_t i = 0; i < colCount; ++i )
    {
        ...
        IntRect sliceRect = IntRect( logicalLeft, borderBefore() + paddingBefore() + i * colLogicalHeight, colLogicalWidth,
                                     colLogicalHeight );
...
void RenderBlock::computePreferredLogicalWidths()
{
    ...
    {
        m_maxPreferredLogicalWidth = max( m_maxPreferredLogicalWidth,
                                          computeContentBoxLogicalWidth( style()->logicalMinWidth().value() ) );
    ...
    if ( style()->logicalMaxWidth().isFixed() && style()->logicalMaxWidth().value() != undefinedLength )
    {
        m_maxPreferredLogicalWidth = min( m_maxPreferredLogicalWidth,
                                          computeContentBoxLogicalWidth( style()->logicalMaxWidth().value() ) );
...
int RenderBlock::baselinePosition( FontBaseline baselineType, bool firstLine, LineDirectionMode direction,
                                   LinePositionMode linePositionMode ) const
{
    ...
    if ( isReplaced() && linePositionMode == PositionOnContainingLine )
    {
        ...
        // of our content box.
        bool ignoreBaseline = ( layer() && ( layer()->marquee() || ( direction == HorizontalLine ? ( layer()->verticalScrollbar()
                                             || layer()->scrollYOffset() != 0 )
...
int RenderBlock::lastLineBoxBaseline() const
{
    ...
    if ( childrenInline() )
    {
        ...
        if ( lastLineBox() )
        {
            return lastLineBox()->logicalTop() + style( lastLineBox() == firstLineBox() )->fontMetrics().ascent(
...
void RenderBlock::updateFirstLetter()
{
    ...
    while ( currChild && ( ( !currChild->isReplaced() && !currChild->isRenderButton() && !currChild->isMenuList() )
    ...
    if ( oldText && oldText->length() > 0 )
    {
        ...
        RenderTextFragment *remainingText =
            new ( renderArena() ) RenderTextFragment( textObj->node() ? textObj->node() : textObj->document(), oldText.get(), length,
        ...
        RenderTextFragment *letter =
            new ( renderArena() ) RenderTextFragment( remainingText->node() ? remainingText->node() : remainingText->document(),
                    oldText.get(), 0, length );
...
void RenderBlock::setMaxMarginBeforeValues( int pos, int neg )
{
    if ( !m_rareData )
    {
        if ( pos == RenderBlockRareData::positiveMarginBeforeDefault( this )
                && neg == RenderBlockRareData::negativeMarginBeforeDefault( this ) )
        {
            return;
        }
...
void RenderBlock::setMaxMarginAfterValues( int pos, int neg )
{
    if ( !m_rareData )
    {
        if ( pos == RenderBlockRareData::positiveMarginAfterDefault( this )
                && neg == RenderBlockRareData::negativeMarginAfterDefault( this ) )
        {
            return;
        }
...
int RenderBlock::applyBeforeBreak( RenderBox *child, int logicalOffset )
{
    ...
    bool checkPageBreaks = !checkColumnBreaks
                           && view()->layoutState()->m_pageLogicalHeight; // FIXME: Once columns can print we have to check this.
    bool checkBeforeAlways = ( checkColumnBreaks && child->style()->columnBreakBefore() == PBALWAYS ) || ( checkPageBreaks
...
int RenderBlock::applyAfterBreak( RenderBox *child, int logicalOffset, MarginInfo &marginInfo )
{
    ...
    bool checkPageBreaks = !checkColumnBreaks
                           && view()->layoutState()->m_pageLogicalHeight; // FIXME: Once columns can print we have to check this.
    bool checkAfterAlways = ( checkColumnBreaks && child->style()->columnBreakAfter() == PBALWAYS ) || ( checkPageBreaks
                            && child->style()->pageBreakAfter() == PBALWAYS );
...
int RenderBlock::adjustForUnsplittableChild( RenderBox *child, int logicalOffset, bool includeMargins )
{
    ...
    int childLogicalHeight = logicalHeightForChild( child ) + ( includeMargins ? marginBeforeForChild( child ) + marginAfterForChild(
                                 child ) : 0 );
    LayoutState *layoutState = view()->layoutState();
...
void RenderBlock::showLineTreeAndMark( const InlineBox *markedBox1, const char *markedLabel1, const InlineBox *markedBox2,
                                       const char *markedLabel2, const RenderObject *obj ) const
{
    showRenderObject();
