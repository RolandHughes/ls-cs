...
static bool findPlaceForCounter( RenderObject *counterOwner, const AtomicString &identifier, bool isReset, CounterNode *&parent,
...
static CounterNode *makeCounterNode( RenderObject *object, const AtomicString &identifier, bool alwaysCreateCounter )
{
    ...
    for ( RenderObject *currentRenderer = nextInPreOrder( object, stayWithin ); currentRenderer;
            currentRenderer = nextInPreOrder( currentRenderer, stayWithin, skipDescendants ) )
    {
...
void showCounterRendererTree( const WebCore::RenderObject *renderer, const char *counterName )
{
    ...
    for ( const WebCore::RenderObject *current = root; current; current = current->nextInPreOrder() )
    {
        ...
        fprintf( stderr, "%p N:%p P:%p PS:%p NS:%p C:%p\n",
                 ...
                 counterName ? WebCore::counterMaps().get( current )->get( identifier.impl() ).get() : ( WebCore::CounterNode * )1 :
                 ( WebCore::CounterNode * )0 );
    }
