...
static Color disabledTextColor( const Color &textColor, const Color &backgroundColor )
{
    ...
    if ( textColor.rgb() == Color::black
            || differenceSquared( textColor, Color::white ) > differenceSquared( backgroundColor, Color::white ) )
    {
...
void RenderTextControl::adjustInnerTextStyle( const RenderStyle *startStyle, RenderStyle *textBlockStyle ) const
{
    ...
    if ( disabled )
    {
        textBlockStyle->setColor( disabledTextColor( textBlockStyle->visitedDependentColor( CSSPropertyColor ),
                                  startStyle->visitedDependentColor( CSSPropertyBackgroundColor ) ) );
    }
}
...
void setSelectionRange( Node *node, int start, int end )
{
    ...
    {
        ASSERT( startPosition.deepEquivalent().deprecatedNode()->shadowAncestorNode() == node
                && endPosition.deepEquivalent().deprecatedNode()->shadowAncestorNode() == node );
    }
...
void RenderTextControl::computeLogicalHeight()
{
    ...
    if ( style()->overflowX() == OSCROLL ||  ( style()->overflowX() == OAUTO
            && m_innerText->renderer()->style()->wordWrap() == NormalWordWrap ) )
    {
...
void RenderTextControl::computePreferredLogicalWidths()
{
    ...
    else
    {
        ...
        m_maxPreferredLogicalWidth = preferredContentWidth( getAvgCharWidth( family ) ) + m_innerText->renderBox()->paddingLeft() +
                                     m_innerText->renderBox()->paddingRight();
    }
...
void RenderTextControl::paintPlaceholder( PaintInfo &paintInfo, int tx, int ty )
{
    ...
    IntRect clipRect( tx + borderLeft(), ty + borderTop(), width() - borderLeft() - borderRight(),
    ...
    TextRun textRun( placeholderText.characters(), placeholderText.length(), false, 0, 0, TextRun::AllowTrailingExpansion,
                     placeholderStyle->direction(), placeholderStyle->unicodeBidi() == Override );
