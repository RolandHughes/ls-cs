...
public:
    ...
    IntRect contentBoxRect() const
    {
        return IntRect( borderLeft() + paddingLeft(), borderTop() + paddingTop(), contentWidth(), contentHeight() );
    }
    ...
    int logicalLeftLayoutOverflow() const
    {
        return style()->isHorizontalWritingMode() ? minXLayoutOverflow() : minYLayoutOverflow();
    }
    int logicalRightLayoutOverflow() const
    {
        return style()->isHorizontalWritingMode() ? maxXLayoutOverflow() : maxYLayoutOverflow();
    }
    ...
    int logicalLeftVisualOverflow() const
    {
        return style()->isHorizontalWritingMode() ? minXVisualOverflow() : minYVisualOverflow();
    }
    int logicalRightVisualOverflow() const
    {
        return style()->isHorizontalWritingMode() ? maxXVisualOverflow() : maxYVisualOverflow();
    }
    ...
    {
        addOverflowFromChild( child, IntSize( child->x(), child->y() ) );
    }
    ...
    int clientLogicalWidth() const
    {
        return style()->isHorizontalWritingMode() ? clientWidth() : clientHeight();
    }
    int clientLogicalHeight() const
    {
        return style()->isHorizontalWritingMode() ? clientHeight() : clientWidth();
    }
    int clientLogicalBottom() const
    {
        return borderBefore() + clientLogicalHeight();
    }
    IntRect clientBoxRect() const
    {
        return IntRect( clientLeft(), clientTop(), clientWidth(), clientHeight() );
    }
    ...
    void setInlineBoxWrapper( InlineBox *boxWrapper )
    {
    ...
    {
        return style()->isHorizontalWritingMode() ? availableLogicalWidth() : availableLogicalHeight();
    }
    int availableHeight() const
    {
        return style()->isHorizontalWritingMode() ? availableLogicalHeight() : availableLogicalWidth();
    }
    ...
    {
        return hasOverflowClip() && ( style()->overflowY() == OAUTO || style()->overflowY() == OOVERLAY );
    }
    bool hasAutoHorizontalScrollbar() const
    {
        return hasOverflowClip() && ( style()->overflowX() == OAUTO || style()->overflowX() == OOVERLAY );
    }
    ...
    {
        return hasOverflowClip() && ( style()->overflowY() == OSCROLL || hasAutoVerticalScrollbar() );
    }
    ...
    {
        return !isInline() && !isFloatingOrPositioned() && parent() && parent()->isFlexibleBox();
    }
    ...
    const; // The offset is in the block direction (y for horizontal writing modes, x for vertical writing modes).
    ...
    RenderOverflow *hasRenderOverflow() const
    {
...
protected:
    ...
    void paintFillLayer( const PaintInfo &, const Color &, const FillLayer *, int tx, int ty, int width, int height,
                         BackgroundBleedAvoidance, CompositeOperator, RenderObject *backgroundObject );
    void paintFillLayers( const PaintInfo &, const Color &, const FillLayer *, int tx, int ty, int width, int height,
                          BackgroundBleedAvoidance = BackgroundBleedNone, CompositeOperator = CompositeSourceOver, RenderObject *backgroundObject = 0 );
    ...
    {
        return isReplaced() && !isInlineBlockOrInlineTable();
    }

    virtual void mapLocalToContainer( RenderBoxModelObject *repaintContainer, bool fixed, bool useTransforms,
...
private:
    ...
    int containingBlockLogicalWidthForPositioned( const RenderBoxModelObject *containingBlock,
            bool checkForPerpendicularWritingMode = true ) const;
    int containingBlockLogicalHeightForPositioned( const RenderBoxModelObject *containingBlock,
            bool checkForPerpendicularWritingMode = true ) const;

    void computePositionedLogicalHeight();
    void computePositionedLogicalWidthUsing( Length logicalWidth, const RenderBoxModelObject *containerBlock,
