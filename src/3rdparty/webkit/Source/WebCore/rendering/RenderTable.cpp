...
void RenderTable::addChild( RenderObject *child, RenderObject *beforeChild )
{
    ...
    while ( lastBox && lastBox->parent()->isAnonymous() && !lastBox->isTableSection() && lastBox->style()->display() != TABLE_CAPTION
            && lastBox->style()->display() != TABLE_COLUMN_GROUP )
    {
...
void RenderTable::computeLogicalWidth()
{
    ...
    int containerWidthInInlineDirection = hasPerpendicularContainingBlock ? perpendicularContainingBlockLogicalHeight() :
...
//     if ( oldWidth != width() || columns.size() + 1 != columnPos.size() )
    ...
    for ( RenderObject *child = firstChild(); child; child = child->nextSibling() )
    {
        ...
        {
            toRenderTableSection( child )->layoutRows( child == m_firstBody ? max( 0,
                    computedLogicalHeight - totalSectionLogicalHeight ) : 0 );
        }
    }
    ...
    while ( section )
    {
        ...
        {
            sectionMoved = true;
            movedSectionLogicalTop = min( logicalHeight(),
                                          section->logicalTop() ) + ( style()->isHorizontalWritingMode() ? section->minYVisualOverflow() : section->minXVisualOverflow() );
        }
    ...
    if ( !didFullRepaint && sectionMoved )
    {
        ...
        {
            repaintRectangle( IntRect( minXVisualOverflow(), movedSectionLogicalTop, maxXVisualOverflow() - minXVisualOverflow(),
                                       maxYVisualOverflow() - movedSectionLogicalTop ) );
        }
        else
        {
            repaintRectangle( IntRect( movedSectionLogicalTop, minYVisualOverflow(), maxXVisualOverflow() - movedSectionLogicalTop,
                                       maxYVisualOverflow() - minYVisualOverflow() ) );
        }
    }
...
void RenderTable::addOverflowFromChildren()
{
    ...
    if ( collapseBorders() )
    {
        ...
        IntRect borderOverflowRect( leftBorderOverflow, topBorderOverflow, rightBorderOverflow - leftBorderOverflow,
...
void RenderTable::paintObject( PaintInfo &paintInfo, int tx, int ty )
{
    ...
    if ( ( paintPhase == PaintPhaseBlockBackground || paintPhase == PaintPhaseChildBlockBackground ) && hasBoxDecorations()
...
void RenderTable::paintBoxDecorations( PaintInfo &paintInfo, int tx, int ty )
{
    ...
    {
        paintFillLayers( paintInfo, style()->visitedDependentColor( CSSPropertyBackgroundColor ), style()->backgroundLayers(), rect.x(),
...
RenderTableSection *RenderTable::sectionAbove( const RenderTableSection *section, bool skipEmptySections ) const
{
    ...
    while ( prevSection )
    {
        if ( prevSection->isTableSection() && prevSection != m_head && prevSection != m_foot && ( !skipEmptySections
                || toRenderTableSection( prevSection )->numRows() ) )
        {
            break;
        }
...
RenderTableSection *RenderTable::sectionBelow( const RenderTableSection *section, bool skipEmptySections ) const
{
    ...
    while ( nextSection )
    {
        if ( nextSection->isTableSection() && nextSection != m_head && nextSection != m_foot && ( !skipEmptySections
                || toRenderTableSection( nextSection )->numRows() ) )
        {
            break;
        }
...
bool RenderTable::nodeAtPoint( const HitTestRequest &request, HitTestResult &result, int xPos, int yPos, int tx, int ty,
                               HitTestAction action )
{
    ...
    if ( visibleToHitTesting() && ( action == HitTestBlockBackground || action == HitTestChildBlockBackground )
            && boundsRect.intersects( result.rectForPoint( xPos, yPos ) ) )
    {
