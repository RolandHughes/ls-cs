...
void RootInlineBox::placeEllipsis( const AtomicString &ellipsisStr,  bool ltr, float blockLeftEdge, float blockRightEdge,
                                   float ellipsisWidth,
                                   InlineBox *markupBox )
{
...
void RootInlineBox::addHighlightOverflow()
{
    ...
    IntRect inflatedRect = enclosingIntRect( page->chrome()->client()->customHighlightRect( renderer()->node(),
                           renderer()->style()->highlight(), rootRect ) );
...
bool RootInlineBox::nodeAtPoint( const HitTestRequest &request, HitTestResult &result, int x, int y, int tx, int ty, int lineTop,
                                 int lineBottom )
{
        ...
        VerticalPositionCache &verticalPositionCache )
{
    ...
    computeLogicalBoxHeights( this, maxPositionTop, maxPositionBottom, maxAscent, maxDescent, setMaxAscent, setMaxDescent,
                              noQuirksMode,
...
GapRects RootInlineBox::lineSelectionGap( RenderBlock *rootBlock, const IntPoint &rootBlockPhysicalPosition,
    ...
    if ( firstBox && firstBox != lastBox )
    {
        ...
        for ( InlineBox *box = firstBox->nextLeafChild(); box; box = box->nextLeafChild() )
        {
            if ( box->selectionState() != RenderObject::SelectionNone )
            {
                ...
                logicalRect.move( renderer()->isHorizontalWritingMode() ? offsetFromRootBlock : IntSize( offsetFromRootBlock.height(),
                ...
                if ( isPreviousBoxSelected && gapRect.width() > 0 && gapRect.height() > 0 )
                {
                    if ( paintInfo && box->parent()->renderer()->style()->visibility() == VISIBLE )
                    {
                        paintInfo->context->fillRect( gapRect, box->parent()->renderer()->selectionBackgroundColor(),
                                                      box->parent()->renderer()->style()->colorSpace() );
                    }
...
int RootInlineBox::selectionTop() const
{
    ...
    if ( m_hasAnnotationsBefore )
    {
        selectionTop -= !renderer()->style()->isFlippedLinesWritingMode() ? computeOverAnnotationAdjustment(
                            m_lineTop ) : computeUnderAnnotationAdjustment( m_lineTop );
    }
...
InlineBox *RootInlineBox::closestLeafChildForLogicalLeftPosition( int leftPosition, bool onlyEditableLeaves )
{
    ...
    if ( leftPosition <= firstLeaf->logicalLeft() && !firstLeaf->renderer()->isListMarker() && ( !onlyEditableLeaves
            || isEditableLeaf( firstLeaf ) ) )
    ...
    if ( leftPosition >= lastLeaf->logicalRight() && !lastLeaf->renderer()->isListMarker() && ( !onlyEditableLeaves
            || isEditableLeaf( lastLeaf ) ) )
...
void RootInlineBox::ascentAndDescentForBox( InlineBox *box, GlyphOverflowAndFallbackFontsMap &textBoxDataMap, int &ascent,
        int &descent,
        bool &affectsAscent, bool &affectsDescent ) const
{
    ...
    if ( usedFonts && !usedFonts->isEmpty() && ( includeFont || ( box->renderer()->style( m_firstLine )->lineHeight().isNegative()
            && includeLeading ) ) )
    {
    ...
    if ( includeGlyphsForBox( box ) && glyphOverflow && glyphOverflow->computeBounds )
    {
        ...
        glyphOverflow->top = min( glyphOverflow->top, max( 0,
                                  glyphOverflow->top - box->renderer()->style( m_firstLine )->fontMetrics().ascent() ) );
        glyphOverflow->bottom = min( glyphOverflow->bottom, max( 0,
                                     glyphOverflow->bottom - box->renderer()->style( m_firstLine )->fontMetrics().descent() ) );
    }
    ...
    if ( includeMarginForBox( box ) )
    {
        ...
        {
            ascentWithMargin += box->boxModelObject()->borderBefore() + box->boxModelObject()->paddingBefore() +
                                box->boxModelObject()->marginBefore();
            descentWithMargin += box->boxModelObject()->borderAfter() + box->boxModelObject()->paddingAfter() +
...
int RootInlineBox::verticalPositionForBox( InlineBox *box, VerticalPositionCache &verticalPositionCache )
{
    ...
    if ( verticalAlign != BASELINE )
    {
        ...
        {
            verticalPosition += renderer->baselinePosition( baselineType(), firstLine,
                                lineDirection ) - fontMetrics.ascent( baselineType() );
        }
        else if ( verticalAlign == MIDDLE )
        {
            verticalPosition += -static_cast<int>( fontMetrics.xHeight() / 2 ) - renderer->lineHeight( firstLine,
                                lineDirection ) / 2 + renderer->baselinePosition( baselineType(), firstLine, lineDirection );
        }
        else if ( verticalAlign == TEXT_BOTTOM )
        {
            ...
            {
                verticalPosition -= ( renderer->lineHeight( firstLine, lineDirection ) - renderer->baselinePosition( baselineType(), firstLine,
                                      lineDirection ) );
            }
        }
        else if ( verticalAlign == BASELINE_MIDDLE )
        {
            verticalPosition += -renderer->lineHeight( firstLine, lineDirection ) / 2 + renderer->baselinePosition( baselineType(), firstLine,
                                lineDirection );
        }
...
{
    LineBoxContain lineBoxContain = renderer()->style()->lineBoxContain();
    return ( lineBoxContain & LineBoxContainBlock ) || ( lineBoxContain & LineBoxContainInline )
