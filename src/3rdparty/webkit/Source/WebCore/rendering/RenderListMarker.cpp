...
{
    if ( style() && ( newStyle->listStylePosition() != style()->listStylePosition()
...
IntRect RenderListMarker::localSelectionRect()
{
    ...
    int newLogicalTop = root->block()->style()->isFlippedBlocksWritingMode() ? m_inlineBoxWrapper->logicalBottom() -
                        root->selectionBottom() : root->selectionTop() - m_inlineBoxWrapper->logicalTop();
...
void RenderListMarker::imageChanged( WrappedImagePtr o, const IntRect * )
{
    ...
    if ( width() != m_image->imageSize( this, style()->effectiveZoom() ).width()
            || height() != m_image->imageSize( this, style()->effectiveZoom() ).height() || m_image->errorOccurred() )
    {
        setNeedsLayoutAndPrefWidthsRecalc();
    }
    else
...
int RenderListMarker::baselinePosition( FontBaseline baselineType, bool firstLine, LineDirectionMode direction,
...
IntRect RenderListMarker::getRelativeMarkerRect()
{
    if ( isImage() )
    {
        return IntRect( 0, 0, m_image->imageSize( this, style()->effectiveZoom() ).width(), m_image->imageSize( this,
                        style()->effectiveZoom() ).height() );
    }
