...
void RenderInline::styleDidChange( StyleDifference diff, const RenderStyle *oldStyle )
{
    ...
    {
        bool alwaysCreateLineBoxes = hasSelfPaintingLayer() || hasBoxDecorations() || style()->hasPadding() || style()->hasMargin()
                                     || style()->hasOutline();
    ...
    for ( RenderObject *curr = firstChild(); curr; curr = curr->nextSibling() )
    {
        ...
        if ( curr->isBox() )
        {
            ...
            if ( currBox->inlineBoxWrapper() )
            {
                ...
                int logicalTop = rootBox->logicalTop() + ( rootBox->renderer()->style(
                                     rootBox->isFirstLineStyle() )->font().fontMetrics().ascent() - container->style(
                ...
                if ( isHorizontal )
                {
                    result = FloatRect( offset.width() + currBox->inlineBoxWrapper()->x() - currBox->marginLeft(), offset.height() + logicalTop,
                                        currBox->width() + currBox->marginLeft() + currBox->marginRight(), logicalHeight );
                }
                else
                {
                    result = FloatRect( offset.width() + logicalTop, offset.height() + currBox->inlineBoxWrapper()->y() - currBox->marginTop(),
                                        logicalHeight, currBox->height() + currBox->marginTop() + currBox->marginBottom() );
                }
    ...
    for ( RenderObject *curr = firstChild(); curr; curr = curr->nextSibling() )
    {
        ...
        if ( curr->isBox() )
        {
            ...
            if ( currBox->inlineBoxWrapper() )
            {
                ...
                if ( isHorizontal )
                {
                    result = FloatRect( currBox->inlineBoxWrapper()->x() - currBox->marginLeft(), logicalTop,
                                        currBox->width() + currBox->marginLeft() + currBox->marginRight(), logicalHeight );
                }
                else
                {
                    result = FloatRect( logicalTop, currBox->inlineBoxWrapper()->y() - currBox->marginTop(), logicalHeight,
                                        currBox->height() + currBox->marginTop() + currBox->marginBottom() );
                }
    ...
    for ( RenderObject *curr = firstChild(); curr; curr = curr->nextSibling() )
    {
        ...
        if ( curr->isBox() )
        {
            ...
            if ( currBox->inlineBoxWrapper() )
            {
                ...
                if ( isHorizontal )
                {
                    result.uniteIfNonZero( FloatRect( currBox->inlineBoxWrapper()->x() - currBox->marginLeft(), logicalTop,
                                                      currBox->width() + currBox->marginLeft() + currBox->marginRight(), logicalHeight ) );
                }
                else
                {
                    result.uniteIfNonZero( FloatRect( logicalTop, currBox->inlineBoxWrapper()->y() - currBox->marginTop(), logicalHeight,
                                                      currBox->height() + currBox->marginTop() + currBox->marginBottom() ) );
                }
            }
        }
...
void RenderInline::mapLocalToContainer( RenderBoxModelObject *repaintContainer, bool fixed, bool useTransforms,
...
int RenderInline::baselinePosition( FontBaseline baselineType, bool firstLine, LineDirectionMode direction,
