...
public:
    ...
    {
        return IntSize( scrollXOffset() + m_scrollLeftOverflow, scrollYOffset() + m_scrollTopOverflow );
    }

    int scrollXOffset() const
    {
        return m_scrollX + m_scrollOrigin.x();
    }
    int scrollYOffset() const
    {
        return m_scrollY + m_scrollOrigin.y();
    }

    void scrollToOffset( int x, int y );
    void scrollToXOffset( int x )
    {
        scrollToOffset( x, m_scrollY + m_scrollOrigin.y() );
    }
    void scrollToYOffset( int y )
    {
        scrollToOffset( m_scrollX + m_scrollOrigin.x(), y );
    }
    void scrollRectToVisible( const IntRect &, bool scrollToAnchor = false,
    ...
    IntRect getRectToExpose( const IntRect &visibleRect, const IntRect &exposeRect, const ScrollAlignment &alignX,
    ...
    void updateLayerPositions( UpdateLayerPositionsFlags = CheckForRepaint | IsCompositingUpdateRoot | UpdateCompositingLayers,
                               IntPoint *cachedOffset = 0 );
    ...
    {
        return !hasAutoZIndex() || renderer()->isRenderView();
    }
    ...
    {
        return enclosingCompositingLayer( false );
    }
#endif
    ...
    void calculateClipRects( const RenderLayer *rootLayer, ClipRects &, bool useCached = false,
                             OverlayScrollbarSizeRelevancy = IgnoreOverlayScrollbarSize ) const;
    ...
    {
        m_needsFullRepaint = f;
    }

    int staticInlinePosition() const
    {
        return m_staticInlinePosition;
    }
    int staticBlockPosition() const
    {
        return m_staticBlockPosition;
    }
    void setStaticInlinePosition( int position )
    {
        m_staticInlinePosition = position;
    }
    void setStaticBlockPosition( int position )
    {
    ...
    {
        return renderer()->style()->transformStyle3D() == TransformStyle3DPreserve3D;
    }
...
private:
    ...
    {
        return renderer()->isBox() ? toRenderBox( renderer() )->x() : 0;
    }
    int renderBoxY() const
    {
        return renderer()->isBox() ? toRenderBox( renderer() )->y() : 0;
    }
    ...
    RenderLayer *hitTestLayer( RenderLayer *rootLayer, RenderLayer *containerLayer, const HitTestRequest &request,
    ...
    RenderLayer *hitTestPaginatedChildLayer( RenderLayer *childLayer, RenderLayer *rootLayer, const HitTestRequest &request,
            HitTestResult &result,
            const IntRect &hitTestRect, const IntPoint &hitTestPoint,
            const HitTestingTransformState *transformState, double *zOffset );
    RenderLayer *hitTestChildLayerColumns( RenderLayer *childLayer, RenderLayer *rootLayer, const HitTestRequest &request,
                                           HitTestResult &result,
                                           const IntRect &hitTestRect, const IntPoint &hitTestPoint,
                                           const HitTestingTransformState *transformState, double *zOffset,
    ...
    bool hitTestContents( const HitTestRequest &, HitTestResult &, const IntRect &layerBounds, const IntPoint &hitTestPoint,
                          HitTestFilter ) const;
    ...
    {
        m_has3DTransformedDescendant = b;
    }
    ...
    {
        return m_paintingInsideReflection;
    }
    void setPaintingInsideReflection( bool b )
    {
        m_paintingInsideReflection = b;
    }

    void parentClipRects( const RenderLayer *rootLayer, ClipRects &, bool temporaryClipRects = false,
                          OverlayScrollbarSizeRelevancy = IgnoreOverlayScrollbarSize ) const;
    IntRect backgroundClipRect( const RenderLayer *rootLayer, bool temporaryClipRects,
                                OverlayScrollbarSizeRelevancy = IgnoreOverlayScrollbarSize ) const;
    ...
    void setHasCompositingDescendant( bool b )
    {
        m_hasCompositingDescendant = b;
    }

    bool mustOverlapCompositedLayers() const
    {
        return m_mustOverlapCompositedLayers;
    }
    void setMustOverlapCompositedLayers( bool b )
    {
        m_mustOverlapCompositedLayers = b;
    }
#endif
