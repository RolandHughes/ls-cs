...
bool SVGTextLayoutEngine::parentDefinesTextLength( RenderObject *parent ) const
{
    ...
    while ( currentParent )
    {
        ...
        {
            SVGTextContentElement::SVGLengthAdjustType lengthAdjust = static_cast<SVGTextContentElement::SVGLengthAdjustType>
                    ( textContentElement->lengthAdjust() );
    ...
    for ( unsigned boxPosition = 0; boxPosition < boxCount; ++boxPosition )
    {
        ...
        for ( unsigned i = 0; i < fragmentCount; ++i )
        {
            ...
            fprintf( stderr, "    -> Fragment %i, x=%lf, y=%lf, width=%lf, height=%lf, characterOffset=%i, length=%i, characters='%s'\n"
                     , i, fragment.x, fragment.y, fragment.width, fragment.height, fragment.characterOffset, fragment.length,
                     fragmentString.utf8().data() );
        }
    }
}
#endif
...
bool SVGTextLayoutEngine::currentLogicalCharacterMetrics( SVGTextLayoutAttributes &logicalAttributes,
...
bool SVGTextLayoutEngine::currentVisualCharacterMetrics( SVGInlineTextBox *textBox, RenderSVGInlineText *text,
...
void SVGTextLayoutEngine::layoutTextOnLineOrPath( SVGInlineTextBox *textBox, RenderSVGInlineText *text, const RenderStyle *style )
{
    ...
    while ( true )
    {
        ...
        float glyphAdvance = baselineLayout.calculateGlyphAdvanceAndOrientation( m_isVerticalText, visualMetrics, orientationAngle,
