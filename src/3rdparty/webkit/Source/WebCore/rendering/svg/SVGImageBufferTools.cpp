...
void SVGImageBufferTools::calculateTransformationToOutermostSVGCoordinateSystem( const RenderObject *renderer,
...
bool SVGImageBufferTools::createImageBuffer( const FloatRect &absoluteTargetRect, const FloatRect &clampedAbsoluteTargetRect,
        OwnPtr<ImageBuffer> &imageBuffer, ColorSpace colorSpace )
{
    ...
    imageContext->scale( FloatSize( unclampedImageSize.width() / absoluteTargetRect.width(),
                                    unclampedImageSize.height() / absoluteTargetRect.height() ) );

    imageBuffer = image.release();
    return true;
}

void SVGImageBufferTools::renderSubtreeToImageBuffer( ImageBuffer *image, RenderObject *item,
...
void SVGImageBufferTools::clipToImageBuffer( GraphicsContext *context, const AffineTransform &absoluteTransform,
        const FloatRect &clampedAbsoluteTargetRect, OwnPtr<ImageBuffer> &imageBuffer )
{
    ASSERT( context );
    ASSERT( imageBuffer );
...
FloatRect SVGImageBufferTools::clampedAbsoluteTargetRectForRenderer( const RenderObject *renderer,
