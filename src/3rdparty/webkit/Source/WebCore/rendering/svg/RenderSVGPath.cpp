...
void RenderSVGPath::paint( PaintInfo &paintInfo, int, int )
{
    ...
    bool drawsOutline = style()->outlineWidth() && ( childPaintInfo.phase == PaintPhaseOutline
                        || childPaintInfo.phase == PaintPhaseSelfOutline );
...
bool RenderSVGPath::nodeAtFloatPoint( const HitTestRequest &request, HitTestResult &result, const FloatPoint &pointInParent,
                                      HitTestAction hitTestAction )
{
    ...
    if ( isVisible || !hitRules.requireVisible )
    {
        ...
        if ( ( hitRules.canHitStroke && ( svgStyle->hasStroke() || !hitRules.requireStroke )
...
FloatRect RenderSVGPath::calculateMarkerBoundsIfNeeded()
{
    ...
    return m_markerLayoutInfo.calculateBoundaries( markerStart, markerMid, markerEnd, svgStyle->strokeWidth().value( svgElement ),
            m_path );
}
