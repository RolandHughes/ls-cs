...
bool RenderSVGResourceMasker::applyResource( RenderObject *object, RenderStyle *, GraphicsContext *&context,
        unsigned short resourceMode )
{
    ...
    if ( !maskerData->maskImage && !clampedAbsoluteTargetRect.isEmpty() )
    {
        ...
        if ( !SVGImageBufferTools::createImageBuffer( absoluteTargetRect, clampedAbsoluteTargetRect, maskerData->maskImage,
                ColorSpaceLinearRGB ) )
        {
            return false;
        }
