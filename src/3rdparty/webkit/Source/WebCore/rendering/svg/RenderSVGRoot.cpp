...
void RenderSVGRoot::paint( PaintInfo &paintInfo, int parentX, int parentY )
{
    ...
    if ( hasBoxDecorations() && ( paintInfo.phase == PaintPhaseBlockBackground || paintInfo.phase == PaintPhaseChildBlockBackground )
            && isVisible )
    {
...
const AffineTransform &RenderSVGRoot::localToParentTransform() const
{
    ...
    m_localToParentTransform = AffineTransform::translation( parentToBorderBoxOffset.width(),
                               parentToBorderBoxOffset.height() ) * localToBorderBoxTransform();
...
void RenderSVGRoot::mapLocalToContainer( RenderBoxModelObject *repaintContainer, bool fixed, bool useTransforms,
...
bool RenderSVGRoot::nodeAtPoint( const HitTestRequest &request, HitTestResult &result, int x, int y, int tx, int ty,
