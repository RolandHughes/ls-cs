...
int SVGInlineTextBox::offsetForPositionInFragment( const SVGTextFragment &fragment, float position,
        bool includePartialGlyphs ) const
{
    ...
    return fragment.characterOffset - start() + textRenderer->scaledFont().offsetForPosition( textRun, position * scalingFactor,
...
FloatRect SVGInlineTextBox::selectionRectForTextFragment( const SVGTextFragment &fragment, int startPosition, int endPosition,
        RenderStyle *style )
{
    ...
    FloatRect selectionRect = scaledFont.selectionRectForText( constructTextRun( style, fragment ), textOrigin,
...
void SVGInlineTextBox::paintSelectionBackground( PaintInfo &paintInfo )
{
    ...
    for ( unsigned i = 0; i < textFragmentsSize; ++i )
    {
        ...
        paintInfo.context->fillRect( selectionRectForTextFragment( fragment, fragmentStartPosition, fragmentEndPosition, style ),
                                     backgroundColor, style->colorSpace() );
...
bool SVGInlineTextBox::acquirePaintingResource( GraphicsContext *&context, float scalingFactor, RenderObject *renderer,
        RenderStyle *style )
{
    ASSERT( scalingFactor );
    ASSERT( renderer );
...
bool SVGInlineTextBox::prepareGraphicsContextForTextPainting( GraphicsContext *&context, float scalingFactor, TextRun &textRun,
        RenderStyle *style )
{
...
bool SVGInlineTextBox::mapStartEndPositionsIntoFragmentCoordinates( const SVGTextFragment &fragment, int &startPosition,
...
void SVGInlineTextBox::paintDecorationWithStyle( GraphicsContext *context, ETextDecoration decoration,
        const SVGTextFragment &fragment, RenderObject *decorationRenderer )
{
    ...
    decorationOrigin.move( 0, -scaledFontMetrics.floatAscent() + positionOffsetForDecoration( decoration, scaledFontMetrics,
                           thickness ) );
...
void SVGInlineTextBox::paintTextWithShadows( GraphicsContext *context, RenderStyle *style, TextRun &textRun,
        const SVGTextFragment &fragment, int startPosition, int endPosition )
{
    RenderSVGInlineText *textRenderer = toRenderSVGInlineText( this->textRenderer() );
    ASSERT( textRenderer );
...
void SVGInlineTextBox::paintText( GraphicsContext *context, RenderStyle *style, RenderStyle *selectionStyle,
