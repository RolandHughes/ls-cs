...
static inline RenderSVGResourceContainer *paintingResourceFromSVGPaint( Document *document,
        const SVGPaint::SVGPaintType &paintType, const String &paintUri, AtomicString &id, bool &hasPendingResource )
{
    ...
    if ( resourceType != PatternResourceType && resourceType != LinearGradientResourceType
...
bool SVGResources::buildCachedResources( const RenderObject *object, const SVGRenderStyle *style )
{
    ...
    if ( fillAndStrokeTags().contains( tagNameImpl ) )
    {
        ...
        if ( style->hasStroke() )
        {
            ...
            if ( setStroke( paintingResourceFromSVGPaint( document, style->strokePaintType(), style->strokePaintUri(), id,
                            hasPendingResource ) ) )
            {
                foundResources = true;
            }
