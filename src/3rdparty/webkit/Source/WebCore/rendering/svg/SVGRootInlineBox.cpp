...
void SVGRootInlineBox::paint( PaintInfo &paintInfo, int, int, int, int )
{
    ...
    if ( SVGRenderSupport::prepareToRenderSVGContent( boxRenderer, childPaintInfo ) )
    {
        for ( InlineBox *child = firstChild(); child; child = child->nextOnLine() )
        {
            if ( child->isSVGInlineTextBox() )
            {
                SVGInlineFlowBox::computeTextMatchMarkerRectForRenderer( toRenderSVGInlineText( static_cast<SVGInlineTextBox *>
                        ( child )->textRenderer() ) );
            }
...
static inline void swapItemsInLayoutAttributes( SVGTextLayoutAttributes &firstAttributes, SVGTextLayoutAttributes &lastAttributes,
        unsigned firstPosition, unsigned lastPosition )
{
...
static inline void findFirstAndLastAttributesInVector( Vector<SVGTextLayoutAttributes> &attributes,
        RenderSVGInlineText *firstContext, RenderSVGInlineText *lastContext,
...
static inline void reverseInlineBoxRangeAndValueListsIfNeeded( void *userData, Vector<InlineBox *>::iterator first,
