        ...
        RenderObject *object )
{
    ...
    FloatRect clampedAbsoluteTargetRect = SVGImageBufferTools::clampedAbsoluteTargetRectForRenderer( textRootBlock,
                                          absoluteTargetRect );
...
bool RenderSVGResourceGradient::applyResource( RenderObject *object, RenderStyle *style, GraphicsContext *&context,
        ...
        const Path *path )
{
    ...
    if ( resourceMode & ApplyToTextMode )
    {
        ...
        if ( m_savedContext && m_gradient.contains( object ) )
        {
            ...
            FloatRect targetRect;
            gradientData->gradient->setGradientSpaceTransform( clipToTextMask( context, m_imageBuffer, targetRect, object, boundingBoxMode(),
                    gradientTransform ) );
            context->setFillGradient( gradientData->gradient );
