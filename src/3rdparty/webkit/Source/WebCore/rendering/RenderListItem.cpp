...
{
    for ( RenderObject *renderer = item->previousInPreOrder(); renderer
            && renderer != list->renderer(); renderer = renderer->previousInPreOrder() )
    {
...
static RenderObject *getParentOfFirstLineBox( RenderBlock *curr, RenderObject *marker )
{
    ...
    for ( RenderObject *currChild = firstChild; currChild; currChild = currChild->nextSibling() )
    {
        ...
        if ( currChild->isTable() || !currChild->isRenderBlock() || ( currChild->isBox()
                && toRenderBox( currChild )->isWritingModeRoot() ) )
        {
            break;
        }
