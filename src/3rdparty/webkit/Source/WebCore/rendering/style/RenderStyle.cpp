...
void RenderStyle::applyTransform( TransformationMatrix &transform, const IntSize &borderBoxSize,
                                  ApplyTransformOrigin applyOrigin ) const
{
    ...
    {
        transform.translate3d( transformOriginX().calcFloatValue( borderBoxSize.width() ),
                               transformOriginY().calcFloatValue( borderBoxSize.height() ), transformOriginZ() );
    }
...
static float calcConstraintScaleFor( const IntRect &rect, const RoundedIntRect::Radii &radii )
{
    ...
    // top
    radiiSum = static_cast<unsigned>( radii.topLeft().width() ) + static_cast<unsigned>
...
RoundedIntRect RenderStyle::getRoundedBorderFor( const IntRect &borderRect, bool includeLogicalLeftEdge,
...
RoundedIntRect RenderStyle::getRoundedInnerBorderFor( const IntRect &borderRect, bool includeLogicalLeftEdge,
        bool includeLogicalRightEdge ) const
{
    ...
    return getRoundedInnerBorderFor( borderRect, topWidth, bottomWidth, leftWidth, rightWidth, includeLogicalLeftEdge,
