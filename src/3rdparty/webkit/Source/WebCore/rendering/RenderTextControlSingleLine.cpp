...
bool RenderTextControlSingleLine::nodeAtPoint( const HitTestRequest &request, HitTestResult &result, int xPos, int yPos, int tx,
        int ty, HitTestAction hitTestAction )
{
...
void RenderTextControlSingleLine::forwardEvent( Event *event )
{
    ...
    if ( RenderBox *speechBox = m_speechButton ? m_speechButton->renderBox() : 0 )
    {
        ...
        FloatPoint pointInTextControlCoords = parent->absoluteToLocal( static_cast<MouseEvent *>( event )->absoluteLocation(), false,
                                              true );
...
void RenderTextControlSingleLine::createSubtreeIfNeeded()
{
    if ( inputElement()->isSearchField() )
    {
        ...
        if ( !m_resultsButton )
        {
            ...
            m_resultsButton->attachInnerElement( m_innerBlock.get(), createResultsButtonStyle( m_innerBlock->renderer()->style() ),
                                                 renderArena() );
        }
        ...
        if ( !m_cancelButton )
        {
            ...
            m_cancelButton->attachInnerElement( m_innerBlock.get(), createCancelButtonStyle( m_innerBlock->renderer()->style() ),
                                                renderArena() );
        }
    }
    else
...
{
    return PopupMenuStyle( style()->visitedDependentColor( CSSPropertyColor ),
                           style()->visitedDependentColor( CSSPropertyBackgroundColor ), style()->font(), style()->visibility() == VISIBLE,
                           style()->display() == NONE, style()->textIndent(), style()->direction(), style()->unicodeBidi() == Override );
}
...
int RenderTextControlSingleLine::clientPaddingLeft() const
{
    ...
    if ( RenderBox *resultsRenderer = m_resultsButton ? m_resultsButton->renderBox() : 0 )
    {
        padding += resultsRenderer->width() + resultsRenderer->marginLeft() + resultsRenderer->paddingLeft() +
                   resultsRenderer->marginRight() + resultsRenderer->paddingRight();
    }
...
int RenderTextControlSingleLine::clientPaddingRight() const
{
    ...
    if ( RenderBox *cancelRenderer = m_cancelButton ? m_cancelButton->renderBox() : 0 )
    {
        padding += cancelRenderer->width() + cancelRenderer->marginLeft() + cancelRenderer->paddingLeft() + cancelRenderer->marginRight()
                   + cancelRenderer->paddingRight();
    }
...
bool RenderTextControlSingleLine::logicalScroll( ScrollLogicalDirection direction, ScrollGranularity granularity,
        float multiplier, Node **stopNode )
{
    RenderLayer *layer = innerTextElement()->renderBox()->layer();

    if ( layer
            && layer->scroll( logicalToPhysical( direction, style()->isHorizontalWritingMode(), style()->isFlippedBlocksWritingMode() ),
...
PassRefPtr<Scrollbar> RenderTextControlSingleLine::createScrollbar( ScrollableArea *scrollableArea,
