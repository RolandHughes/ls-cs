...
IntSize RenderImage::imageSizeForError( CachedImage *newImage ) const
{
    ...
    return IntSize( paddingWidth + newImage->image()->width() * style()->effectiveZoom(),
                    paddingHeight + newImage->image()->height() * style()->effectiveZoom() );
}
...
bool RenderImage::setImageSizeForAltText( CachedImage *newImage /* = 0 */ )
{
    ...
    if ( !m_altText.isEmpty() )
    {
        ...
        IntSize textSize( min( font.width( TextRun( m_altText.characters(), m_altText.length() ) ), maxAltTextWidth ),
                          min( font.fontMetrics().height(), maxAltTextHeight ) );
...
void RenderImage::imageDimensionsChanged( bool imageSizeChanged, const IntRect *rect )
{
    ...
    if ( shouldRepaint )
    {
        ...
        if ( rect )
        {
            ...
            repaintRect = enclosingIntRect( mapRect( *rect, FloatRect( FloatPoint(), m_imageResource->imageSize( 1.0f ) ),
                                            contentBoxRect() ) );
...
void RenderImage::paintIntoRect( GraphicsContext *context, const IntRect &rect )
{
    ...
    context->drawImage( m_imageResource->image( rect.width(), rect.height() ).get(), style()->colorSpace(), rect, compositeOperator,
...
bool RenderImage::nodeAtPoint( const HitTestRequest &request, HitTestResult &result, int x, int y, int tx, int ty,
