...
void AutoTableLayout::recalcColumn( int effCol )
{
    ...
    for ( RenderObject *child = m_table->firstChild(); child; child = child->nextSibling() )
    {
        ...
        else if ( child->isTableSection() )
        {
            ...
            for ( int i = 0; i < numRows; i++ )
            {
                ...
                bool cellHasContent = cell && !current.inColSpan && ( cell->firstChild() || cell->style()->hasBorder()
                    ...
                    switch ( cellLogicalWidth.type() )
                    {
                        ...
                        case Percent:
                            ...
                            if ( cellLogicalWidth.isPositive() && ( !columnLayout.logicalWidth.isPercent()
                                                                    || cellLogicalWidth.value() > columnLayout.logicalWidth.value() ) )
                            {
                        ...
                        case Relative:
                            ...
                            if ( cellLogicalWidth.isAuto() || ( cellLogicalWidth.isRelative()
                                                                && cellLogicalWidth.value() > columnLayout.logicalWidth.value() ) )
                            {
                                columnLayout.logicalWidth = cellLogicalWidth;
                            }
    ...
    if ( columnLayout.logicalWidth.isFixed() )
    {
        if ( m_table->document()->inQuirksMode() && columnLayout.maxLogicalWidth > columnLayout.logicalWidth.value()
...
int AutoTableLayout::calcEffectiveLogicalWidth()
{
    ...
    for ( size_t i = 0; i < m_spanCells.size(); ++i )
    {
        ...
        if ( cellLogicalWidth.isPercent() )
        {
            ...
            {
                maxLogicalWidth = max( maxLogicalWidth, static_cast<float>( max( spanMaxLogicalWidth,
                                       cellMaxLogicalWidth ) * 100  / cellLogicalWidth.percent() ) );
        ...
        if ( cellMinLogicalWidth > spanMinLogicalWidth )
        {
            if ( allColsAreFixed )
            {
                ...
                {
                    int cellLogicalWidth = max( m_layoutStruct[pos].effectiveMinLogicalWidth,
                                                cellMinLogicalWidth * m_layoutStruct[pos].logicalWidth.value() / fixedWidth );
            ...
            else
            {
                ...
                for ( unsigned pos = effCol; remainingMaxLogicalWidth >= 0 && pos < lastCol
                        && remainingMinLogicalWidth < cellMinLogicalWidth; ++pos )
                {
                    if ( !( m_layoutStruct[pos].logicalWidth.isFixed() && haveAuto && fixedWidth <= cellMinLogicalWidth ) )
                    {
                        int colMinLogicalWidth = max( m_layoutStruct[pos].effectiveMinLogicalWidth,
                                                      static_cast<int>( remainingMaxLogicalWidth ? cellMinLogicalWidth * static_cast<float>
                                                              ( m_layoutStruct[pos].effectiveMaxLogicalWidth ) / remainingMaxLogicalWidth : cellMinLogicalWidth ) );
                        colMinLogicalWidth = min( m_layoutStruct[pos].effectiveMinLogicalWidth + ( cellMinLogicalWidth - remainingMinLogicalWidth ),
                                                  colMinLogicalWidth );
                        remainingMaxLogicalWidth -= m_layoutStruct[pos].effectiveMaxLogicalWidth;
                        remainingMinLogicalWidth -= m_layoutStruct[pos].effectiveMinLogicalWidth;
                        cellMinLogicalWidth -= colMinLogicalWidth;
        ...
        if ( !cellLogicalWidth.isPercent() )
        {
            if ( cellMaxLogicalWidth > spanMaxLogicalWidth )
            {
                ...
                {
                    int colMaxLogicalWidth = max( m_layoutStruct[pos].effectiveMaxLogicalWidth,
                                                  static_cast<int>( spanMaxLogicalWidth ? cellMaxLogicalWidth * static_cast<float>( m_layoutStruct[pos].effectiveMaxLogicalWidth ) /
...
void AutoTableLayout::layout()
{
    ...
    if ( available > 0 && numAuto )
    {
        ...
        for ( size_t i = 0; i < nEffCols; ++i )
        {
            ...
            {
                int cellLogicalWidth = max( m_layoutStruct[i].computedLogicalWidth,
