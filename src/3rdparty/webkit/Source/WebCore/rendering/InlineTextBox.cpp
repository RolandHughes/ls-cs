...
int InlineTextBox::baselinePosition( FontBaseline baselineType ) const
{
    ...
    return toRenderBoxModelObject( renderer()->parent() )->baselinePosition( baselineType, m_firstLine,
...
int InlineTextBox::lineHeight() const
{
    ...
    return toRenderBoxModelObject( renderer()->parent() )->lineHeight( m_firstLine, isHorizontal() ? HorizontalLine : VerticalLine,
...
static void adjustCharactersAndLengthForHyphen( BufferForAppendingHyphen &charactersWithHyphen, RenderStyle *style,
...
IntRect InlineTextBox::selectionRect( int tx, int ty, int startPos, int endPos )
{
    ...
    IntRect r = enclosingIntRect( f.selectionRectForText( TextRun( characters, len, textObj->allowTabs(), textPos(), m_expansion,
                                  expansionBehavior(), direction(), m_dirOverride ),
...
float InlineTextBox::placeEllipsisBox( bool flowIsLTR, float visibleLeftEdge, float visibleRightEdge, float ellipsisWidth,
                                       bool &foundBox )
{
    if ( foundBox )
    {
...
bool InlineTextBox::isLineBreak() const
{
    return renderer()->isBR() || ( renderer()->style()->preserveNewline() && len() == 1
                                   && ( *textRenderer()->text() )[start()] == '\n' );
}

bool InlineTextBox::nodeAtPoint( const HitTestRequest &, HitTestResult &result, int x, int y, int tx, int ty, int /* lineTop */,
                                 int /*lineBottom*/ )
{
    if ( isLineBreak() )
    {
...
FloatSize InlineTextBox::applyShadowToGraphicsContext( GraphicsContext *context, const ShadowData *shadow,
        const FloatRect &textRect, bool stroked, bool opaque, bool horizontal )
{
    if ( !shadow )
    {
        return FloatSize();
...
static void paintTextWithShadows( GraphicsContext *context, const Font &font, const TextRun &textRun,
                                  const AtomicString &emphasisMark, int emphasisMarkOffset, int startOffset, int endOffset, int truncationPoint,
                                  ...
                                  const FloatRect &boxRect, const ShadowData *shadow, bool stroked, bool horizontal )
{
    ...
    do
    {
        ...
        if ( shadow )
        {
            extraOffset = roundedIntSize( InlineTextBox::applyShadowToGraphicsContext( context, shadow, boxRect, stroked, opaque,
                                          horizontal ) );
        }
        else if ( !opaque )
        {
        ...
        if ( startOffset <= endOffset )
        {
            ...
            {
                context->drawEmphasisMarks( font, textRun, emphasisMark, textOrigin + extraOffset + IntSize( 0, emphasisMarkOffset ), startOffset,
                                            endOffset );
            }
        }
        else
        {
            if ( endOffset > 0 )
            {
                ...
                {
                    context->drawEmphasisMarks( font, textRun, emphasisMark, textOrigin + extraOffset + IntSize( 0, emphasisMarkOffset ),  0,
                                                endOffset );
                }
            }
            ...
            if ( startOffset < truncationPoint )
            {
                ...
                {
                    context->drawEmphasisMarks( font, textRun, emphasisMark, textOrigin + extraOffset + IntSize( 0, emphasisMarkOffset ),
                                                startOffset, truncationPoint );
                }
            }
        }
...
void InlineTextBox::paint( PaintInfo &paintInfo, int tx, int ty, int /*lineTop*/, int /*lineBottom*/ )
{
    ...
    RenderCombineText *combinedText = styleToUse->hasTextCombine() && textRenderer()->isCombineText()
                                      && toRenderCombineText( textRenderer() )->isCombined() ? toRenderCombineText( textRenderer() ) : 0;
    ...
    TextRun textRun( characters, length, textRenderer()->allowTabs(), textPos(), m_expansion, expansionBehavior(), direction(),
    ...
    if ( !emphasisMark.isEmpty() )
    {
        emphasisMarkOffset = emphasisMarkPosition == TextEmphasisPositionOver ? -font.fontMetrics().ascent() - font.emphasisMarkDescent(
                                 emphasisMark ) : font.fontMetrics().descent() + font.emphasisMarkAscent( emphasisMark );
    }
    ...
    if ( !paintSelectedTextOnly )
    {
        ...
        if ( !paintSelectedTextSeparately || ePos <= sPos )
        {
            ...
            paintTextWithShadows( context, font, textRun, nullAtom, 0, 0, length, length, textOrigin, boxRect, textShadow,
                                  textStrokeWidth > 0, isHorizontal() );
        }
        else
        {
            paintTextWithShadows( context, font, textRun, nullAtom, 0, ePos, sPos, length, textOrigin, boxRect, textShadow,
                                  textStrokeWidth > 0, isHorizontal() );
        }
        ...
        if ( !emphasisMark.isEmpty() )
        {
            ...
            FloatPoint emphasisMarkTextOrigin = combinedText ? FloatPoint( boxOrigin.x() + boxRect.width() / 2,
                                                boxOrigin.y() + font.fontMetrics().ascent() ) : textOrigin;
            ...
            {
                // FIXME: Truncate right-to-left text correctly.
                paintTextWithShadows( context, combinedText ? combinedText->originalFont() : font, emphasisMarkTextRun, emphasisMark,
                                      emphasisMarkOffset, 0, length, length, emphasisMarkTextOrigin, boxRect, textShadow, textStrokeWidth > 0, isHorizontal() );
            }
            else
            {
                paintTextWithShadows( context, combinedText ? combinedText->originalFont() : font, emphasisMarkTextRun, emphasisMark,
                                      emphasisMarkOffset, ePos, sPos, length, emphasisMarkTextOrigin, boxRect, textShadow, textStrokeWidth > 0, isHorizontal() );
            }
    ...
    if ( ( paintSelectedTextOnly || paintSelectedTextSeparately ) && sPos < ePos )
    {
        ...
        paintTextWithShadows( context, font, textRun, nullAtom, 0, sPos, ePos, length, textOrigin, boxRect, selectionShadow,
                              selectionStrokeWidth > 0, isHorizontal() );
        ...
        if ( !emphasisMark.isEmpty() )
        {
            ...
            paintTextWithShadows( context, combinedText ? combinedText->originalFont() : font, emphasisMarkTextRun, emphasisMark,
                                  emphasisMarkOffset, sPos, ePos, length, emphasisMarkTextOrigin, boxRect, selectionShadow, selectionStrokeWidth > 0,
                                  isHorizontal() );
...
void InlineTextBox::paintSelection( GraphicsContext *context, const FloatPoint &boxOrigin, RenderStyle *style, const Font &font )
{
    ...
    int deltaY = renderer()->style()->isFlippedLinesWritingMode() ? selectionBottom() - logicalBottom() : logicalTop() -
                 selectionTop();
    int selHeight = selectionHeight();
    ...
    context->drawHighlightForText( font, TextRun( characters, length, textRenderer()->allowTabs(), textPos(), m_expansion,
                                   expansionBehavior(),
                                   direction(), m_dirOverride || style->visuallyOrdered() ),
                                   localOrigin, selHeight, c, style->colorSpace(), sPos, ePos );
}

void InlineTextBox::paintCompositionBackground( GraphicsContext *context, const FloatPoint &boxOrigin, RenderStyle *style,
        const Font &font, int startPos, int endPos )
{
    int offset = m_start;
    ...
    int selHeight = selectionHeight();
    FloatPoint localOrigin( boxOrigin.x(), boxOrigin.y() - deltaY );
    context->drawHighlightForText( font, TextRun( textRenderer()->text()->characters() + m_start, m_len, textRenderer()->allowTabs(),
                                   textPos(), m_expansion, expansionBehavior(),
                                   direction(), m_dirOverride || style->visuallyOrdered() ),
                                   localOrigin, selHeight, c, style->colorSpace(), sPos, ePos );
}
...
void InlineTextBox::paintDecoration( GraphicsContext *context, const FloatPoint &boxOrigin, int deco, const ShadowData *shadow )
{
    ...
    bool linesAreOpaque = !isPrinting && ( !( deco & UNDERLINE ) || underline.alpha() == 255 ) && ( !( deco & OVERLINE )
                          || overline.alpha() == 255 ) && ( !( deco & LINE_THROUGH ) || linethrough.alpha() == 255 );
...
void InlineTextBox::paintSpellingOrGrammarMarker( GraphicsContext *pt, const FloatPoint &boxOrigin, const DocumentMarker &marker,
        RenderStyle *style, const Font &font, bool grammar )
{
    ...
    if ( !markerSpansWholeBox || grammar )
    {
        ...
        TextRun run( textRenderer()->text()->characters() + m_start, m_len, textRenderer()->allowTabs(), textPos(), m_expansion,
                     expansionBehavior(), direction(), m_dirOverride || style->visuallyOrdered() );
    ...
    pt->drawLineForTextChecking( FloatPoint( boxOrigin.x() + start, boxOrigin.y() + underlineOffset ), width,
                                 textCheckingLineStyleForMarkerType( marker.type ) );
}

void InlineTextBox::paintTextMatchMarker( GraphicsContext *pt, const FloatPoint &boxOrigin, const DocumentMarker &marker,
        RenderStyle *style, const Font &font )
{
    ...
    if ( renderer()->frame()->editor()->markedTextMatchesAreHighlighted() )
    {
        ...
        pt->drawHighlightForText( font, run, FloatPoint( boxOrigin.x(), boxOrigin.y() - deltaY ), selHeight, color, style->colorSpace(),
                                  sPos, ePos );
    }
}
...
void InlineTextBox::paintDocumentMarkers( GraphicsContext *pt, const FloatPoint &boxOrigin, RenderStyle *style, const Font &font,
        bool background )
{
    if ( !renderer()->node() )
    {
...
void InlineTextBox::paintCompositionUnderline( GraphicsContext *ctx, const FloatPoint &boxOrigin,
...
int InlineTextBox::offsetForPosition( float lineOffset, bool includePartialGlyphs ) const
{
    ...
    int offset = f->offsetForPosition( TextRun( textRenderer()->text()->characters() + m_start, m_len,
                                       textRenderer()->allowTabs(), textPos(), m_expansion, expansionBehavior(), direction(), m_dirOverride
                                       || style->visuallyOrdered() ),
...
float InlineTextBox::positionForOffset( int offset ) const
{
    ...
    return f.selectionRectForText( TextRun( text->text()->characters() + m_start, m_len, textRenderer()->allowTabs(), textPos(),
                                            m_expansion, expansionBehavior(), direction(), m_dirOverride ),
