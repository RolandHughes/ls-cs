...
public:
    ...
    bool isEmpty()
    {
        return m_objectLayerSizeMap.isEmpty();
    }
...
bool ImageQualityController::shouldPaintAtLowQuality( GraphicsContext *context, RenderBoxModelObject *object, Image *image,
        const void *layer, const IntSize &size )
{
...
bool RenderBoxModelObject::shouldPaintAtLowQuality( GraphicsContext *context, Image *image, const void *layer,
        const IntSize &size )
{
...
RoundedIntRect RenderBoxModelObject::getBackgroundRoundedRect( const IntRect &borderRect, InlineFlowBox *box, int inlineBoxWidth,
        int inlineBoxHeight,
        bool includeLogicalLeftEdge, bool includeLogicalRightEdge )
{
    ...
    {
        RoundedIntRect segmentBorder = style()->getRoundedBorderFor( IntRect( 0, 0, inlineBoxWidth, inlineBoxHeight ),
...
static IntRect backgroundRectAdjustedForBleedAvoidance( GraphicsContext *context, const IntRect &borderRect,
...
void RenderBoxModelObject::paintFillLayerExtended( const PaintInfo &paintInfo, const Color &color, const FillLayer *bgLayer,
        int tx, int ty, int w, int h,
        BackgroundBleedAvoidance bleedAvoidance, InlineFlowBox *box, int inlineBoxWidth, int inlineBoxHeight, CompositeOperator op,
        RenderObject *backgroundObject )
{
    ...
    if ( !isRoot && !clippedWithLocalScrolling && !shouldPaintBackgroundImage && isBorderFill )
    {
        ...
        {
            RoundedIntRect border = getBackgroundRoundedRect( backgroundRectAdjustedForBleedAvoidance( context, borderRect, bleedAvoidance ),
                                    box, inlineBoxWidth, inlineBoxHeight, includeLeftEdge, includeRightEdge );
...
IntSize RenderBoxModelObject::calculateFillTileSize( const FillLayer *fillLayer, IntSize positioningAreaSize ) const
{
    ...
    switch ( type )
    {
        ...
        case Cover:
        {
            ...
            float scaleFactor = type == Contain ? min( horizontalScaleFactor, verticalScaleFactor ) : max( horizontalScaleFactor,
                                verticalScaleFactor );
            return IntSize( max<int>( 1, imageIntrinsicSize.width() * scaleFactor ), max<int>( 1,
                            imageIntrinsicSize.height() * scaleFactor ) );
        }
...
bool RenderBoxModelObject::paintNinePieceImage( GraphicsContext *graphicsContext, int tx, int ty, int w, int h,
        ...
        const NinePieceImage &ninePieceImage, CompositeOperator op )
{
    ...
    if ( drawRight )
    {
        ...
        if ( drawBottom )
            graphicsContext->drawImage( image.get(), colorSpace, IntRect( tx + w - rightWidth, ty + h - bottomWidth, rightWidth,
                                        bottomWidth ),
...
public:
    ...
    {
        return isPresent && hasVisibleColorAndStyle();
    }
...
void RenderBoxModelObject::paintOneBorderSide( GraphicsContext *graphicsContext, const RenderStyle *style,
        const RoundedIntRect &outerBorder, const RoundedIntRect &innerBorder,
        const IntRect &sideRect, BoxSide side, BoxSide adjacentSide1, BoxSide adjacentSide2, const BorderEdge edges[], const Path *path,
        BackgroundBleedAvoidance bleedAvoidance, bool includeLogicalLeftEdge, bool includeLogicalRightEdge, bool antialias,
        const Color *overrideColor )
{
    ...
    else
    {
        ...
        drawLineForBoxSide( graphicsContext, sideRect.x(), sideRect.y(), sideRect.maxX(), sideRect.maxY(), side, colorToPaint,
                            edgeToRender.style,
                            mitreAdjacentSide1 ? adjacentEdge1.width : 0, mitreAdjacentSide2 ? adjacentEdge2.width : 0, antialias );
    }
}

void RenderBoxModelObject::paintBorderSides( GraphicsContext *graphicsContext, const RenderStyle *style,
        const RoundedIntRect &outerBorder, const RoundedIntRect &innerBorder,
        ...
        bool includeLogicalLeftEdge, bool includeLogicalRightEdge, bool antialias, const Color *overrideColor )
{
    ...
    if ( edges[BSTop].shouldRender() && includesEdge( edgeSet, BSTop ) )
    {
        ...
        bool usePath = renderRadii && ( borderStyleHasInnerDetail( edges[BSTop].style )
                                        || borderWillArcInnerEdge( innerBorder.radii().topLeft(), innerBorder.radii().topRight() ) );
        paintOneBorderSide( graphicsContext, style, outerBorder, innerBorder, sideRect, BSTop, BSLeft, BSRight, edges,
                            usePath ? &roundedPath : 0, bleedAvoidance, includeLogicalLeftEdge, includeLogicalRightEdge, antialias, overrideColor );
    }
    ...
    if ( edges[BSBottom].shouldRender() && includesEdge( edgeSet, BSBottom ) )
    {
        ...
        bool usePath = renderRadii && ( borderStyleHasInnerDetail( edges[BSBottom].style )
                                        || borderWillArcInnerEdge( innerBorder.radii().bottomLeft(), innerBorder.radii().bottomRight() ) );
        paintOneBorderSide( graphicsContext, style, outerBorder, innerBorder, sideRect, BSBottom, BSLeft, BSRight, edges,
                            usePath ? &roundedPath : 0, bleedAvoidance, includeLogicalLeftEdge, includeLogicalRightEdge, antialias, overrideColor );
    }
    ...
    if ( edges[BSLeft].shouldRender() && includesEdge( edgeSet, BSLeft ) )
    {
        ...
        bool usePath = renderRadii && ( borderStyleHasInnerDetail( edges[BSLeft].style )
                                        || borderWillArcInnerEdge( innerBorder.radii().bottomLeft(), innerBorder.radii().topLeft() ) );
        paintOneBorderSide( graphicsContext, style, outerBorder, innerBorder, sideRect, BSLeft, BSTop, BSBottom, edges,
                            usePath ? &roundedPath : 0, bleedAvoidance, includeLogicalLeftEdge, includeLogicalRightEdge, antialias, overrideColor );
    }
    ...
    if ( edges[BSRight].shouldRender() && includesEdge( edgeSet, BSRight ) )
    {
        ...
        bool usePath = renderRadii && ( borderStyleHasInnerDetail( edges[BSRight].style )
                                        || borderWillArcInnerEdge( innerBorder.radii().bottomRight(), innerBorder.radii().topRight() ) );
        paintOneBorderSide( graphicsContext, style, outerBorder, innerBorder, sideRect, BSRight, BSTop, BSBottom, edges,
                            usePath ? &roundedPath : 0, bleedAvoidance, includeLogicalLeftEdge, includeLogicalRightEdge, antialias, overrideColor );
    }
}

void RenderBoxModelObject::paintTranslucentBorderSides( GraphicsContext *graphicsContext, const RenderStyle *style,
        const RoundedIntRect &outerBorder, const RoundedIntRect &innerBorder,
        const BorderEdge edges[], BackgroundBleedAvoidance bleedAvoidance, bool includeLogicalLeftEdge, bool includeLogicalRightEdge,
        bool antialias )
{
    ...
    while ( edgesToDraw )
    {
        ...
        paintBorderSides( graphicsContext, style, outerBorder, innerBorder, edges, commonColorEdgeSet, bleedAvoidance,
                          includeLogicalLeftEdge, includeLogicalRightEdge, antialias, &commonColor );
                                        ...
                                        const RenderStyle *style, BackgroundBleedAvoidance bleedAvoidance, bool includeLogicalLeftEdge, bool includeLogicalRightEdge )
{
    ...
    if ( haveAlphaColor )
    {
        paintTranslucentBorderSides( graphicsContext, style, outerBorder, innerBorder, edges, bleedAvoidance, includeLogicalLeftEdge,
                                     includeLogicalRightEdge, antialias );
    }
    else
    {
...
void RenderBoxModelObject::drawBoxSideFromPath( GraphicsContext *graphicsContext, const IntRect &borderRect,
        const Path &borderPath, const BorderEdge edges[],
        float thickness, float drawThickness, BoxSide side, const RenderStyle *style,
        Color color, EBorderStyle borderStyle, BackgroundBleedAvoidance bleedAvoidance, bool includeLogicalLeftEdge,
    ...
    switch ( borderStyle )
    {
        ...
        case DOUBLE:
        {
            ...
            // Draw inner border line
            {
                ...
                drawBoxSideFromPath( graphicsContext, borderRect, borderPath, edges, thickness, drawThickness, side, style, color, SOLID,
                                     bleedAvoidance, includeLogicalLeftEdge, includeLogicalRightEdge );
            }
        ...
        case GROOVE:
        {
            ...
            // Paint full border
            drawBoxSideFromPath( graphicsContext, borderRect, borderPath, edges, thickness, drawThickness, side, style, color, s1,
                                 bleedAvoidance, includeLogicalLeftEdge, includeLogicalRightEdge );
            ...
            drawBoxSideFromPath( graphicsContext, borderRect, borderPath, edges, thickness, drawThickness, side, style, color, s2,
                                 bleedAvoidance, includeLogicalLeftEdge, includeLogicalRightEdge );
            return;
        }
                                        ...
                                        const RenderStyle *style, BackgroundBleedAvoidance, bool includeLogicalLeftEdge, bool includeLogicalRightEdge )
{
    ...
    bool upperRightBorderStylesMatch = renderRight && ( topStyle == rightStyle ) && ( topColor == rightColor )
                                       && ( topStyle != OUTSET ) && ( topStyle != RIDGE ) && ( topStyle != INSET ) && ( topStyle != GROOVE );
    bool lowerLeftBorderStylesMatch = renderLeft && ( bottomStyle == leftStyle ) && ( bottomColor == leftColor )
                                      && ( bottomStyle != OUTSET ) && ( bottomStyle != RIDGE ) && ( bottomStyle != INSET ) && ( bottomStyle != GROOVE );
        ...
        if ( renderRadii )
        {
            ...
            if ( border.radii().topLeft().width() )
            {
                ...
                if ( applyLeftInnerClip )
                    graphicsContext->addInnerRoundedRectClip( IntRect( leftX, leftY, border.radii().topLeft().width() * 2,
                            border.radii().topLeft().height() * 2 ),
            ...
            if ( border.radii().topRight().width() )
            {
                ...
                if ( applyRightInnerClip )
                    graphicsContext->addInnerRoundedRectClip( IntRect( rightX, leftY, border.radii().topRight().width() * 2,
                            border.radii().topRight().height() * 2 ),
                            style->borderTopWidth() );
    ...
    if ( renderBottom )
    {
        ...
        if ( renderRadii )
        {
            ...
            if ( border.radii().bottomLeft().width() )
            {
                ...
                if ( applyLeftInnerClip )
                    graphicsContext->addInnerRoundedRectClip( IntRect( leftX, leftY, border.radii().bottomLeft().width() * 2,
                            border.radii().bottomLeft().height() * 2 ),
            ...
            if ( border.radii().bottomRight().width() )
            {
                ...
                if ( applyRightInnerClip )
                    graphicsContext->addInnerRoundedRectClip( IntRect( rightX, rightY, border.radii().bottomRight().width() * 2,
                            border.radii().bottomRight().height() * 2 ),
                            style->borderBottomWidth() );
        ...
        if ( renderRadii && ( !upperLeftBorderStylesMatch || !lowerLeftBorderStylesMatch ) )
        {
            ...
            if ( !upperLeftBorderStylesMatch && border.radii().topLeft().width() )
            {
                ...
                if ( applyTopInnerClip )
                    graphicsContext->addInnerRoundedRectClip( IntRect( topX, topY, border.radii().topLeft().width() * 2,
                            border.radii().topLeft().height() * 2 ),
            ...
            if ( !lowerLeftBorderStylesMatch && border.radii().bottomLeft().width() )
            {
                ...
                if ( applyBottomInnerClip )
                    graphicsContext->addInnerRoundedRectClip( IntRect( topX, bottomY, border.radii().bottomLeft().width() * 2,
                            border.radii().bottomLeft().height() * 2 ),
                            style->borderLeftWidth() );
    ...
    if ( renderRight )
    {
        ...
        if ( renderRadii && ( !upperRightBorderStylesMatch || !lowerRightBorderStylesMatch ) )
        {
            ...
            if ( !upperRightBorderStylesMatch && border.radii().topRight().width() )
            {
                ...
                if ( applyTopInnerClip )
                    graphicsContext->addInnerRoundedRectClip( IntRect( topX, topY, border.radii().topRight().width() * 2,
                            border.radii().topRight().height() * 2 ),
            ...
            if ( !lowerRightBorderStylesMatch && border.radii().bottomRight().width() )
            {
                ...
                if ( applyBottomInnerClip )
                    graphicsContext->addInnerRoundedRectClip( IntRect( bottomX, bottomY, border.radii().bottomRight().width() * 2,
                            border.radii().bottomRight().height() * 2 ),
                            style->borderRightWidth() );
...
static void findInnerVertex( const FloatPoint &outerCorner, const FloatPoint &innerCorner, const FloatPoint &centerPoint,
                             FloatPoint &result )
{
...
void RenderBoxModelObject::clipBorderSidePolygon( GraphicsContext *graphicsContext, const RoundedIntRect &outerBorder,
        const RoundedIntRect &innerBorder,
        BoxSide side, bool firstEdgeMatches, bool secondEdgeMatches )
{
    ...
    FloatPoint centerPoint( innerRect.location().x() + static_cast<float>( innerRect.width() ) / 2,
                            innerRect.location().y() + static_cast<float>( innerRect.height() ) / 2 );
...
void RenderBoxModelObject::getBorderEdgeInfo( BorderEdge edges[], bool includeLogicalLeftEdge,
...
static inline IntRect areaCastingShadowInHole( const IntRect &holeRect, int shadowBlur, int shadowSpread,
...
void RenderBoxModelObject::paintBoxShadow( GraphicsContext *context, int tx, int ty, int w, int h, const RenderStyle *s,
        ShadowStyle shadowStyle, bool includeLogicalLeftEdge, bool includeLogicalRightEdge )
{
    ...
    IntRect borderRect( tx, ty, w, h );
    RoundedIntRect border = ( shadowStyle == Inset ) ? s->getRoundedInnerBorderFor( borderRect, includeLogicalLeftEdge,
