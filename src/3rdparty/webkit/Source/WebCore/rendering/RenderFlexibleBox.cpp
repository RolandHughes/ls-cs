...
static void gatherFlexChildrenInfo( FlexBoxIterator &iterator, bool relayoutChildren, unsigned int &highestFlexGroup,
                                    unsigned int &lowestFlexGroup, bool &haveFlex )
{
...
void RenderFlexibleBox::layoutHorizontalBox( bool relayoutChildren )
{
    ...
    do
    {
        ...
        while ( child )
        {
            ...
            if ( relayoutChildren || ( child->isReplaced() && ( child->style()->width().isPercent()
        ...
        {
            setHeight( height() + lineHeight( true, style()->isHorizontalWritingMode() ? HorizontalLine : VerticalLine,
...
void RenderFlexibleBox::layoutVerticalBox( bool relayoutChildren )
{
    ...
    do
    {
        ...
        {
            // make sure we relayout children if we need it.
            if ( !haveLineClamp && ( relayoutChildren || ( child->isReplaced() && ( child->style()->width().isPercent()
                                     || child->style()->height().isPercent() ) ) ) )
            {
                child->setChildNeedsLayout( true, false );
            }
...
void RenderFlexibleBox::applyLineClamp( FlexBoxIterator &iterator, bool relayoutChildren )
{
    ...
    for ( RenderBox *child = iterator.first(); child; child = iterator.next() )
    {
        ...
        if ( !lastVisibleLine->lineCanAccommodateEllipsis( leftToRight, blockEdge, lastVisibleLine->x() + lastVisibleLine->logicalWidth(),
                totalWidth ) )
        {
            continue;
        }
        ...
        lastVisibleLine->placeEllipsis( anchorBox ? ellipsisAndSpaceStr : ellipsisStr, leftToRight, blockLeftEdge, blockRightEdge,
