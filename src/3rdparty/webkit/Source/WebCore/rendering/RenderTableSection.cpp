...
int RenderTableSection::layoutRows( int toAdd )
{
    ...
    for ( int r = 0; r < totalRows; r++ )
    {
        ...
        for ( int c = 0; c < nEffCols; c++ )
        {
            ...
            {
                if ( !o->isText() && o->style()->logicalHeight().isPercent() && ( flexAllChildren || o->isReplaced() || ( o->isBox()
                        && toRenderBox( o )->scrollsOverflow() ) ) )
                {
            ...
            {
                cell->setLogicalLocation( table()->columnPositions()[nEffCols] - table()->columnPositions()[table()->colToEffCol(
                                              cell->col() + cell->colSpan() )] + hspacing, m_rowPos[rindx] );
            }
            else
            {
            ...
            if ( !cell->needsLayout() && view()->layoutState()->pageLogicalHeight()
                    && view()->layoutState()->pageLogicalOffset( cell->logicalTop() ) != cell->pageLogicalOffset() )
            {
                cell->setChildNeedsLayout( true, false );
            }
            ...
            {
                cell->setHeight(
                    rHeight );    // FIXME: Pagination might have made us change size.  For now just shrink or grow the cell to fit without doing a relayout.
            }
...
int RenderTableSection::calcOuterBorderBefore() const
{
    ...
    for ( int c = 0; c < totalCols; c++ )
    {
        ...
        const BorderValue &cb =
            current.primaryCell()->style()->borderBefore(); // FIXME: Make this work with perpendicular and flipped cells.
...
int RenderTableSection::calcOuterBorderAfter() const
{
    ...
    for ( int c = 0; c < totalCols; c++ )
    {
        ...
        const BorderValue &cb =
            current.primaryCell()->style()->borderAfter(); // FIXME: Make this work with perpendicular and flipped cells.
        // FIXME: Don't repeat for the same col group
        RenderTableCol *colGroup = table()->colElement( c );
...
int RenderTableSection::calcOuterBorderStart() const
{
    ...
    for ( int r = 0; r < m_gridRows; r++ )
    {
        ...
        const BorderValue &cb =
            current.primaryCell()->style()->borderStart(); // FIXME: Make this work with perpendicular and flipped cells.
...
int RenderTableSection::calcOuterBorderEnd() const
{
    ...
    for ( int r = 0; r < m_gridRows; r++ )
    {
        ...
        // FIXME: Don't repeat for the same cell
        const BorderValue &cb =
            current.primaryCell()->style()->borderEnd(); // FIXME: Make this work with perpendicular and flipped cells.
...
int RenderTableSection::firstLineBoxBaseline() const
{
    ...
    for ( size_t i = 0; i < firstRow->size(); ++i )
    {
        ...
        if ( cell )
        {
            firstLineBaseline = max( firstLineBaseline,
                                     cell->logicalTop() + cell->paddingBefore() + cell->borderBefore() + cell->contentLogicalHeight() );
        }
    }
...
// Hit Testing
bool RenderTableSection::nodeAtPoint( const HitTestRequest &request, HitTestResult &result, int xPos, int yPos, int tx, int ty,
