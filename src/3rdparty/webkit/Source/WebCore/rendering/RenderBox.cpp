...
void RenderBox::destroy()
{
    ...
    if ( style() && ( style()->logicalHeight().isPercent() || style()->logicalMinHeight().isPercent()
...
void RenderBox::styleDidChange( StyleDifference diff, const RenderStyle *oldStyle )
{
    ...
    {
        if ( oldStyle && ( oldStyle->logicalHeight().isPercent() || oldStyle->logicalMinHeight().isPercent()
                           || oldStyle->logicalMaxHeight().isPercent() ) )
        {
            RenderBlock::removePercentHeightDescendant( this );
        }
        ...
        if ( isPositioned() && style()->hasStaticBlockPosition( isHorizontalWritingMode() )
                && oldStyle->marginBefore() != style()->marginBefore()
...
IntRect RenderBox::outlineBoundsForRepaint( RenderBoxModelObject *repaintContainer,
        IntPoint *cachedOffsetToRepaintContainer ) const
{
...
bool RenderBox::logicalScroll( ScrollLogicalDirection direction, ScrollGranularity granularity, float multiplier,
                               Node **stopNode )
{
    bool scrolled = false;
    ...
    if ( l )
    {
        ...
        {
            scrolled = l->scroll( logicalToPhysical( ScrollInlineDirectionBackward, isHorizontalWritingMode(),
                                  style()->isFlippedBlocksWritingMode() ), ScrollByDocument, multiplier );
        }

#endif

        if ( l->scroll( logicalToPhysical( direction, isHorizontalWritingMode(), style()->isFlippedBlocksWritingMode() ), granularity,
                        multiplier ) )
        {
            scrolled = true;
        }
...
{
    return ( hasOverflowClip() && ( scrollsOverflow() || ( node() && node()->rendererIsEditable() ) ) ) || ( node()
            && node()->isDocumentNode() );
}
...
// Hit Testing
bool RenderBox::nodeAtPoint( const HitTestRequest &request, HitTestResult &result, int xPos, int yPos, int tx, int ty,
...
void RenderBox::paintRootBoxFillLayers( const PaintInfo &paintInfo )
{
    ...
    paintFillLayers( paintInfo, bgColor, bgLayer, view()->docLeft(), view()->docTop(), view()->docWidth(), view()->docHeight(),
                     BackgroundBleedNone, CompositeSourceOver, bodyObject );
}
...
void RenderBox::paintBoxDecorationsWithSize( PaintInfo &paintInfo, int tx, int ty, int width, int height )
{
    ...
    if ( !themePainted )
    {
        ...
        else if ( !isBody() || document()->documentElement()->renderer()->hasBackground() )
        {
            ...
            paintFillLayers( paintInfo, style()->visitedDependentColor( CSSPropertyBackgroundColor ), style()->backgroundLayers(), tx, ty,
                             width, height, bleedAvoidance );
        }
    ...
    if ( ( !style()->hasAppearance() || ( !themePainted
                                          && theme()->paintBorderOnly( this, paintInfo, IntRect( tx, ty, width, height ) ) ) ) && style()->hasBorder() )
    {
...
void RenderBox::paintFillLayers( const PaintInfo &paintInfo, const Color &c, const FillLayer *fillLayer, int tx, int ty,
...
void RenderBox::paintFillLayer( const PaintInfo &paintInfo, const Color &c, const FillLayer *fillLayer, int tx, int ty, int width,
                                int height,
                                BackgroundBleedAvoidance bleedAvoidance, CompositeOperator op, RenderObject *backgroundObject )
{
...
bool RenderBox::repaintLayerRectsForImage( WrappedImagePtr image, const FillLayer *layers, bool drawingBackground )
{
    ...
    for ( const FillLayer *curLayer = layers; curLayer; curLayer = curLayer->next() )
    {
        if ( curLayer->image() && image == curLayer->image()->data() && curLayer->image()->canRender( style()->effectiveZoom() ) )
        {
            ...
            if ( !layerRenderer )
            {
                bool drawingRootBackground = drawingBackground && ( isRoot() || ( isBody()
                                             && !document()->documentElement()->renderer()->hasBackground() ) );
                ...
                if ( drawingRootBackground )
                {
                    ...
                    rendererRect = IntRect( -layerRenderer->marginLeft(),
                                            ...
                                            max( layerRenderer->width() + layerRenderer->marginLeft() + layerRenderer->marginRight() + layerRenderer->borderLeft() +
                                                 layerRenderer->borderRight(), rw ),
                                            max( layerRenderer->height() + layerRenderer->marginTop() + layerRenderer->marginBottom() + layerRenderer->borderTop() +
                                                 layerRenderer->borderBottom(), rh ) );
                }
                else
                {
                    layerRenderer = this;
            ...
            IntPoint phase;
            IntSize tileSize;
            layerRenderer->calculateBackgroundImageGeometry( curLayer, rendererRect.x(), rendererRect.y(), rendererRect.width(),
                    rendererRect.height(), repaintRect, phase, tileSize );
...
{
    if ( paintInfo.phase == PaintPhaseBlockBackground || paintInfo.phase == PaintPhaseSelfOutline
...
int RenderBox::perpendicularContainingBlockLogicalHeight() const
{
    ...
    if ( !logicalHeightLength.isFixed() )
    {
        ...
        // will decide.
        return containingBlockStyle->isHorizontalWritingMode() ? view()->frameView()->visibleHeight() :
               view()->frameView()->visibleWidth();
    }
...
void RenderBox::mapLocalToContainer( RenderBoxModelObject *repaintContainer, bool fixed, bool useTransforms,
                                     TransformState &transformState ) const
{
    ...
    else
    {
        transformState.move( containerOffset.width(), containerOffset.height(),
                             preserve3D ? TransformState::AccumulateTransform : TransformState::FlattenTransform );
    }
        ...
        if ( wasInline )
        {
            ...
            if ( style()->hasStaticInlinePosition( box->isHorizontal() ) )
            {
                setChildNeedsLayout( true,
                                     false );    // Just go ahead and mark the positioned object as needing layout, so it will update its position properly.
            }
        }
        else
        {
...
bool RenderBox::sizesToIntrinsicLogicalWidth( LogicalWidthType widthType ) const
{
    ...
    // FIXME: Think about block-flow here.
    // https://bugs.webkit.org/show_bug.cgi?id=46473
    if ( logicalWidth.type() == Auto && !( parent()->isFlexibleBox() && parent()->style()->boxOrient() == VERTICAL
                                           && parent()->style()->boxAlign() == BSTRETCH ) && node() && ( node()->hasTagName( inputTag ) || node()->hasTagName( selectTag )
                                                   || node()->hasTagName( buttonTag ) || node()->hasTagName( textareaTag ) || node()->hasTagName( legendTag ) ) )
    {
        return true;
    }
...
void RenderBox::computeInlineDirectionMargins( RenderBlock *containingBlock, int containerWidth, int childWidth )
{
    ...
    bool pushToEndFromTextAlign = !marginEndLength.isAuto() && ( ( !containingBlockStyle->isLeftToRightDirection()
...
void RenderBox::computeLogicalHeight()
{
    ...
    if ( stretchesToViewport() || paginatedContentNeedsBaseHeight )
    {
        ...
        {
            int marginsBordersPadding = margins + parentBox()->marginBefore() + parentBox()->marginAfter() +
...
int RenderBox::computePercentageLogicalHeight( const Length &height )
{
    ...
    while ( !cb->isRenderView() && !cb->isBody() && !cb->isTableCell() && !cb->isPositioned()
    ...
    // https://bugs.webkit.org/show_bug.cgi?id=46500
    bool isPositionedWithSpecifiedHeight = cb->isPositioned() && ( !cb->style()->logicalHeight().isAuto()
...
int RenderBox::computeReplacedLogicalWidthUsing( Length logicalWidth ) const
{
    switch ( logicalWidth.type() )
    {
        ...
        case Percent:
        {
            ...
            const int cw = isPositioned() ? containingBlockLogicalWidthForPositioned( toRenderBoxModelObject(
                               container() ) ) : containingBlockLogicalWidthForContent();
...
int RenderBox::computeReplacedLogicalHeight() const
{
    ...
    int maxLogicalHeight = style()->logicalMaxHeight().isUndefined() ? logicalHeight : computeReplacedLogicalHeightUsing(
...
int RenderBox::computeReplacedLogicalHeightUsing( Length logicalHeight ) const
{
    switch ( logicalHeight.type() )
    {
        ...
        case Percent:
        {
            ...
            // containing block's block-flow.
            // https://bugs.webkit.org/show_bug.cgi?id=46496
            int availableHeight = isPositioned() ? containingBlockLogicalHeightForPositioned( toRenderBoxModelObject( cb ) ) : toRenderBox(
                                      cb )->availableLogicalHeight();
...
int RenderBox::availableLogicalHeightUsing( const Length &h ) const
{
    ...
    if ( isRenderView() )
    {
        return isHorizontalWritingMode() ? toRenderView( this )->frameView()->visibleHeight() : toRenderView(
                   this )->frameView()->visibleWidth();
    }
    ...
    // FIXME: We can't just check top/bottom here.
    // https://bugs.webkit.org/show_bug.cgi?id=46500
    if ( isRenderBlock() && isPositioned() && style()->height().isAuto() && !( style()->top().isAuto()
            || style()->bottom().isAuto() ) )
    {
...
int RenderBox::containingBlockLogicalWidthForPositioned( const RenderBoxModelObject *containingBlock,
...
int RenderBox::containingBlockLogicalHeightForPositioned( const RenderBoxModelObject *containingBlock,
        bool checkForPerpendicularWritingMode ) const
{
    if ( checkForPerpendicularWritingMode && containingBlock->isHorizontalWritingMode() != isHorizontalWritingMode() )
    {
...
static void computeInlineStaticDistance( Length &logicalLeft, Length &logicalRight, const RenderBox *child,
        const RenderBoxModelObject *containerBlock, int containerLogicalWidth,
...
void RenderBox::computePositionedLogicalWidth()
{
    ...
    TextDirection containerDirection = ( document()->inQuirksMode() ) ? parent()->style()->direction() :
                                       containerBlock->style()->direction();
    ...
    {
        computePositionedLogicalWidthUsing( Length( minPreferredLogicalWidth() - bordersPlusPadding, Fixed ), containerBlock,
                                            containerDirection,
                                            containerLogicalWidth, bordersPlusPadding,
                                            logicalLeft, logicalRight, marginLogicalLeft, marginLogicalRight,
                                            logicalWidthResult, marginLogicalLeftAlias, marginLogicalRightAlias, logicalLeftResult );
...
static void computeLogicalLeftPositionedOffset( int &logicalLeftPos, const RenderBox *child, int logicalWidthValue,
        const RenderBoxModelObject *containerBlock, int containerLogicalWidth )
{
    // Deal with differing writing modes here.  Our offset needs to be in the containing block's coordinate space. If the containing block is flipped
    // along this axis, then we need to flip the coordinate.  This can only happen if the containing block is both a flipped mode and perpendicular to us.
    if ( containerBlock->isHorizontalWritingMode() != child->isHorizontalWritingMode()
...
void RenderBox::computePositionedLogicalWidthUsing( Length logicalWidth, const RenderBoxModelObject *containerBlock,
        TextDirection containerDirection,
        ...
        int &logicalWidthValue, int &marginLogicalLeftValue, int &marginLogicalRightValue, int &logicalLeftPos )
{
    ...
    if ( !logicalLeftIsAuto && !logicalWidthIsAuto && !logicalRightIsAuto )
    {
        ...
        const int availableSpace = containerLogicalWidth - ( logicalLeftValue + logicalWidthValue + logicalRight.calcValue(
                                       containerLogicalWidth ) + bordersPlusPadding );
    ...
    if ( containerBlock->isRenderInline() && !containerBlock->style()->isLeftToRightDirection() )
    {
        ...
        {
            logicalLeftPos = logicalLeftValue + marginLogicalLeftValue + lastLine->borderLogicalLeft() +
...
static void computeBlockStaticDistance( Length &logicalTop, Length &logicalBottom, const RenderBox *child,
...
void RenderBox::computePositionedLogicalHeight()
{
    ...
    int &marginBeforeAlias = isHorizontal ? ( isFlipped ? m_marginBottom : m_marginTop ) : ( isFlipped ? m_marginRight:
                             m_marginLeft );
...
static void computeLogicalTopPositionedOffset( int &logicalTopPos, const RenderBox *child, int logicalHeightValue,
        const RenderBoxModelObject *containerBlock, int containerLogicalHeight )
{
    // Deal with differing writing modes here.  Our offset needs to be in the containing block's coordinate space. If the containing block is flipped
    // along this axis, then we need to flip the coordinate.  This can only happen if the containing block is both a flipped mode and perpendicular to us.
    if ( ( child->style()->isFlippedBlocksWritingMode()
            && child->isHorizontalWritingMode() != containerBlock->isHorizontalWritingMode() )
            || ( child->style()->isFlippedBlocksWritingMode() != containerBlock->style()->isFlippedBlocksWritingMode()
                 && child->isHorizontalWritingMode() == containerBlock->isHorizontalWritingMode() ) )
    {
        ...
        int &logicalHeightValue, int &marginBeforeValue, int &marginAfterValue, int &logicalTopPos )
{
    ...
    if ( !logicalTopIsAuto && !logicalHeightIsAuto && !logicalBottomIsAuto )
    {
        ...
        const int availableSpace = containerLogicalHeight - ( logicalTopValue + logicalHeightValue + logicalBottom.calcValue(
...
void RenderBox::computePositionedLogicalWidthReplaced()
{
    ...
    if ( containerBlock->isRenderInline() && !containerBlock->style()->isLeftToRightDirection() )
    {
        ...
        if ( firstLine && lastLine && firstLine != lastLine )
        {
            setLogicalLeft( logicalLeftValue + marginLogicalLeftAlias + lastLine->borderLogicalLeft() +
                            ( lastLine->logicalLeft() - firstLine->logicalLeft() ) );
            return;
        }
    }
...
int RenderBox::baselinePosition( FontBaseline baselineType, bool /*firstLine*/, LineDirectionMode direction,
...
IntPoint RenderBox::flipForWritingMode( const RenderBox *child, const IntPoint &point, FlippingAdjustment adjustment ) const
{
    ...
    if ( isHorizontalWritingMode() )
    {
        return IntPoint( point.x(), point.y() + height() - child->height() - child->y() - ( adjustment == ParentToChildFlippingAdjustment
                         ? child->y() : 0 ) );
    }

    return IntPoint( point.x() + width() - child->width() - child->x() - ( adjustment == ParentToChildFlippingAdjustment ? child->x()
                     : 0 ), point.y() );
}
...
IntPoint RenderBox::flipForWritingMode( const IntPoint &position ) const
{
    ...
    return isHorizontalWritingMode() ? IntPoint( position.x(), height() - position.y() ) : IntPoint( width() - position.x(),
            position.y() );
}
...
IntSize RenderBox::flipForWritingMode( const IntSize &offset ) const
{
    ...
    return isHorizontalWritingMode() ? IntSize( offset.width(), height() - offset.height() ) : IntSize( width() - offset.width(),
            offset.height() );
}
...
FloatPoint RenderBox::flipForWritingMode( const FloatPoint &position ) const
{
    ...
    return isHorizontalWritingMode() ? FloatPoint( position.x(), height() - position.y() ) : FloatPoint( width() - position.x(),
            position.y() );
}
...
IntSize RenderBox::locationOffsetIncludingFlipping() const
{
    ...
    containerBlock->flipForWritingMode(
        rect ); // FIXME: This is wrong if we are an absolutely positioned object enclosed by a relative-positioned inline.
