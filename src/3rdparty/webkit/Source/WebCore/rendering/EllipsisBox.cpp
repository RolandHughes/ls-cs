...
void EllipsisBox::paint( PaintInfo &paintInfo, int tx, int ty, int lineTop, int lineBottom )
{
    ...
    context->drawText( style->font(), TextRun( str.characters(), str.length(), false, 0, 0, TextRun::AllowTrailingExpansion, LTR,
                       style->visuallyOrdered() ), IntPoint( m_x + tx, m_y + ty + style->fontMetrics().ascent() ) );
    ...
    if ( m_markupBox )
    {
        ...
        ty += m_y + style->fontMetrics().ascent() - ( m_markupBox->y() + m_markupBox->renderer()->style(
                    m_firstLine )->fontMetrics().ascent() );
...
IntRect EllipsisBox::selectionRect( int tx, int ty )
{
    ...
    return enclosingIntRect( f.selectionRectForText( TextRun( m_str.characters(), m_str.length(), false, 0, 0,
...
void EllipsisBox::paintSelection( GraphicsContext *context, int tx, int ty, RenderStyle *style, const Font &font )
{
    ...
    // FIXME: Why is this always LTR?
    context->drawHighlightForText( font, TextRun( m_str.characters(), m_str.length(), false, 0, 0, TextRun::AllowTrailingExpansion,
                                   LTR, style->visuallyOrdered() ),
                                   IntPoint( m_x + tx, m_y + ty + y ), h, c, style->colorSpace() );
}

bool EllipsisBox::nodeAtPoint( const HitTestRequest &request, HitTestResult &result, int x, int y, int tx, int ty, int lineTop,
                               int lineBottom )
{
    tx += m_x;
    ty += m_y;
    ...
    if ( m_markupBox )
    {
        ...
        int mty = ty + style->fontMetrics().ascent() - ( m_markupBox->y() + m_markupBox->renderer()->style(
                      m_firstLine )->fontMetrics().ascent() );
