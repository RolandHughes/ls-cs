...
void RenderLayer::updateLayerPosition()
{
    ...
    if ( renderer()->isPositioned() && enclosingPositionedAncestor() )
    {
        ...
        if ( renderer()->isPositioned() && positionedParent->renderer()->isRelPositioned()
                && positionedParent->renderer()->isRenderInline() )
        {
...
RenderLayer *RenderLayer::stackingContext() const
{
    ...
    while ( layer && !layer->renderer()->isRenderView() && !layer->renderer()->isRoot()
            && layer->renderer()->style()->hasAutoZIndex() )
    {
        layer = layer->parent();
    }
...
void RenderLayer::insertOnlyThisLayer()
{
    if ( !m_parent && renderer()->parent() )
    {
        ...
        ASSERT( parentLayer );
        RenderLayer *beforeChild = parentLayer->reflectionLayer() != this ? renderer()->parent()->findNextLayer( parentLayer,
                                   renderer() ) : 0;
...
void RenderLayer::convertToLayerCoords( const RenderLayer *ancestorLayer, int &xPos, int &yPos ) const
{
...
void RenderLayer::scrollTo( int x, int y )
{
    ...
    renderer()->node()->document()->eventQueue()->enqueueOrDispatchScrollEvent( renderer()->node(),
            EventQueue::ScrollEventElementTarget );
}

void RenderLayer::scrollRectToVisible( const IntRect &rect, bool scrollToAnchor, const ScrollAlignment &alignX,
...
IntRect RenderLayer::getRectToExpose( const IntRect &visibleRect, const IntRect &exposeRect, const ScrollAlignment &alignX,
                                      const ScrollAlignment &alignY )
{
...
void RenderLayer::autoscroll()
{
    ...
    scrollRectToVisible( IntRect( currentDocumentPosition, IntSize( 1, 1 ) ), false, ScrollAlignment::alignToEdgeIfNeeded,
...
void RenderLayer::updateOverflowStatus( bool horizontalOverflow, bool verticalOverflow )
{
    ...
    if ( horizontalOverflowChanged || verticalOverflowChanged )
    {
        ...
        {
            frameView->scheduleEvent( OverflowEvent::create( horizontalOverflowChanged, horizontalOverflow, verticalOverflowChanged,
                                      verticalOverflow ),
                                      renderer()->node() );
        }
    }
...
void RenderLayer::updateScrollInfoAfterLayout()
{
    ...
    if ( m_hBar )
    {
        ...
        int pageStep = max( max<int>( clientWidth * Scrollbar::minFractionToStepWhenPaging(),
                                      clientWidth - Scrollbar::maxOverlapBetweenPages() ), 1 );
    ...
    if ( m_vBar )
    {
        ...
        int pageStep = max( max<int>( clientHeight * Scrollbar::minFractionToStepWhenPaging(),
                                      clientHeight - Scrollbar::maxOverlapBetweenPages() ), 1 );
...
void RenderLayer::paintOverlayScrollbars( GraphicsContext *p, const IntRect &damageRect, PaintBehavior paintBehavior,
...
static void performOverlapTests( OverlapTestRequestMap &overlapTestRequests, const RenderLayer *rootLayer,
                              ...
                              PaintLayerFlags paintFlags )
{
    ...
    if ( paintsWithTransform( paintBehavior ) && !( paintFlags & PaintLayerAppliedTransform ) )
    {
        ...
        // Apply the transform.
        {
            ...
            paintLayer( this, p, transform.inverse().mapRect( paintDirtyRect ), paintBehavior, paintingRoot, overlapTestRequests,
    ...
    if ( m_reflection && !m_paintingInsideReflection )
    {
        ...
        reflectionLayer()->paintLayer( rootLayer, p, paintDirtyRect, paintBehavior, paintingRoot, overlapTestRequests,
                                       localPaintFlags | PaintLayerPaintingReflection );
        m_paintingInsideReflection = false;
    }
                             ...
                             PaintLayerFlags paintFlags )
{
    ...
    for ( size_t i = 0; i < list->size(); ++i )
    {
        ...
        {
            paintPaginatedChildLayer( childLayer, rootLayer, p, paintDirtyRect, paintBehavior, paintingRoot, overlapTestRequests,
                                      paintFlags );
        }
    }
}
        ...
        PaintLayerFlags paintFlags )
{
    ...
    paintChildLayerIntoColumns( childLayer, rootLayer, context, paintDirtyRect, paintBehavior, paintingRoot, overlapTestRequests,
                                paintFlags, columnLayers, columnLayers.size() - 1 );
}
        ...
        PaintLayerFlags paintFlags, const Vector<RenderLayer *> &columnLayers, size_t colIndex )
{
    ...
    for ( unsigned i = 0; i < colCount; i++ )
    {
        ...
        IntSize offset = isHorizontal ? IntSize( logicalLeftOffset, currLogicalTopOffset ) : IntSize( currLogicalTopOffset,
                         logicalLeftOffset );
        ...
        if ( !localDirtyRect.isEmpty() )
        {
            ...
            else
            {
                ...
                paintChildLayerIntoColumns( childLayer, columnLayers[colIndex - 1], context, transform.inverse().mapRect( localDirtyRect ),
                                            paintBehavior,
...
static bool isHitCandidate( const RenderLayer *hitLayer, bool canDepthSort, double *zOffset,
...
RenderLayer *RenderLayer::hitTestLayer( RenderLayer *rootLayer, RenderLayer *containerLayer, const HitTestRequest &request,
                                        ...
                                        const HitTestingTransformState *transformState, double *zOffset )
{
    ...
    if ( transform() && !appliedTransform )
    {
        ...
        return hitTestLayer( this, containerLayer, request, result, localHitTestRect, localPoint, true, newTransformState.get(),
                             zOffset );
    }
    ...
    if ( fgRect.intersects( hitTestArea ) && isSelfPaintingLayer() )
    {
        ...
        HitTestResult tempResult( result.point(), result.topPadding(), result.rightPadding(), result.bottomPadding(),
                                  result.leftPadding() );
...
bool RenderLayer::hitTestContents( const HitTestRequest &request, HitTestResult &result, const IntRect &layerBounds,
                                   const IntPoint &hitTestPoint, HitTestFilter hitTestFilter ) const
{
                                       ...
                                       bool depthSortDescendants )
{
    ...
    for ( int i = list->size() - 1; i >= 0; --i )
    {
        ...
        if ( childLayer->isPaginated() )
        {
            hitLayer = hitTestPaginatedChildLayer( childLayer, rootLayer, request, tempResult, hitTestRect, hitTestPoint, transformState,
                                                   zOffsetForDescendants );
        }
        else
        {
            hitLayer = childLayer->hitTestLayer( rootLayer, this, request, tempResult, hitTestRect, hitTestPoint, false, transformState,
                                                 zOffsetForDescendants );
        }
...
RenderLayer *RenderLayer::hitTestPaginatedChildLayer( RenderLayer *childLayer, RenderLayer *rootLayer,
        const HitTestRequest &request, HitTestResult &result,
...
RenderLayer *RenderLayer::hitTestChildLayerColumns( RenderLayer *childLayer, RenderLayer *rootLayer,
        const HitTestRequest &request, HitTestResult &result,
        const IntRect &hitTestRect, const IntPoint &hitTestPoint, const HitTestingTransformState *transformState, double *zOffset,
    ...
    for ( i = colCount - 1; i >= 0; i-- )
    {
        ...
        if ( !localClipRect.isEmpty() && localClipRect.intersects( result.rectForPoint( hitTestPoint ) ) )
        {
            ...
            if ( !columnIndex )
            {
                ...
                childLayer->m_transform = adoptPtr( new TransformationMatrix( newTransform ) );
                hitLayer = childLayer->hitTestLayer( rootLayer, columnLayers[0], request, result, localClipRect, hitTestPoint, false,
            ...
            else
            {
                ...
                RefPtr<HitTestingTransformState> newTransformState = nextLayer->createLocalTransformState( rootLayer, nextLayer, localClipRect,
                        hitTestPoint, transformState );
...
void RenderLayer::calculateClipRects( const RenderLayer *rootLayer, ClipRects &clipRects, bool useCached,
...
void RenderLayer::parentClipRects( const RenderLayer *rootLayer, ClipRects &clipRects, bool temporaryClipRects,
                                   OverlayScrollbarSizeRelevancy relevancy ) const
{
    ASSERT( parent() );
...
IntRect RenderLayer::backgroundClipRect( const RenderLayer *rootLayer, bool temporaryClipRects,
        OverlayScrollbarSizeRelevancy relevancy ) const
{
...
bool RenderLayer::intersectsDamageRect( const IntRect &layerBounds, const IntRect &damageRect,
                                        const RenderLayer *rootLayer ) const
{
...
{
    RenderObject *actualRenderer = renderer()->node() ? renderer()->node()->shadowAncestorNode()->renderer() : renderer();
    RefPtr<RenderStyle> corner = renderer()->hasOverflowClip() ? actualRenderer->getUncachedPseudoStyle( SCROLLBAR_CORNER,
                                 actualRenderer->style() ) : PassRefPtr<RenderStyle>( 0 );
...
{
    RenderObject *actualRenderer = renderer()->node() ? renderer()->node()->shadowAncestorNode()->renderer() : renderer();
    RefPtr<RenderStyle> resizer = renderer()->hasOverflowClip() ? actualRenderer->getUncachedPseudoStyle( RESIZER,
                                  actualRenderer->style() ) : PassRefPtr<RenderStyle>( 0 );
...
void RenderLayer::updateReflectionStyle()
{
    ...
    switch ( renderer()->style()->boxReflect()->direction() )
    {
        case ReflectionBelow:
            transform.operations().append( TranslateTransformOperation::create( Length( 0, Fixed ), Length( 100., Percent ),
        ...
        case ReflectionRight:
            transform.operations().append( TranslateTransformOperation::create( Length( 100., Percent ), Length( 0, Fixed ),
                                           TransformOperation::TRANSLATE ) );
            transform.operations().append( TranslateTransformOperation::create( renderer()->style()->boxReflect()->offset(), Length( 0,
                                           Fixed ), TransformOperation::TRANSLATE ) );
            transform.operations().append( ScaleTransformOperation::create( -1.0, 1.0, ScaleTransformOperation::SCALE ) );
            break;
...
void showLayerTree( const WebCore::RenderLayer *layer )
{
    ...
    if ( WebCore::Frame *frame = layer->renderer()->frame() )
    {
        WTF::String output = externalRepresentation( frame,
                             WebCore::RenderAsTextShowAllLayers | WebCore::RenderAsTextShowLayerNesting | WebCore::RenderAsTextShowCompositedLayers |
                             WebCore::RenderAsTextShowAddresses | WebCore::RenderAsTextShowIDAndClass | WebCore::RenderAsTextDontUpdateLayout |
