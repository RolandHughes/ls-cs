...
void RenderMenuList::adjustInnerStyle()
{
    ...
    if ( document()->page()->chrome()->selectItemWritingDirectionIsNatural() )
    {
        ...
        TextDirection direction = ( m_buttonText
                                    && m_buttonText->text()->defaultWritingDirection() == WTF::Unicode::RightToLeft ) ? RTL : LTR;
...
void RenderMenuList::computePreferredLogicalWidths()
{
    ...
    {
        m_maxPreferredLogicalWidth = max( m_optionsWidth,
...
PopupMenuStyle RenderMenuList::itemStyle( unsigned listIndex ) const
{
    ...
    return style ? PopupMenuStyle( style->visitedDependentColor( CSSPropertyColor ), itemBackgroundColor( listIndex ), style->font(),
                                   style->visibility() == VISIBLE, style->display() == NONE, style->textIndent(), style->direction(),
                                   style->unicodeBidi() == Override ) : menuStyle();
}
...
PopupMenuStyle RenderMenuList::menuStyle() const
{
    ...
    return PopupMenuStyle( s->visitedDependentColor( CSSPropertyColor ), s->visitedDependentColor( CSSPropertyBackgroundColor ),
                           s->font(), s->visibility() == VISIBLE, s->display() == NONE, s->textIndent(), style()->direction(),
...
PassRefPtr<Scrollbar> RenderMenuList::createScrollbar( ScrollableArea *scrollableArea, ScrollbarOrientation orientation,
