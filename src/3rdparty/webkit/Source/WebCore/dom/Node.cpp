...
Node::StyleChange Node::diff( const RenderStyle *s1, const RenderStyle *s2 )
{
    ...
    {
        for ( PseudoId pseudoId = FIRST_PUBLIC_PSEUDOID; ch == NoChange
                && pseudoId < FIRST_INTERNAL_PSEUDOID; pseudoId = static_cast<PseudoId>( pseudoId + 1 ) )
        {
...
PassRefPtr<NodeList> Node::getElementsByTagNameNS( const AtomicString &namespaceURI, const AtomicString &localName )
{
    ...
    pair<NodeListsNodeData::TagNodeListCacheNS::iterator, bool> result = data->nodeLists()->m_tagNodeListCacheNS.add( QualifiedName(
                nullAtom, localNameAtom, namespaceURI ).impl(), 0 );
...
PassRefPtr<Element> Node::querySelector( const String &selectors, ExceptionCode &ec )
{
    ...
    if ( strictParsing && inDocument() && querySelectorList.hasOneSelector()
            && querySelectorList.first()->m_match == CSSSelector::Id )
    {
        ...
        if ( element && ( isDocumentNode() || element->isDescendantOf( this ) )
                && selectorChecker.checkSelector( querySelectorList.first(), element ) )
        {
            return element;
        }
...
static void traverseTreeAndMark( const String &baseIndent, const Node *rootNode, const Node *markedNode1,
                                 const char *markedLabel1, const Node *markedNode2, const char *markedLabel2 )
{
...
void Node::showTreeAndMark( const Node *markedNode1, const char *markedLabel1, const Node *markedNode2,
...
static inline bool tryAddEventListener( Node *targetNode, const AtomicString &eventType, PassRefPtr<EventListener> listener,
                                        bool useCapture )
{
...
bool Node::addEventListener( const AtomicString &eventType, PassRefPtr<EventListener> listener, bool useCapture )
{
    ...
    for ( HashSet<SVGElementInstance *>::const_iterator it = instances.begin(); it != end; ++it )
    {
        ...
        bool result = tryAddEventListener( ( *it )->shadowTreeElement(), eventType, listenerForCurrentShadowTreeElement.release(),
                                           useCapture );
...
static inline bool tryRemoveEventListener( Node *targetNode, const AtomicString &eventType, EventListener *listener,
        bool useCapture )
{
...
void Node::dispatchUIEvent( const AtomicString &eventType, int detail, PassRefPtr<Event> underlyingEvent )
{
    ...
    ASSERT( eventType == eventNames().focusinEvent || eventType == eventNames().focusoutEvent ||
            eventType == eventNames().DOMFocusInEvent || eventType == eventNames().DOMFocusOutEvent
            || eventType == eventNames().DOMActivateEvent );
...
{
    return EventDispatcher::dispatchEvent( this, KeyboardEventDispatchMediator( KeyboardEvent::create( event,
...
{
    return EventDispatcher::dispatchEvent( this, MouseEventDispatchMediator( MouseEvent::create( eventType, document()->defaultView(),
