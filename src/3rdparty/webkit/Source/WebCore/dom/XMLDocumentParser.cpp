...
{
    handleError( type, m, TextPosition1( WTF::OneBasedNumber::fromOneBasedInt( lineNumber ),
...
{
    if ( type == fatal || ( m_errorCount < maxErrors && m_lastErrorPosition.m_line != position.m_line
                            && m_lastErrorPosition.m_column != position.m_column ) )
    {
        switch ( type )
        {
            case warning:
                m_errorMessages += makeString( "warning on line ", String::number( position.m_line.oneBasedInt() ), " at column ",
                                               String::number( position.m_column.oneBasedInt() ), ": ", m );
                break;

            case fatal:
            case nonFatal:
                m_errorMessages += makeString( "error on line ", String::number( position.m_line.oneBasedInt() ), " at column ",
                                               String::number( position.m_column.oneBasedInt() ), ": ", m );
        }
...
static inline RefPtr<Element> createXHTMLParserErrorHeader( Document *doc, const String &errorMessages )
{
    ...
    reportElement->setAttribute( styleAttr,
...
void XMLDocumentParser::insertErrorMessageBlock()
{
    ...
    if ( document->transformSourceDocument() )
    {
        ...
        paragraph->appendChild(
            document->createTextNode( "This document was created as the result of an XSL transformation. The line and column numbers given are from the transformed result." ),
            ec );
...
bool XMLDocumentParser::parseDocumentFragment( const String &chunk, DocumentFragment *fragment, Element *contextElement,
        FragmentScriptingPermission scriptingPermission )
{
    ...
    if ( contextElement && ( contextElement->hasLocalName( HTMLNames::scriptTag )
