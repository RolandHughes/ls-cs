...
static EventTarget *findElementInstance( Node *referenceNode )
{
    ...
    for ( Node *n = referenceNode; n; n = n->parentNode() )
    {
        ...
        if ( SVGElementInstance *instance = static_cast<SVGUseElement *>( shadowTreeParentElement )->instanceForShadowTreeElement(
                                                referenceNode ) )
        {
            return instance;
        }
...
void EventDispatcher::dispatchSimulatedClick( Node *node, PassRefPtr<Event> underlyingEvent, bool sendMouseEvents,
        bool showPressedLook )
{
    ...
    if ( sendMouseEvents )
    {
        dispatcher.dispatchEvent( SimulatedMouseEvent::create( eventNames().mousedownEvent, node->document()->defaultView(),
                                  underlyingEvent ) );
    }
    ...
    if ( sendMouseEvents )
    {
        dispatcher.dispatchEvent( SimulatedMouseEvent::create( eventNames().mouseupEvent, node->document()->defaultView(),
                                  underlyingEvent ) );
    }
    ...
    // always send click
    dispatcher.dispatchEvent( SimulatedMouseEvent::create( eventNames().clickEvent, node->document()->defaultView(),
                              underlyingEvent ) );
...
PassRefPtr<EventTarget> EventDispatcher::adjustToShadowBoundaries( PassRefPtr<Node> relatedTarget,
        const Vector<Node *> relatedTargetAncestors )
{
...
bool EventDispatcher::dispatchEvent( PassRefPtr<Event> event )
{
    ...
    InspectorInstrumentationCookie cookie = InspectorInstrumentation::willDispatchEvent( m_node->document(), *event,
                                            windowContext.window(), m_node.get(), m_ancestors );
