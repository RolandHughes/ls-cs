...
public:
    ...
    {
        return m_referenceNode.node.get();
    }
    ...
    {
        return nextNode( scriptStateFromNode( mainThreadNormalWorld(), referenceNode() ), ec );
    }
    PassRefPtr<Node> previousNode( ExceptionCode &ec )
    {
        return previousNode( scriptStateFromNode( mainThreadNormalWorld(), referenceNode() ), ec );
    }
