...
inline Range::Range( PassRefPtr<Document> ownerDocument, PassRefPtr<Node> startContainer, int startOffset,
...
PassRefPtr<Range> Range::create( PassRefPtr<Document> ownerDocument, PassRefPtr<Node> startContainer, int startOffset,
                                 PassRefPtr<Node> endContainer, int endOffset )
{
...
{
    return adoptRef( new Range( ownerDocument, start.containerNode(), start.computeOffsetInContainerNode(), end.containerNode(),
...
PassRefPtr<DocumentFragment> Range::processContents( ActionType action, ExceptionCode &ec )
{
    ...
    {
        leftContents = processContentsBetweenOffsets( action, 0, m_start.container(), m_start.offset(),
                       lengthOfContentsInNode( m_start.container() ), ec );
        leftContents = processAncestorsAndTheirSiblings( action, m_start.container(), ProcessContentsForward, leftContents,
                       commonRoot.get(), ec );
    }
    ...
    if ( m_end.container() != commonRoot && commonRoot->contains( m_end.container() ) )
    {
        ...
        rightContents = processAncestorsAndTheirSiblings( action, m_end.container(), ProcessContentsBackward, rightContents,
                        commonRoot.get(), ec );
    }
...
static inline void deleteCharacterData( PassRefPtr<CharacterData> data, unsigned startOffset, unsigned endOffset,
                                        ExceptionCode &ec )
{
...
void Range::processNodes( ActionType action, Vector<RefPtr<Node> > &nodes, PassRefPtr<Node> oldContainer,
                          PassRefPtr<Node> newContainer, ExceptionCode &ec )
{
...
PassRefPtr<Node> Range::processAncestorsAndTheirSiblings( ActionType action, Node *container, ContentsProcessDirection direction,
        PassRefPtr<Node> passedClonedContainer, Node *commonRoot, ExceptionCode &ec )
{
    ...
    RefPtr<Node> firstChildInAncestorToProcess = direction == ProcessContentsForward ? container->nextSibling() :
...
static void adjustFloatQuadsForScrollAndAbsoluteZoomAndPageScale( Vector<FloatQuad> &quads, Document *document,
