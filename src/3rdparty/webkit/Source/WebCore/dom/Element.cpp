...
PassRefPtr<DocumentFragment> Element::deprecatedCreateContextualFragment( const String &markup,
...
void Element::scrollIntoView( bool alignToTop )
{
    ...
    if ( renderer() )
    {
        ...
        if ( alignToTop )
        {
            renderer()->enclosingLayer()->scrollRectToVisible( bounds, false, ScrollAlignment::alignToEdgeIfNeeded,
...
void Element::scrollIntoViewIfNeeded( bool centerIfNeeded )
{
    ...
    if ( renderer() )
    {
        if ( centerIfNeeded )
        {
            renderer()->enclosingLayer()->scrollRectToVisible( bounds, false, ScrollAlignment::alignCenterIfNeeded,
...
{
    return ( name.localName().endsWith( hrefAttr.localName() ) || name == srcAttr || name == actionAttr )
           && protocolIsJavaScript( stripLeadingAndTrailingHTMLSpaces( value ) );
}
...
bool Element::pseudoStyleCacheIsInvalid( const RenderStyle *currentStyle, RenderStyle *newStyle )
{
    ...
    for ( size_t i = 0; i < cacheSize; ++i )
    {
        ...
        {
            newPseudoStyle =  newStyle->getCachedPseudoStyle(
                                  VISITED_LINK ); // This pseudo-style was aggressively computed already when we first called styleForElement on the new style.
...
void Element::recalcStyle( StyleChange change )
{
    ...
    if ( hasParentStyle && ( change >= Inherit || needsStyleRecalc() ) )
    {
        ...
        if ( ch != NoChange || pseudoStyleCacheIsInvalid( currentStyle.get(), newStyle.get() ) || ( change == Force && renderer()
                && renderer()->requiresForcedStyleRecalcPropagation() ) )
        {
        ...
        if ( change != Force )
        {
            ...
            if ( document()->usesRemUnits() && ch != NoChange && currentStyle && newStyle && currentStyle->fontSize() != newStyle->fontSize()
        ...
        Node *beforeChange, Node *afterChange, int childCountDelta )
{
    ...
    if ( style->childrenAffectedByFirstChildRules() && afterChange )
    {
        ...
        for ( newFirstChild = e->firstChild(); newFirstChild
                && !newFirstChild->isElementNode(); newFirstChild = newFirstChild->nextSibling() ) {};
        ...
        if ( childCountDelta < 0 && newFirstChild == firstElementAfterInsertion && newFirstChild && newFirstChild->renderStyle()
                && !newFirstChild->renderStyle()->firstChildState() )
        {
    ...
    if ( style->childrenAffectedByLastChildRules() && beforeChange )
    {
        ...
        for ( newLastChild = e->lastChild(); newLastChild
                && !newLastChild->isElementNode(); newLastChild = newLastChild->previousSibling() ) {};
        ...
        // to match now.
        if ( ( childCountDelta < 0 || finishedParsingCallback ) && newLastChild == lastElementBeforeInsertion && newLastChild
                && newLastChild->renderStyle() && !newLastChild->renderStyle()->lastChildState() )
        {
...
void Element::setAttributeNS( const AtomicString &namespaceURI, const AtomicString &qualifiedName, const AtomicString &value,
                              ExceptionCode &ec, FragmentScriptingPermission scriptingPermission )
{
    ...
    if ( scriptingPermission == FragmentScriptingNotAllowed && ( isEventHandlerAttribute( qName )
            || isAttributeToRemove( qName, value ) ) )
    {
        return;
    }
...
RenderStyle *Element::computedStyle( PseudoId pseudoElementSpecifier )
{
    ...
    return pseudoElementSpecifier ? data->m_computedStyle->getCachedPseudoStyle( pseudoElementSpecifier ) :
           data->m_computedStyle.get();
}
