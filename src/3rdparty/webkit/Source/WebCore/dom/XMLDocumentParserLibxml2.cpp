...
public:
    ...
    void appendStartElementNSCallback( const xmlChar *xmlLocalName, const xmlChar *xmlPrefix, const xmlChar *xmlURI,
                                       int nb_namespaces,
...
PassRefPtr<XMLParserContext> XMLParserContext::createMemoryParser( xmlSAXHandlerPtr handlers, void *userData,
        const CString &chunk )
{
    if ( !didInit )
    {
...
XMLDocumentParser::XMLDocumentParser( DocumentFragment *fragment, Element *parentElement,
...
void XMLDocumentParser::doWrite( const String &parseString )
{
    ...
    if ( parseString.length() )
    {
        ...
        xmlParseChunk( context->context(), reinterpret_cast<const char *>( parseString.characters() ),
                       sizeof( UChar ) * parseString.length(), 0 );
...
static inline void handleElementNamespaces( Element *newElement, const xmlChar **libxmlNamespaces, int nb_namespaces,
...
static inline void handleElementAttributes( Element *newElement, const xmlChar **libxmlAttributes, int nb_attributes,
        ExceptionCode &ec, FragmentScriptingPermission scriptingPermission )
{
    ...
    for ( int i = 0; i < nb_attributes; i++ )
    {
        ...
        AtomicString attrQName = attrPrefix.isEmpty() ? toAtomicString( attributes[i].localname ) : AtomicString(
...
void XMLDocumentParser::startElementNs( const xmlChar *xmlLocalName, const xmlChar *xmlPrefix, const xmlChar *xmlURI,
                                        int nb_namespaces,
...
void XMLDocumentParser::endElementNs()
{
    ...
    if ( m_scriptingPermission == FragmentScriptingNotAllowed && n->isElementNode()
...
static void startElementNsHandler( void *closure, const xmlChar *localname, const xmlChar *prefix, const xmlChar *uri,
                                   int nb_namespaces, const xmlChar **namespaces, int nb_attributes, int nb_defaulted, const xmlChar **libxmlAttributes )
{
    ...
    getParser( closure )->startElementNs( localname, prefix, uri, nb_namespaces, namespaces, nb_attributes, nb_defaulted,
                                          libxmlAttributes );
}
...
HashMap<String, String> parseAttributes( const String &string, bool &attrsOK )
{
    ...
    xmlParseChunk( parser->context(), reinterpret_cast<const char *>( parseString.characters() ),
                   parseString.length() * sizeof( UChar ), 1 );
