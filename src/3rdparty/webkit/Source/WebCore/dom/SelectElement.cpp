...
void SelectElement::setSelectedIndex( SelectElementData &data, Element *element, int optionIndex, bool deselect,
                                      bool fireOnChangeNow, bool userDrivenChange )
{
        ...
        HTMLFormElement *htmlForm )
{
    ...
    if ( event->type() == eventNames().mousedownEvent && event->isMouseEvent()
        ...
        HTMLFormElement *htmlForm )
{
    ...
    else if ( event->type() == eventNames().mouseupEvent && event->isMouseEvent()
              && static_cast<MouseEvent *>( event )->button() == LeftButton
              && element->document()->frame()->eventHandler()->autoscrollRenderer() != element->renderer() )
    {
    ...
    else if ( event->type() == eventNames().keydownEvent )
    {
        ...
        if ( isSpatialNavigationEnabled( element->document()->frame() ) )
            ...
            if ( keyIdentifier == "Left" || keyIdentifier == "Right" || ( ( keyIdentifier == "Down" || keyIdentifier == "Up" )
                    && endIndex == data.activeSelectionEndIndex() ) )
            {
                return;
            }
        ...
        if ( endIndex >= 0 && handled )
        {
            ...
            bool selectNewItem = !data.multiple() || static_cast<KeyboardEvent *>( event )->shiftKey()
                                 || !isSpatialNavigationEnabled( element->document()->frame() );
    ...
    else if ( event->type() == eventNames().keypressEvent )
    {
        ...
        else if ( data.multiple() && keyCode == ' ' && isSpatialNavigationEnabled( element->document()->frame() ) )
        {
            ...
            data.setActiveSelectionState( !data.activeSelectionState() );
            updateSelectedState( data, element, listToOptionIndex( data, element, data.activeSelectionEndIndex() ), true /*multi*/,
                                 false /*shift*/ );
            listBoxOnChange( data, element );
            event->setDefaultHandled();
        }
