...
template<typename ValueArg, size_t inlineCapacity = 256, typename HashArg = typename DefaultHash<ValueArg>::Hash> class
    ListHashSet
{
...
private:
    Node *pool()
    {
        return reinterpret_cast_ptr<Node *>( m_pool.pool );
    }
    Node *pastPool()
    {
        return pool() + m_poolSize;
    }
...
template<typename T, size_t inlineCapacity, typename U>
inline typename ListHashSet<T, inlineCapacity, U>::const_iterator ListHashSet<T, inlineCapacity, U>::find(
...
public:
    ...
    static bool equal( Node *const &a, const T &b )
    {
...
inline typename ListHashSet<ValueType, inlineCapacity, U>::iterator ListHashSet<ValueType, inlineCapacity, U>::find(
    const T &value )
{
...
template<typename ValueType, size_t inlineCapacity, typename U>
template<typename T, typename HashTranslator>
inline typename ListHashSet<ValueType, inlineCapacity, U>::const_iterator ListHashSet<ValueType, inlineCapacity, U>::find(
    const T &value ) const
{
    typedef ListHashSetTranslatorAdapter<ValueType, inlineCapacity, T, HashTranslator> Adapter;
    ImplTypeConstIterator it = m_impl.template find<T, Adapter>( value );
...
{
    typedef ListHashSetTranslator<ValueType, inlineCapacity, HashFunctions> Translator;
    pair<typename ImplType::iterator, bool> result = m_impl.template add<ValueType, NodeAllocator *, Translator>( value,
            m_allocator.get() );
...
template<typename T, size_t inlineCapacity, typename U>
pair<typename ListHashSet<T, inlineCapacity, U>::iterator, bool> ListHashSet<T, inlineCapacity, U>::insertBefore( iterator it,
        const ValueType &newValue )
{
    typedef ListHashSetTranslator<ValueType, inlineCapacity, HashFunctions> Translator;
    pair<typename ImplType::iterator, bool> result = m_impl.template add<ValueType, NodeAllocator *, Translator>( newValue,
            m_allocator.get() );
...
template<typename T, size_t inlineCapacity, typename U>
pair<typename ListHashSet<T, inlineCapacity, U>::iterator, bool> ListHashSet<T, inlineCapacity, U>::insertBefore(
    const ValueType &beforeValue, const ValueType &newValue )
{
