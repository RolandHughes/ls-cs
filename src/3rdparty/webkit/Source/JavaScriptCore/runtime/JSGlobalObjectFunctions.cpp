...
static double parseInt( const UString &s, int radix )
{
    ...
    if ( number >= mantissaOverflowLowerBound )
    {
        ...
        {
            number = parseIntOverflow( s.substringSharingImpl( firstDigitPosition, p - firstDigitPosition ).utf8().data(),
                                       p - firstDigitPosition, radix );
        }
    }
...
EncodedJSValue JSC_HOST_CALL globalFuncEval( ExecState *exec )
{
    ...
    {
        return throwVMError( exec, createEvalError( exec,
    ...
    return JSValue::encode( exec->interpreter()->execute( eval, exec, thisObject,
