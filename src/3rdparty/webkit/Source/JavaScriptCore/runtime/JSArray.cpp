...
static inline size_t storageSize( unsigned vectorLength )
{
    ...
    ASSERT( ( ( size - ( sizeof( ArrayStorage ) - sizeof( JSValue ) ) ) / sizeof( JSValue ) == vectorLength )
            && ( size >= ( sizeof( ArrayStorage ) - sizeof( JSValue ) ) ) );
...
NEVER_INLINE void JSArray::putSlowCase( ExecState *exec, unsigned i, JSValue value )
{
    ...
    if ( i >= MIN_SPARSE_ARRAY_INDEX )
    {
        ...
        if ( ( i > MAX_STORAGE_VECTOR_INDEX ) || !isDenseEnoughForVector( i + 1, storage->m_numValuesInVector + 1 ) )
        {
            ...
            {
                Heap::heap( this )->reportExtraMemoryCost( ( capacity - storage->reportedMapCapacity ) * ( sizeof( unsigned ) + sizeof(
                            JSValue ) ) );
...
bool JSArray::increaseVectorLength( unsigned newLength )
{
    ...
    storage = m_storage = reinterpret_cast_ptr<ArrayStorage *>( static_cast<char *>( baseStorage ) + m_indexBias * sizeof(
                              JSValue ) );
    m_storage->m_allocBase = baseStorage;
...
struct AVLTreeAbstractorForArrayCompare
{
    ...
    handle get_less( handle h )
    {
        return m_nodes[h].lt & 0x7FFFFFFF;
    }
    ...
    handle get_greater( handle h )
    {
        return m_nodes[h].gt & 0x7FFFFFFF;
    }
    ...
    int compare_key_key( key va, key vb )
    {
        ...
        else
        {
            ...
            arguments.append( va );
            arguments.append( vb );
            compareResult = call( m_exec, m_compareFunction, m_compareCallType, *m_compareCallData, m_globalThisValue,
                                  arguments ).toNumber( m_exec );
        }
    ...
    {
        return compare_key_key( k, m_nodes[h].value );
    }
    int compare_node_node( handle h1, handle h2 )
    {
        return compare_key_key( m_nodes[h1].value, m_nodes[h2].value );
    }
