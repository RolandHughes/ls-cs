...
void FunctionPrototype::addFunctionProperties( ExecState *exec, JSGlobalObject *globalObject, Structure *functionStructure,
        JSFunction **callFunction, JSFunction **applyFunction )
{
    putDirectFunctionWithoutTransition( exec, new ( exec ) JSFunction( exec, globalObject, functionStructure, 0,
                                        exec->propertyNames().toString, functionProtoFuncToString ), DontEnum );
    *applyFunction = new ( exec ) JSFunction( exec, globalObject, functionStructure, 2, exec->propertyNames().apply,
            functionProtoFuncApply );
    putDirectFunctionWithoutTransition( exec, *applyFunction, DontEnum );
    *callFunction = new ( exec ) JSFunction( exec, globalObject, functionStructure, 1, exec->propertyNames().call,
...
static inline void insertSemicolonIfNeeded( UString &functionBody )
{
    ...
    for ( size_t i = functionBody.length() - 2; i > 0; --i )
    {
        ...
        if ( !Lexer::isWhiteSpace( ch ) && !Lexer::isLineTerminator( ch ) )
        {
            if ( ch != ';' && ch != '}' )
            {
                functionBody = makeUString( functionBody.substringSharingImpl( 0, i + 1 ), ";", functionBody.substringSharingImpl( i + 1,
                                            functionBody.length() - ( i + 1 ) ) );
            }
...
EncodedJSValue JSC_HOST_CALL functionProtoFuncToString( ExecState *exec )
{
    ...
    if ( thisValue.inherits( &JSFunction::s_info ) )
    {
        ...
        return JSValue::encode( jsMakeNontrivialString( exec, "function ", function->name( exec ), "(", executable->paramString(), ") ",
                                sourceString ) );
    }
