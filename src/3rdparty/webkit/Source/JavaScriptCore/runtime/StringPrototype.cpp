...
static NEVER_INLINE UString substituteBackreferencesSlow( const UString &replacement, const UString &source, const int *ovector,
        RegExp *reg, size_t i )
{
...
static inline UString substituteBackreferences( const UString &replacement, const UString &source, const int *ovector,
        RegExp *reg )
{
...
{
    return Collator::userDefault()->collate( reinterpret_cast<const ::UChar *>( a.characters() ), a.length(),
            reinterpret_cast<const ::UChar *>( b.characters() ), b.length() );
}
...
static ALWAYS_INLINE JSValue jsSpliceSubstringsWithSeparators( ExecState *exec, JSString *sourceVal, const UString &source,
        const StringRange *substringRanges, int rangeCount, const UString *separators, int separatorCount )
{
...
EncodedJSValue JSC_HOST_CALL stringProtoFuncReplace( ExecState *exec )
{
    ...
    if ( pattern.inherits( &RegExpObject::s_info ) )
    {
        ...
        return JSValue::encode( jsSpliceSubstringsWithSeparators( exec, sourceVal, source, sourceRanges.data(), sourceRanges.size(),
    ...
    return JSValue::encode( jsString( exec, source.substringSharingImpl( 0, matchPos ), substituteBackreferences( replacementString,
                                      source, ovector, 0 ), source.substringSharingImpl( matchEnd ) ) );
}
...
EncodedJSValue JSC_HOST_CALL stringProtoFuncSlice( ExecState *exec )
{
    ...
    if ( to > from && to > 0 && from < len )
    {
        ...
        return JSValue::encode( jsSubstring( exec, s, static_cast<unsigned>( from ),
                                             static_cast<unsigned>( to ) - static_cast<unsigned>( from ) ) );
    }
