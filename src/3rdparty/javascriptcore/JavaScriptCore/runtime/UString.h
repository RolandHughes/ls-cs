...
public:
    ...
    UString spliceSubstringsWithSeparators( const Range *substringRanges, int rangeCount, const UString *separators,
...
{
    static unsigned hash( const RefPtr<JSC::UString::Rep> &key )
    {
        return key->existingHash();
    }
    static unsigned hash( JSC::UString::Rep *key )
    {
        return key->existingHash();
    }
};
                    ...
                    StringType6 string6, StringType7 string7 )
{
    ...
    UChar *buffer;
    unsigned length = adapter1.length() + adapter2.length() + adapter3.length() + adapter4.length() + adapter5.length() +
                      adapter6.length() + adapter7.length();
    PassRefPtr<UStringImpl> resultImpl = UStringImpl::tryCreateUninitialized( length, buffer );
                    ...
                    StringType6 string6, StringType7 string7, StringType8 string8 )
{
    ...
    UChar *buffer;
    unsigned length = adapter1.length() + adapter2.length() + adapter3.length() + adapter4.length() + adapter5.length() +
                      adapter6.length() + adapter7.length() + adapter8.length();
    PassRefPtr<UStringImpl> resultImpl = UStringImpl::tryCreateUninitialized( length, buffer );
...
{
    static unsigned hash( const JSC::UString::Rep *key )
    {
        return key->hash();
    }
    static bool equal( const JSC::UString::Rep *a, const JSC::UString::Rep *b )
    {
        return JSC::equal( a, b );
    }
...
template<> struct StrHash<RefPtr<JSC::UString::Rep> > : public StrHash<JSC::UString::Rep *>
{
    ...
    static bool equal( const RefPtr<JSC::UString::Rep> &a, const RefPtr<JSC::UString::Rep> &b )
    {
        return JSC::equal( a.get(), b.get() );
    }
    static bool equal( const JSC::UString::Rep *a, const RefPtr<JSC::UString::Rep> &b )
    {
        return JSC::equal( a, b.get() );
    }
    static bool equal( const RefPtr<JSC::UString::Rep> &a, const JSC::UString::Rep *b )
    {
        return JSC::equal( a.get(), b );
    }
