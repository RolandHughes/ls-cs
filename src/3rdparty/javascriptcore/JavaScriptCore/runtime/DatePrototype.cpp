...
extern "C" size_t strftime( char *const s, const size_t maxsize, const char *const format,
                            const struct tm *const t );  //provided by libce
#endif
                                 ...
                                 const ArgList &args )
{
    ...
    CFStringRef string = CFDateFormatterCreateStringWithAbsoluteTime( 0, formatter,
                         floor( timeInMilliseconds / msPerSecond ) - kCFAbsoluteTimeIntervalSince1970 );
...
JSValue JSC_HOST_CALL dateProtoFuncToISOString( ExecState *exec, JSObject *, JSValue thisValue, const ArgList & )
{
    ...
    char buffer[28];
    snprintf( buffer, sizeof( buffer ) - 1, "%04d-%02d-%02dT%02d:%02d:%02d.%03dZ", 1900 + gregorianDateTime->year,
...
static JSValue setNewValueFromTimeArgs( ExecState *exec, JSValue thisValue, const ArgList &args, int numArgsToUse,
                                        bool inputIsUTC )
{
    if ( !thisValue.inherits( &DateInstance::info ) )
    {
...
static JSValue setNewValueFromDateArgs( ExecState *exec, JSValue thisValue, const ArgList &args, int numArgsToUse,
                                        bool inputIsUTC )
{
    if ( !thisValue.inherits( &DateInstance::info ) )
    {
