...
public:
    ...
    HashSet<MarkedArgumentBuffer *> &markListSet()
    {
        if ( !m_markListSet )
        {
            m_markListSet = new HashSet<MarkedArgumentBuffer *>;
        }
...
#endif
template<> struct CellSize<sizeof( uint64_t )>
{
    static const size_t m_value = 64;
};
...
const size_t CELL_ARRAY_LENGTH = ( MINIMUM_CELL_SIZE / sizeof( double ) ) + ( MINIMUM_CELL_SIZE % sizeof( double ) != 0 ? sizeof(
                                     double ) : 0 );
const size_t CELL_SIZE = CELL_ARRAY_LENGTH * sizeof( double );
const size_t SMALL_CELL_SIZE = CELL_SIZE / 2;
const size_t CELL_MASK = CELL_SIZE - 1;
const size_t CELL_ALIGN_MASK = ~CELL_MASK;
const size_t CELLS_PER_BLOCK = ( BLOCK_SIZE - sizeof( Heap * ) ) * 8 * CELL_SIZE / ( 8 * CELL_SIZE + 1 ) /
                               CELL_SIZE; // one bitmap byte can represent 8 cells.
