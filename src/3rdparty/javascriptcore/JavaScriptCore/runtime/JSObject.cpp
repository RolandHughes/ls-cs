...
static inline void getClassPropertyNames( ExecState *exec, const ClassInfo *classInfo, PropertyNameArray &propertyNames,
    ...
    for ( JSObject *obj = this; ; obj = asObject( prototype ) )
    {
        ...
        if ( obj->getPropertyDescriptor( exec, propertyName, descriptor ) )
        {
            ...
            if ( ( descriptor.isAccessorDescriptor() && !!descriptor.setter() && ( ( setterFunc = asObject( descriptor.setter() ) ), true ) )
                    || ( !!descriptor.value() && descriptor.value().isGetterSetter()
                         && ( ( setterFunc = asGetterSetter( descriptor.value() )->setter() ), true ) ) )
            {
#else
...
static bool putDescriptor( ExecState *exec, JSObject *target, const Identifier &propertyName, PropertyDescriptor &descriptor,
                           unsigned attributes, JSValue oldValue )
{
    if ( descriptor.isGenericDescriptor() || descriptor.isDataDescriptor() )
    {
        target->putWithAttributes( exec, propertyName, descriptor.value() ? descriptor.value() : oldValue,
...
bool JSObject::defineOwnProperty( ExecState *exec, const Identifier &propertyName, PropertyDescriptor &descriptor,
                                  bool throwException )
{
    ...
    if ( descriptor.isDataDescriptor() != current.isDataDescriptor() )
    {
        ...
        deleteProperty( exec, propertyName );
        return putDescriptor( exec, this, propertyName, descriptor, current.attributesWithOverride( descriptor ),
                              current.value() ? current.value() : jsUndefined() );
    }
